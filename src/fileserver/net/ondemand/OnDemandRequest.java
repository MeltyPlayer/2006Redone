...
public final class OnDemandRequest implements Comparable<OnDemandRequest> {
  ...
  /**
   * High priority - used in-game when data is required immediately but has not
   * yet been received.
   */
  HIGH,

  /**
   * Medium priority - used while loading the 'bare minimum' required to run the
   * game.
   */
  MEDIUM,

  /**
   * Low priority - used when a file is not required urgently. The client login
   * screen says "loading extra files.." when low priority loading is being
   * performed.
   */
  LOW;
