...
public class RSApplet extends Applet implements Runnable, MouseListener,
	...
	protected final void initClientFrame(int i, int j) {
		myWidth = j;
		myHeight = i;
		graphics = getGameComponent().getGraphics();
		fullGameScreen = new RSImageProducer(myWidth, myHeight,
	...
	protected final void method4(int i) {
		delayTime = 1000 / i;
	}
	...
	public final void mousePressed(MouseEvent mouseevent) {
		...
		if (gameFrame != null) {
			i -= UiConstants.MARGIN_X;
			j -= UiConstants.MARGIN_Y;
		}
		idleTime = 0;
		clickX = i;
		clickY = j;
	...
	protected final int readChar(int dummy) {
		while (dummy >= 0) {
			for (int j = 1; j > 0; j++) {
				;
			}
