...
public final class Sprite extends DrawingArea {
  ...
  public void method345() {
    /*
     * int ai[] = new int[trimWidth * trimHeight]; for (int j = 0; j < height;
     * j++) { System.arraycopy(pixels, j * width, ai, j + anInt1443 * trimWidth
     * + anInt1442, width); }
     * 
     * pixels = ai; width = trimWidth; height = trimHeight; anInt1442 = 0;
     * anInt1443 = 0;
     */
    int ai[] = new int[trimWidth * trimHeight];
    for (int j = 0; j < height; j++) {
      for (int k = 0; k < width; k++)
        ai[(j + anInt1443) * trimWidth + (k + anInt1442)] = pixels[j * width + k];
    }
    pixels = ai;
    width = trimWidth;
    height = trimHeight;
