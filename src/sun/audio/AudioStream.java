...
public final class AudioStream extends FilterInputStream {
  ...
  /*
   * create the AudioStream; if we survive without throwing an exception, we
   * should now have some subclass of ACIS with all the header info already read
   */
  ...
  public AudioData getData() throws IOException {
     ...
     * acis.setData();
     * 
     * if (acis.stream instanceof ByteArrayInputStream) { Format[] format =
     * acis.getFormat(); byte[] bytes = acis.getBytes(); if (bytes == null)
     * throw new
     * IOException("could not create AudioData object: no data received");
