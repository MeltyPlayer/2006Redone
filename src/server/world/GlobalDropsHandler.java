  ...
  public static void initialize() {
    ...
    try {
      ...
      while ((Data = Checker.readLine()) != null) {
        ...
        globalDrops.add(new GlobalDrop(Integer.parseInt(args[0]), Integer.parseInt(args[1]), Integer.parseInt(args[2]),
              ...
              if (drop.isTaken()) {
                if (System.currentTimeMillis() - drop.getTakenAt() >= TIME_TO_RESPAWN * 1000) {
                  ...
                  if (client.distanceToPoint(drop.getX(), drop.getY()) <= 60) {
                    client.getActionSender().createGroundItem(drop.getId(), drop.getX(), drop.getY(), drop.getAmount(),
                        0);
                  }
                }
              }
