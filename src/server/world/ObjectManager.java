...
public class ObjectManager {
  ...
  public static void objectTicks(final Client player, final int objectId, final int objectX, final int objectY,
      final int objectH, final int face, final int objectType, int ticks) {
  ...
  public static void singleGateTicks(final Client player, final int objectId, final int objectX, final int objectY,
      final int x1, final int y1, final int objectH, final int face, int ticks) {
    CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
      @Override
      public void execute(CycleEventContainer container) {
  ...
  public static void doubleGateTicks(final Client player, final int objectId, final int objectX, final int objectY,
      final int x1, final int y1, final int x2, final int y2, final int objectH, final int face, int ticks) {
    CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
      @Override
      public void execute(CycleEventContainer container) {
        if (DoubleGates.gateAmount == 0) {
  ...
  public void process() {
    ...
    for (final Object o : toRemove) {
       ...
       * (Client) player; Server.itemHandler.createGroundItem(c, 592, o.objectX,
       * o.objectY, 1, c.playerId); if (c.playerIsCooking) {
    ...
    for (Player player : PlayerHandler.players) {
      if (player != null) {
        ...
        if (c.goodDistance(c.getX(), c.getY(), obeliskCoords[port][0] + 2, obeliskCoords[port][1] + 2, 1)) {
          c.getPlayerAssistant().startTeleport2(obeliskCoords[random][0] + xOffset, obeliskCoords[random][1] + yOffset,
              0);
        }
      }
    }
