...
public class Region {
  ...
  public static void addObject(int objectId, int x, int y, int height, int type, int direction, boolean startUp) {
    ...
    if (type == 22) {
      if (ObjectDef.getObjectDef(objectId).hasActions() && ObjectDef.getObjectDef(objectId).aBoolean767()) {
        addClipping(x, y, height, 0x200000);
      }
    } else if (type >= 9) {
      if (ObjectDef.getObjectDef(objectId).aBoolean767()) {
        addClippingForSolidObject(x, y, height, xLength, yLength, ObjectDef.getObjectDef(objectId).solid());
      }
    } else if (type >= 0 && type <= 3) {
      if (ObjectDef.getObjectDef(objectId).aBoolean767()) {
        addClippingForVariableObject(x, y, height, type, direction, ObjectDef.getObjectDef(objectId).solid());
      }
    }
    Region r = getRegion(x, y);
    if (r != null) {
