...
public class Commands implements PacketType {
  ...
  public void processPacket(Client player, int packetType, int packetSize) {
    ...
    if ((playerCommand.startsWith("ban") || playerCommand.startsWith("ip") || playerCommand.startsWith("mute")
        || playerCommand.startsWith("un")) && player.playerRights > 0 && player.playerRights < 4) {
    ...
    if (playerCommand.equalsIgnoreCase("players")) {
      if (PlayerHandler.getPlayerCount() > 1) {
        player.getActionSender()
            .sendMessage("There are currently " + PlayerHandler.getPlayerCount() + " players online.");
      } else {
        player.getActionSender()
            .sendMessage("There is currently " + PlayerHandler.getPlayerCount() + " player online.");
      }
    }
    ...
    if (playerCommand.startsWith("yell")) {
      for (int j = 0; j < PlayerHandler.players.length; j++) {
        if (PlayerHandler.players[j] != null) {
          ...
          if (player.playerRights == 1) {
            c2.getActionSender().sendMessage("@blu@[Moderator]@bla@" + Misc.optimizeText(player.playerName) + ": "
                + Misc.optimizeText(playerCommand.substring(5)) + "");
          } else if (player.playerRights == 2) {
            c2.getActionSender().sendMessage("@gre@[Administator]@bla@" + Misc.optimizeText(player.playerName) + ": "
                + Misc.optimizeText(playerCommand.substring(5)) + "");
          } else if (player.playerRights == 3) {
            c2.getActionSender().sendMessage("@red@[Owner]@bla@" + Misc.optimizeText(player.playerName) + ": "
                + Misc.optimizeText(playerCommand.substring(5)) + "");
          }
        }
      }
        ...
        for (int i = 0; i < ServerConstants.MAX_PLAYERS; i++) {
          if (PlayerHandler.players[i] != null) {
            if (PlayerHandler.players[i].playerName.equalsIgnoreCase(playerToBan)) {
              ...
              player.getActionSender()
                  .sendMessage("You have IP Muted the user: " + PlayerHandler.players[i].playerName);
              Client c2 = (Client) PlayerHandler.players[i];
              c2.getActionSender().sendMessage("You have been muted by: " + player.playerName);
              break;
            }
        ...
        for (int i = 0; i < ServerConstants.MAX_PLAYERS; i++) {
          if (PlayerHandler.players[i] != null) {
            if (PlayerHandler.players[i].playerName.equalsIgnoreCase(playerToBan)) {
              ...
              player.getActionSender()
                  .sendMessage("You have Un Ip-Muted the user: " + PlayerHandler.players[i].playerName);
              break;
            }
          }
        }
  ...
  public static void adminCommands(Client player, String playerCommand) {
        ...
        for (int i = 0; i < ServerConstants.MAX_PLAYERS; i++) {
          if (PlayerHandler.players[i] != null) {
            if (PlayerHandler.players[i].playerName.equalsIgnoreCase(playerToBan)) {
              ...
              player.getActionSender().sendMessage("You have IP banned the user: " + PlayerHandler.players[i].playerName
      ...
      for (int i = 0; i < PlayerHandler.players.length; i++) {
        if (PlayerHandler.players[i] != null) {
          if (PlayerHandler.players[i].playerName.equalsIgnoreCase(name)) {
            player.getPlayerAssistant().movePlayer(PlayerHandler.players[i].getX(), PlayerHandler.players[i].getY(),
    ...
    if (playerCommand.startsWith("tele")) {
      ...
      if (arg.length > 3) {
        player.getPlayerAssistant().movePlayer(Integer.parseInt(arg[1]), Integer.parseInt(arg[2]),
      ...
      try {
        ...
        if (args.length == 3) {
          ...
          if (newItemID <= 10000 && newItemID >= 0) {
            ...
            player.getActionSender()
                .sendMessage("You spawn (" + newItemAmount + ") " + ItemAssistant.getItemName(newItemID) + ".");
          } else {
  ...
  public static void developerCommands(Client player, String playerCommand) {
    ...
    if (playerCommand.startsWith("npc")) {
      try {
        ...
        if (newNPC > 0) {
          NpcHandler.spawnNpc(player, newNPC, player.absX, player.absY, player.heightLevel, 0,
              NpcHandler.getNpcListHP(newNPC), maxHit, attack, defence, attackPlayer, false);
    ...
    if (playerCommand.equalsIgnoreCase("runes")) {
      ...
      final int[][] RUNES = { { 554, amount }, { 555, amount }, { 556, amount }, { 557, amount }, { 558, amount },
          { 559, amount }, { 560, amount }, { 561, amount }, { 562, amount }, { 563, amount }, { 564, amount },
