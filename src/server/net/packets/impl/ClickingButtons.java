...
import server.game.content.combat.magic.MagicTeleports;
import server.game.content.combat.prayer.ActivatePrayers;
...
import server.game.content.randomevents.SandwhichLady;
import server.game.content.skills.cooking.Cooking;
import server.game.content.skills.cooking.CookingTutorialIsland;
import server.game.content.skills.crafting.JewelryMaking;
import server.game.content.skills.crafting.LeatherMaking;
import server.game.content.skills.crafting.Pottery;
...
import server.game.content.skills.herblore.Herblore;
import server.game.content.skills.smithing.SilverCrafting;
...
import server.game.items.impl.ExperienceLamp;
import server.game.items.impl.Flowers;
import server.game.items.impl.LightSources;
import server.game.items.impl.Teles;
...
public class ClickingButtons implements PacketType {
  ...
  public void processPacket(final Client player, int packetType, int packetSize) {
    ...
    if (player.playerRights == 3) {
      player.getActionSender().sendMessage(player.playerName + " - actionbutton: " + actionButtonId);
    }
    ...
    switch (actionButtonId) {
      ...
      case 71074:
        if (player.clanId >= 0) {
          if (Server.clanChat.clans[player.clanId].owner.equalsIgnoreCase(player.playerName)) {
            Server.clanChat.sendLootShareMessage(player.clanId, "Lootshare has been toggled to "
                + (!Server.clanChat.clans[player.clanId].lootshare ? "on" : "off") + " by the clan leader.");
      ...
      case 4135:
        if (player.inTrade) {
          player.getActionSender().sendMessage("You can't do this in trade!");
          return;
        }
        if (player.playerLevel[6] < 15) {
          player.getPlayerAssistant().sendFrame126("You need a magic level of @blu@15 @bla@to cast bones to bananas",
              357);
        ...
        if (!player.getItemAssistant().playerHasItem(561, 1) || !player.getItemAssistant().playerHasItem(555, 2)
            || !player.getItemAssistant().playerHasItem(557, 2)) {
          player.getPlayerAssistant().sendFrame126("You do not have the correct runes to cast this spell.", 357);
          player.getPlayerAssistant().sendChatInterface(356);
          return;
        }
        if (System.currentTimeMillis() - player.boneDelay > 2000) {
          ...
          player.getActionSender().sendSound(SoundList.BONES_TO_BANNAS, 100, 0);
      ...
      case 62005:
        ...
        if (player.playerLevel[6] < 60) {
          player.getPlayerAssistant().sendFrame126("You need a magic level of @blu@60  @blu@ to cast bones to peaches.",
              357);
          player.getPlayerAssistant().sendChatInterface(356);
          return;
        }
        ...
        if (!player.getItemAssistant().playerHasItem(561, 2) || !player.getItemAssistant().playerHasItem(555, 4)
      ...
      case 58073:
        if (player.hasBankpin && !player.requestPinDelete) {
          ...
          player.getActionSender().sendMessage("[Notice] A PIN delete has been requested. Your PIN will be deleted in "
              + player.getBankPin().recovery_Delay + " days.");
          player.getActionSender().sendMessage("To cancel this change just type in the correct PIN.");
        } else {
          player.getActionSender()
              .sendMessage("[Notice] Your PIN is already pending deletion. Please wait the entire 2 days.");
          player.getPlayerAssistant().closeAllWindows();
        }
        break;
      ...
      case 58230:
        if (!player.hasBankpin) {
          ...
          player.getActionSender().sendMessage("Your PIN has been deleted as requested.");
        } else {
          player.getActionSender().sendMessage("Please enter your Bank Pin before requesting a delete.");
          player.getActionSender()
              .sendMessage("You can do this by simply opening your bank. This is to verify it's really you.");
          player.getPlayerAssistant().closeAllWindows();
        }
        break;
        ...
        if (player.autocastId > 0) {
          ...
          if (player.playerMagicBook == 1) {
            if (player.playerEquipment[player.playerWeapon] == 4675) {
              player.getActionSender().setSidebarInterface(0, 1689);
            } else {
            ...
            if (player.playerEquipment[player.playerWeapon] == 4170) {
              player.getActionSender().setSidebarInterface(0, 12050);
            } else {
              player.getActionSender().setSidebarInterface(0, 1829);
            }
          }
      ...
      case 9167:
        switch (player.dialogueAction) {
          ...
          case 222:
            ...
            return;
          case 182:
            player.getDialogueHandler().sendNpcChat1("No, I was hoping someone could help me find it though.",
        ...
        switch (player.dialogueAction) {
          ...
          case 53:
            if (player.objectId == 1294 || player.objectId == 1317) {
              player.getPlayerAssistant().startTeleport(2461, 3444, 0, "modern");
            } else {
              player.getActionSender().sendMessage("You can't teleport there, because you are already there!");
              player.getPlayerAssistant().closeAllWindows();
            }
        ...
        switch (player.dialogueAction) {
          ...
          case 53:
            if (player.objectId == 1294 || player.objectId == 1293) {
              player.getPlayerAssistant().startTeleport(3179, 3507, 0, "modern");
            } else {
              player.getActionSender().sendMessage("You can't teleport there, because you are already there!");
              player.getPlayerAssistant().closeAllWindows();
            }
      ...
      case 9157:// barrows tele to tunnels
        if (player.dialogueAction == 1) {
          ...
          player.getActionSender().sendMessage("NOTE: You are now in the wilderness...");
        } else if (player.dialogueAction == 8) {
          player.getPlayerAssistant().resetBarrows();
          player.getActionSender().sendMessage("Your barrows have been reset.");
      ...
      case 9158:
        if (player.dialogueAction == 8) {
          ...
          player.getDialogueHandler().sendNpcChat1("No I like my job as Squire, I just need some help.",
              player.talkingNpc, "Squire");
          player.nextChat = 0;
          return;
        } else if (player.dialogueAction == 183) {
      ...
      case 9178:
        ...
        if (player.dialogueAction == 122 && player.objectId == 12164 || player.objectId == 12163
            || player.objectId == 12166) {// barb
          player.getPlayerAssistant().startTeleport(3112, 3410, 0, "modern");
        } else if (player.objectId == 12165) {
          if (player.dialogueAction == 122) {
            player.getActionSender()
                .sendMessage("You can't take the canoe to barbarian village because you're already there!");
            player.getPlayerAssistant().handleCanoe();
          }
        }
        if (player.dialogueAction == 4) {
          player.getPlayerAssistant().startTeleport(3565, 3314, 0, "modern");
        }
        if (player.dialogueAction == 3) {
          player.getPlayerAssistant().startTeleport(3088, 3500, 0, "modern");
        }
      ...
      case 9179:
        ...
        if (player.dialogueAction == 122 && player.objectId == 12163 || player.objectId == 12165
            || player.objectId == 12166) {// champ
          player.getPlayerAssistant().startTeleport(3203, 3343, 0, "modern");
        } else if (player.objectId == 12164) {
          if (player.dialogueAction == 122) {
            player.getActionSender()
                .sendMessage("You can't take the canoe to the Champion Guild because you're already there!");
            player.getPlayerAssistant().handleCanoe();
          }
        }
        if (player.dialogueAction == 4) {
      ...
      case 9180:
        ...
        if (player.dialogueAction == 122 && player.objectId == 12164 || player.objectId == 12165
            || player.objectId == 12166) {// lumby
      ...
      case 9181:
        ...
        if (player.dialogueAction == 122 && player.objectId == 12163 || player.objectId == 12164
            || player.objectId == 12165) {// edge
      ...
      case 26018:
        if (player.inDuelArena()) {
          ...
          if (player.duelStatus == 2) {
            player.getPlayerAssistant().sendFrame126("Waiting for other player...", 6684);
            opponent.getPlayerAssistant().sendFrame126("Other player has accepted.", 6684);
          }
          if (opponent.duelStatus == 2) {
            opponent.getPlayerAssistant().sendFrame126("Waiting for other player...", 6684);
            player.getPlayerAssistant().sendFrame126("Other player has accepted.", 6684);
          }
          ...
          player.getActionSender().sendMessage("You can't stake out of the Duel Arena.");
        }
        break;
      ...
      case 25120:
        if (player.inDuelArena()) {
          ...
          if (o1.duelStatus == 4 && player.duelStatus == 4) {
            ...
            player.getPlayerAssistant().sendFrame126("Waiting for other player...", 6571);
      ...
      /*
       * if (System.currentTimeMillis() - duelDelay > 800 && duelCount > 0) { if
       * (duelCount != 1) { forcedChat("" + (--duelCount)); duelDelay =
      ...
      case 152:
        /*
         * if (client.performingAction) { return; }
         */
      ...
      case 13092:
        ...
        player.getPlayerAssistant().sendFrame126("Waiting for other player...", 3431);
        ...
        for (GameItem item : player.getTrading().offeredItems) {
          if (item.id > 0) {
            if (ot.getItemAssistant().freeSlots() < player.getTrading().offeredItems.size()) {
              player.getActionSender().sendMessage(
                  ot.playerName + " only has " + ot.getItemAssistant().freeSlots() + " free slots, please remove "
                      + (player.getTrading().offeredItems.size() - ot.getItemAssistant().freeSlots()) + " items.");
              ot.getActionSender()
                  .sendMessage(player.playerName + " has to remove "
                      + (player.getTrading().offeredItems.size() - ot.getItemAssistant().freeSlots())
                      + " items or you could offer them "
                      + (player.getTrading().offeredItems.size() - ot.getItemAssistant().freeSlots()) + " items.");
              player.goodTrade = false;
              ot.goodTrade = false;
              player.getPlayerAssistant().sendFrame126("Not enough inventory space...", 3431);
      ...
      case 13218:
        ...
        if (player.inTrade && player.tradeConfirmed && ot1.tradeConfirmed && !player.tradeConfirmed2) {
          ...
          player.getPlayerAssistant().sendFrame126("Waiting for other player...", 3535);
        }
