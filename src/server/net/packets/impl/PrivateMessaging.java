            ...
            if (player.friends[i1] == 0) {
              ...
              for (int i2 = 1; i2 < PlayerHandler.players.length; i2++) {
                if (PlayerHandler.players[i2] != null && PlayerHandler.players[i2].isActive
                  ...
                  if (o != null) {
                    if (PlayerHandler.players[i2].privateChat == 0 || PlayerHandler.players[i2].privateChat == 1
                        && o.getPlayerAssistant().isInPM(Misc.playerNameToInt64(player.playerName))) {
      ...
      case SEND_PM:
        ...
        if (player.getPlayerAssistant().isPlayer()) {
          GameLogger.writeLog(player.playerName, "pmsent",
              player.playerName + " said " + Misc.textUnpack(pmchatText, packetSize - 8) + "");
        }
              ...
              if (PlayerHandler.players[i2] != null && PlayerHandler.players[i2].isActive
                ...
                if (o != null) {
                  if (PlayerHandler.players[i2].privateChat == 0 || PlayerHandler.players[i2].privateChat == 1
                      && o.getPlayerAssistant().isInPM(Misc.playerNameToInt64(player.playerName))) {
                    if (friend == sendMessageToFriendId) {
                      o.getPlayerAssistant().sendPM(Misc.playerNameToInt64(player.playerName), player.playerRights,
                          pmchatText, pmchatTextSize);
                      if (player.getPlayerAssistant().isPlayer()) {
                        GameLogger.writeLog(o.playerName, "pmrecieved", player.playerName + " said to " + o.playerName
