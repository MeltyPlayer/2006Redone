...
public class ReportHandler {
  ...
  public static final String[] reportNames = { "Offensive language", "Item scamming", "Password scamming", "Bug abuse",
      "Staff impersonation", "Account Sharing/Trading", "Macroing", "Multiple Logging in",
      "Encouragig others to break the rules", "Misuse of Customer Support", "Advertising", "Real World Trading",
  ...
  public static void handleReport(Client c) throws Exception {
    ...
    if (hasSpoke(player)) {
      ...
      writeReport(
          "" + player + " was reported by " + c.playerName + ", " + reportNames[rule] + ", " + month + ", " + day + "",
          sendText + ".", reportNames[rule]);
      c.getActionSender().sendMessage("Thank you, your report has been received and will be reviewed.");
      if (mute == 1 && c.playerRights > 0) {
        c.getActionSender().sendMessage("This user is not muted yet! Go to the MODCP on the forums to mute him!");
        c.getActionSender().sendMessage("After you have muted him there, do ::update (username) to finish the mute!");
      }
      c.lastReported = player;
      c.lastReport = System.currentTimeMillis();
    } else {
      c.getActionSender().sendMessage("You can only report someone who has spoken in the last 60 seconds.");
    }
  }
  ...
  public static void writeReport(String data, String text, String file) {
    ...
    try {
      ...
      String month = getMonth(new SimpleDateFormat("MM").format(new Date()));
      String day = new SimpleDateFormat("dd").format(new Date());
      bw = new BufferedWriter(new FileWriter("C:/Users/Administrator/Dropbox/2006Redone - Reportabuses/" + file
          + " month;" + month + " day;" + day + " ms-id;" + time + ".txt", true));
      bw.write(data);
      bw.newLine();
      bw.write(text);
      bw.newLine();
