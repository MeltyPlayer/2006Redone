...
public class ItemOnNpc implements PacketType {
  ...
  public void processPacket(final Client player, int packetType, int packetSize) {
    ...
    if (player == null || player.disconnected == true || !player.getItemAssistant().playerHasItem(itemId, 1, slot)
        || NpcHandler.npcs[i] == null || NpcHandler.npcs[i].isDead || player.isDead || player.isTeleporting) {
      return;
    }
    player.faceNpc(i);
    switch (npcId) {
