    ...
    switch (interfaceId) {
      ...
      case 5064:
        ...
        if (Item.itemStackable[removeId]) {
          player.getItemAssistant().bankItem(player.playerItems[removeSlot], removeSlot,
              player.playerItemsN[removeSlot]);
        } else {
          player.getItemAssistant().bankItem(player.playerItems[removeSlot], removeSlot,
              player.getItemAssistant().itemAmount(player.playerItems[removeSlot]));
        }
        break;

      case 5382:
        player.getItemAssistant().fromBank(player.bankItems[removeSlot], removeSlot, player.bankItemsN[removeSlot]);
        break;

      case 3322:
        if (player.duelStatus <= 0) {
          if (Item.itemStackable[removeId]) {
            player.getTrading().tradeItem(removeId, removeSlot, player.playerItemsN[removeSlot]);
          } else {
            player.getTrading().tradeItem(removeId, removeSlot, 28);
          }
        } else {
          if (Item.itemStackable[removeId] || Item.itemIsNote[removeId]) {
            player.getDueling().stakeItem(removeId, removeSlot, player.playerItemsN[removeSlot]);
          } else {
            ...
            for (GameItem item : player.getTrading().offeredItems) {
              if (item.id == removeId) {
                player.getTrading().fromTrade(removeId, removeSlot,
                    player.getTrading().offeredItems.get(removeSlot).amount);
              }
            }
          } else {
            for (GameItem item : player.getTrading().offeredItems) {
          ...
          for (GameItem item : player.getDueling().stakedItems) {
            if (item.id == removeId) {
              player.getDueling().fromDuel(removeId, removeSlot,
                  player.getDueling().stakedItems.get(removeSlot).amount);
            }
          }
