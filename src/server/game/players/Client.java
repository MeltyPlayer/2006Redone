...
import server.game.content.BankPin;
import server.game.content.EmoteHandler;
...
import server.game.content.combat.prayer.PrayerData;
import server.game.content.combat.prayer.PrayerDrain;
import server.game.content.combat.range.DwarfCannon;
import server.game.content.consumables.Food;
import server.game.content.consumables.Potions;
import server.game.content.guilds.impl.RangersGuild;
import server.game.content.minigames.Barrows;
import server.game.content.minigames.Dueling;
...
import server.game.content.quests.impl.CooksAssistant;
import server.game.content.quests.impl.DoricsQuest;
import server.game.content.quests.impl.GertrudesCat;
import server.game.content.quests.impl.ImpCatcher;
import server.game.content.quests.impl.PiratesTreasure;
import server.game.content.quests.impl.RestlessGhost;
import server.game.content.quests.impl.RomeoJuliet;
import server.game.content.quests.impl.RuneMysteries;
import server.game.content.quests.impl.SheepShearer;
import server.game.content.quests.impl.VampyreSlayer;
import server.game.content.quests.impl.WitchsPotion;
import server.game.content.skills.SkillInterfaces;
import server.game.content.skills.agility.Agility;
import server.game.content.skills.agility.ApeAtollAgility;
import server.game.content.skills.agility.BarbarianAgility;
import server.game.content.skills.agility.GnomeAgility;
import server.game.content.skills.agility.PyramidAgility;
import server.game.content.skills.agility.WerewolfAgility;
import server.game.content.skills.agility.WildernessAgility;
import server.game.content.skills.cooking.Potatoes;
import server.game.content.skills.core.Mining;
import server.game.content.skills.crafting.GlassBlowing;
import server.game.content.skills.runecrafting.Runecrafting;
import server.game.content.skills.slayer.Slayer;
...
import server.game.items.impl.PotionMixing;
import server.game.items.impl.Teles;
import server.game.items.impl.Weight;
import server.game.npcs.NpcActions;
...
import server.net.packets.PacketHandler;
import server.net.packets.impl.ChallengePlayer;
...
public class Client extends Player {
  ...
  public void initialize() {
    ...
    synchronized (this) {
      ...
      if (hasNpc == true) {
        if (summonId > 0) {
          Server.npcHandler.spawnNpc3(this, summonId, absX, absY - 1, heightLevel, 0, 120, 25, 200, 200, true, false,
              true);
        }
      }
      if (isBotting == true) {
      ...
      getItemAssistant().setEquipment(playerEquipment[playerHat], 1, playerHat);
      getItemAssistant().setEquipment(playerEquipment[playerCape], 1, playerCape);
      getItemAssistant().setEquipment(playerEquipment[playerAmulet], 1, playerAmulet);
      getItemAssistant().setEquipment(playerEquipment[playerArrows], playerEquipmentN[playerArrows], playerArrows);
      getItemAssistant().setEquipment(playerEquipment[playerChest], 1, playerChest);
      getItemAssistant().setEquipment(playerEquipment[playerShield], 1, playerShield);
      getItemAssistant().setEquipment(playerEquipment[playerLegs], 1, playerLegs);
      getItemAssistant().setEquipment(playerEquipment[playerHands], 1, playerHands);
      getItemAssistant().setEquipment(playerEquipment[playerFeet], 1, playerFeet);
      getItemAssistant().setEquipment(playerEquipment[playerRing], 1, playerRing);
      getItemAssistant().setEquipment(playerEquipment[playerWeapon], playerEquipmentN[playerWeapon], playerWeapon);
    ...
    if (freezeTimer > -6) {
      ...
      if (frozenBy > 0) {
        if (PlayerHandler.players[frozenBy] == null) {
          freezeTimer = -1;
          frozenBy = -1;
        } else if (!goodDistance(absX, absY, PlayerHandler.players[frozenBy].absX, PlayerHandler.players[frozenBy].absY,
            20)) {
          freezeTimer = -1;
          frozenBy = -1;
        }
  ...
  /**
   * Outputs a send packet which is built from the data params provided towards
   * a connected user client channel.
   * 
   * @param id
  ...
  public void playSound(Client c, int SOUNDID, int delay) {
    if (Constants.SOUND) {
      ...
      /**
       * Deal with regions We dont need to play this again because you are in
       * the current region
       */
      if (c != null) {
        if (c.soundVolume >= 0) {
          if (c.goodDistance(c.absX, c.absY, absX, absY, 2)) {
            System.out.println("Playing sound " + c.playerName + ", Id: " + SOUNDID + ", Vol: " + c.soundVolume);
