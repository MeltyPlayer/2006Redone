...
public class Trading {
  ...
  public void requestTrade(int id) {
    try {
      ...
      if (!player.inTrade && o.tradeRequested && o.tradeWith == player.playerId && player.playerIsBusy() == false
  ...
  public void openTrade() {
    ...
    player.getPlayerAssistant().sendFrame126("Are you sure you want to make this trade?", 3535);
          ...
          for (GameItem item : offeredItems) {
            if (item.id == itemID) {
              if (!item.stackable) {
                ...
                o.getPlayerAssistant().sendFrame126("Trading with: " + player.playerName + " who has @gre@"
                    + player.getItemAssistant().freeSlots() + " free slots", 3417);
              } else {
  ...
  public boolean tradeItem(int itemID, int fromSlot, int amount) {
    ...
    o.getPlayerAssistant().sendFrame126(
        "Trading with: " + player.playerName + " who has @gre@" + player.getItemAssistant().freeSlots() + " free slots",
        3417);
    player.getItemAssistant().resetItems(3322);
    resetTItems(3415);
    o.getTrading().resetOTItems(3416);
        ...
        if (item.id > 0) {
          ...
          if (player.getPlayerAssistant().isPlayer()) {
            GameLogger.writeLog(o.playerName, "tradesgave",
                o.playerName + " traded " + player.playerName + " and gave " + item.amount + " " + itemName + "");
          }
        }
        if (player.getPlayerAssistant().isPlayer()) {
          GameLogger.writeLog(player.playerName, "tradesrecieved", player.playerName + " was traded by " + o.playerName
              + " and recieved " + item.amount + " " + itemName + "");
        }
      }
      player.getPlayerAssistant().removeAllWindows();
