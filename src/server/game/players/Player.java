...
public abstract class Player {
  ...
  public long lastCast = 0, homeTele, lastDesert, eventTimer, lastRunRecovery, lastButton, lastFire, lastLight,
      muteTime, waitTime, miscTimer, ladderTimer, webSlashDelay, climbDelay, lastReport = 0, lastPlayerMove, lastPoison,
      lastPoisonSip, poisonImmune, lastSpear, lastProtItem, dfsDelay, lastVeng, lastYell, teleGrabDelay, protMageDelay,
      ...
      singleCombatDelay2, reduceStat, restoreStatsDelay, logoutDelay, buryDelay, foodDelay, potDelay, doorDelay,
      doubleDoorDelay, buySlayerTimer, lastIncrease, boneDelay, botAttempts, leverDelay = 0, farmTime,
      searchObjectDelay = 0;
  ...
  public boolean initialized = false, musicOn = true, luthas, playerIsCooking, disconnected = false,
      ruleAgreeButton = false, RebuildNPCList = false, isActive = false, isKicked = false, isSkulled = false,
      friendUpdate = false, newPlayer = false, hasMultiSign = false, saveCharacter = false, mouseButton = false,
      chatEffects = true, acceptAid = false, recievedMask, nextDialogue = false, autocasting = false,
      usedSpecial = false, mageFollow = false, dbowSpec = false, craftingLeather = false, properLogout = false,
      secDbow = false, maxNextHit = false, ssSpec = false, vengOn = false, addStarter = false, accountFlagged = false,
      inPartyRoom = false, msbSpec = false, hasBankPin, enterdBankpin, firstPinEnter, requestPinDelete, secondPinEnter,
      thirdPinEnter, fourthPinEnter, hasBankpin, isBanking, isTeleporting = false, desertWarning,
      isPotionMaking = false, isGrinding = false, hasStarter, isSpinning, clickedSpinning, hasPaidBrim, isHerblore,
      herbloreI, secondHerb, playerStun, playerFletch, isWoodcutting, playerIsFiremaking, hasNpc = false,
      playerIsFishing = false, isOperate, below459 = true, splitChat, strongHold, village, needsNewTask = false,
      canSpeak = true, ignoreFrog, ratdied2 = false, fishingWhirlPool, lostDuel, diedOnTut = false, storing = false,
      rope, rope2, canWalkTutorial, closeTutorialInterface, isCrafting, showedUnfire, showedFire, isPotCrafting,
      isFiremaking, playerIsFletching, milking, stopPlayerPacket, spiritTree = false, isSmelting,
      hasPaifAnTeleport = false, isSmithing, doingAgility = false, hasPaid, canTeleport, magicCharge, isBanned = false,
      fletchNerfed, clickedVamp = false, allowFading, isBotting = false, otherBank = false, recievedReward = false,
      poison, golemSpawned = false, zombieSpawned = false, shadeSpawned = false, treeSpiritSpawned = false,
      chickenSpawned = false, clickedTree = false, filter = true, stopPlayer = false, npcCanAttack = true,
      gliderOpen = false, hasSandwhichLady = false, isHarvesting, openDuel = false, killedJad = false,
      canHealersRespawn = true, playerIsBusy = false, miningRock;

  public int thankedForDonation, saveDelay, playerKilled, gertCat, restGhost, romeojuliet, runeMist, vampSlayer,
      cookAss, doricQuest, dragonSlayerQuestStage, sheepShear, impsC, randomActions, pkPoints, totalPlayerDamageDealt,
      killedBy, lastChatId = 1, privateChat, friendSlot = 0, dialogueId, randomCoffin, newLocation, specEffect,
      specBarId, attackLevelReq, defenceLevelReq, strengthLevelReq, rangeLevelReq, magicLevelReq, slayerLevelReq,
      agilityLevelReq, followId, skullTimer, votingPoints, nextChat = 0, talkingNpc = -1, dialogueAction = 0,
      autocastId, followDistance, followId2, barrageCount = 0, delayedDamage = 0, delayedDamage2 = 0, pcPoints = 0,
      magePoints = 0, desertTreasure = 0, lastArrowUsed = -1, autoRet = 0, pcDamage = 0, xInterfaceId = 0,
      xRemoveId = 0, xRemoveSlot = 0, tzhaarToKill = 0, tzhaarKilled = 0, waveId, frozenBy = 0, poisonDamage = 0,
      teleAction = 0, bonusAttack = 0, lastNpcAttacked = 0, killCount = 0, witchspot, pirateTreasure, ptjob, cwKills,
      cwDeaths, cwGames, tzKekSpawn = 0, playerBankPin, recoveryDelay = 3, attemptsRemaining = 3, lastPinSettings = -1,
      setPinDate = -1, changePinDate = -1, deletePinDate = -1, firstPin, secondPin, thirdPin, fourthPin, bankPin1,
      bankPin2, bankPin3, bankPin4, pinDeleteDateRequested, rememberNpcIndex, lastLoginDate, selectedSkill, log = -1,
      newHerb, newItem, newXp, doingHerb, herbAmount, treeX, treeY, lastH, cookingItem, cookingObject, summonId,
      npcId2 = 0, leatherType = -1, weightCarried, teleotherType, rockX, rockY, itemUsing, tzKekTimer, bananas,
      flourAmount, grain, questPoints, questStages, teleGrabItem, teleGrabX, teleGrabY, duelCount, underAttackBy,
      underAttackBy2, wildLevel, teleTimer, respawnTimer, saveTimer = 0, teleBlockLength, poisonDelay, slayerPoints,
      blackMarks, playerEnergy = 100, SlayerMaster, teleOtherTimer = 0, teleOtherSlot = -1, tutorialProgress,
      Cookstage1 = 1, woodcuttingTree, smeltAmount, knightS, otherDirection, brightness = 3, recoilHits,
  ...
  public boolean inCWsaraBase() {
    if (absX > 2422 && absX < 2432 && absY > 3071 && absY < 3081 && heightLevel == 1) {
      return true;
    }
    return false;
  }
  ...
  public boolean inCWzammyBase() {
    if (absX > 2367 && absX < 2377 && absY > 3126 && absY < 3136 && heightLevel == 1) {
      return true;
    }
    return false;
  }
  ...
  public int npcIndex, npcClickIndex, npcType, castingSpellId, oldSpellId, spellId, hitDelay;
  public int specMaxHitIncrease, freezeDelay, freezeTimer = -6, killerId, playerIndex, oldPlayerIndex, lastWeaponUsed,
      ...
      rangeItemUsed, killingNpcIndex, totalDamageDealt, oldNpcIndex, fightMode, attackTimer;
  ...
  public boolean forcedChatUpdateRequired, inDuel, tradeAccepted, goodTrade, inTrade, tradeRequested, tradeResetNeeded,
      tradeConfirmed, tradeConfirmed2, acceptTrade, acceptedTrade;
  ...
  public boolean inBank() { // Area(top left X, bottom right X, bottom right
                            // Y, top left Y)
    return Area(3090, 3099, 3487, 3500) || Area(3089, 3090, 3492, 3498) || Area(3249, 3258, 3413, 3428)
        || Area(3180, 3191, 3432, 3448) || Area(2945, 2948, 3365, 3374) || Area(2943, 2948, 3367, 3374)
        || Area(2945, 2950, 3365, 3370) || Area(3009, 3018, 3352, 3359) || Area(3017, 3022, 3353, 3357);
  }

  public boolean inLumbBuilding() {
    return Area(3205, 3216, 3209, 3228) || Area(3229, 3233, 3206, 3208) || Area(3228, 3233, 3201, 3205)
        || Area(3230, 3237, 3195, 3198) || Area(3238, 3229, 3209, 3211) || Area(3240, 3247, 3204, 3215)
        || Area(3247, 3252, 3190, 3195) || Area(3227, 3230, 3212, 3216) || Area(3227, 3230, 3221, 3225)
        || Area(3229, 3232, 3236, 3241) || Area(3209, 3213, 3243, 3250) || Area(3222, 3229, 3252, 3257)
        || Area(3184, 3192, 3270, 3275) || Area(3222, 3224, 3292, 3294) || Area(3225, 3230, 3287, 3228)
        || Area(3243, 3248, 3244, 3248) || Area(3202, 3205, 3167, 3170) || Area(3231, 3238, 3151, 3155)
        || Area(3233, 3234, 3156, 3156) || Area(3163, 3170, 3305, 3308) || Area(3165, 3168, 3303, 3310);
  }

  public boolean inDraynorBuilding() {
    return Area(3097, 3102, 3277, 3281) || Area(3088, 3092, 3273, 3276) || Area(3096, 3102, 3266, 3270)
        || Area(3089, 3095, 3265, 3268) || Area(3083, 3088, 3256, 3261) || Area(3087, 3094, 3251, 3255)
        || Area(3121, 3130, 3240, 3246) || Area(3102, 3112, 3162, 3165) || Area(3107, 3111, 3166, 3166)
        || Area(3103, 3115, 3157, 3161) || Area(3105, 3114, 3156, 3156) || Area(3105, 3113, 3155, 3155)
  ...
  public boolean goodDistance(int objectX, int objectY, int playerX, int playerY, int distance) {
    for (int i = 0; i <= distance; i++) {
      for (int j = 0; j <= distance; j++) {
        if (objectId == 2282 || objectId == 10883 || objectId == 2322 || objectId == 4493 || objectId == 12164
            || objectId == 1721 || objectId == 1722 || objectId == 4304 && playerX == 2619 && playerY == 3667) {
          return true;
        }
  ...
  public int getNextWalkingDirection() {
    ...
    /*
     * if (!Region.canMove(absX, absY, (absX + Misc.directionDeltaX[dir]), (absY
     * + Misc.directionDeltaY[dir]), heightLevel, 1, 1)) return -1;
     */
          ...
          if (dir != -1 && (dir & 1) == 0) {
            ...
            if (!found) {
              println_debug("Fatal: Internal error: unable to determine connection vertex!  wp1=(" + wayPointX1 + ", "
                  + wayPointY1 + "), wp2=(" + i + ", " + wayPointY2 + "), " + "first=(" + firstX + ", " + firstY + ")");
            } else {
