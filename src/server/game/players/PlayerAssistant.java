...
import server.game.content.skills.smithing.Superheat;
import server.game.items.GameItem;
import server.game.items.ItemAssistant;
import server.game.items.impl.LightSources;
import server.game.npcs.Npc;
import server.game.npcs.NpcHandler;
import server.game.players.antimacro.AntiBotting;
import server.util.GameLogger;
import server.util.Misc;
import server.world.TileControl;
import server.world.clip.PathFinder;
...
public class PlayerAssistant {
  ...
  public void loginScreen() {
    showInterface(15244);
    sendFrame126("Welcome to " + ServerConstants.SERVER_NAME + "\\n", 15257);
    ...
    if (player.lastLoginDate <= 0) {
      ...
      sendFrame126(
          "You last login @red@" + (currentDay > 1 ? (currentDay + " @bla@days ago") : ("ealier today")) + " @bla@",
          15258);
    }
    sendFrame126("" + ServerConstants.SERVER_NAME
    ...
    sendFrame126("Remember to vote daily to help " + ServerConstants.SERVER_NAME + "", 15803);
  ...
  public ArrayList<GameItem> randomFish(int fish) {
    ...
    while (done != fish) {
      ...
      } else if (random >= 70 - Server.trawler.chanceByLevel(player, 381)) {
        if (turtles) {
          turt++;
        }
      } else if (random >= 40) {
      ...
      if (!all) {
        if (player.getItemAssistant().freeSlots() != 0) {
          if (player.fishingTrawlerReward.get(slot).amount >= 1) {
            player.getItemAssistant().addItem(player.fishingTrawlerReward.get(slot).id, 1);
  ...
  public int backupItems[] = new int[ServerConstants.BANK_SIZE];
  public int backupItemsN[] = new int[ServerConstants.BANK_SIZE];
  ...
  public void sendSidebars() {
    for (int i = 0; i < ServerConstants.SIDEBARS.length; i++) {
      player.getActionSender().setSidebarInterface(i, ServerConstants.SIDEBARS[i]);
   ...
   * represents percent # on interface Integer "a" represents amount %bar fills
   * From what i tested so far it goes like this:1=0%, 2=5%, 3=10%, so and so
   * fouth
   */
  ...
  public static boolean pathBlocked(Client attacker, Client victim) {
    ...
    for (int i = 0; i < ALLOWED_COORDS.length; i++) {
      if (curX == ALLOWED_COORDS[i][0] && curY == ALLOWED_COORDS[i][1] && victim.absX == ALLOWED_COORDS[i][2]
          && victim.absY == ALLOWED_COORDS[i][3]) {
        return true;
      }
    }
    for (int i = 0; i < DISABLED_COORDS.length; i++) {
      if (curX == DISABLED_COORDS[i][0] && curY == DISABLED_COORDS[i][1] && victim.absX == DISABLED_COORDS[i][2]
          && victim.absY == DISABLED_COORDS[i][3]) {
        return false;
      }
    }
  ...
  public void handleCanoe() {
    player.getDialogueHandler().sendOption4("Travel the canoe to Barbarian Village.",
        "Travel the canoe to the Champions Guild.", "Travel the canoe to Lumbridge.", "Travel the canoe to Edgeville.");
  ...
  public boolean bananasCheck() {
    ...
    switch (player.getItemAssistant().getItemCount(1963)) {
      ...
      case 8:
      case 9:
        player.getDialogueHandler().sendPlayerChat1("I'll go collect " + reqAmount + " more bannnas then come back...");
        break;
    }
    return true;
  }
  ...
  public void startTeleport(int x, int y, int height, String teleportType) {
    ...
    int[] cwitems = { 2552, 2554, 2556, 2558, 2560, 2562, 2564, 2566, 1706, 1708, 1710, 1712, 8007, 8008, 8009, 8010,
        8011 };
    ...
    if (player.inWild() && player.wildLevel > ServerConstants.NO_TELEPORT_WILD_LEVEL) {
      player.getActionSender()
          .sendMessage("You can't teleport above level " + ServerConstants.NO_TELEPORT_WILD_LEVEL + " in the wilderness.");
      return;
    }
    ...
    if (SkillHandler.isSkilling(player)) {
      player.getActionSender().sendMessage("You can't teleport while skilling!");
      return;
    }
  ...
  public void gloryTeleport(int x, int y, int height, String teleportType) {
    int[] cwitems = { 2552, 2554, 2556, 2558, 2560, 2562, 2564, 2566, 1706, 1708, 1710, 1712, 8007, 8008, 8009, 8010,
        8011 };
    for (int cwitem : cwitems) {
      if (player.inCw() || player.duelStatus > 0 && player.getItemAssistant().playerHasItem(cwitem)) {
        player.getActionSender().sendMessage("You can't teleport from Castle Wars!");
        return;
      }
    }
  ...
  public void handleEmpty() {
    player.getDialogueHandler().sendOption2("Yes, empty my inventory please.", "No, don't empty my inventory.");
  ...
  public void resetTzhaar() {
    ...
    if (!player.killedJad) {
      if (player.waveId > 1) {
        player.getItemAssistant().addItem(6529,
            ((int) ((player.waveId * player.waveId) + (((double) player.waveId / 2.0) + 0.5))));
  ...
  private static final int[][] STARTER_ITEMS = { { 1351, 1 }, { 590, 1 }, { 303, 1 }, { 315, 1 }, { 1925, 1 },
      { 1931, 1 }, { 2309, 1 }, { 1265, 1 }, { 1205, 1 }, { 1277, 1 }, { 1171, 1 }, { 841, 1 }, { 882, 25 },
      { 556, 25 }, { 558, 15 }, { 555, 6 }, { 557, 4 }, { 559, 2 } };
    ...
    for (Player player : PlayerHandler.players) {
      if (player != null && player.isActive) {
        ...
        if (t.absX >= x1 && t.absX <= x2 && t.absY >= y1 && t.absY <= y2) {
          int hit = t.playerLevel[ServerConstants.HITPOINTS] / hp;
          t.setHitDiff2(hit);
          t.setHitUpdateRequired2(true);
          t.playerLevel[ServerConstants.HITPOINTS] -= hit;
          t.getPlayerAssistant().refreshSkill(ServerConstants.HITPOINTS);
  ...
  public void loadPM(long playerName, int world) {
    ...
    if (player.getOutStream() != null && player != null) {
      if (world != 0) {
        world += 9;
      } else if (!ServerConstants.WORLD_LIST_FIX) {
        world += 1;
      }
  ...
  public void openUpBank() {
    ...
    if (player.inWild()) {
      player.getActionSender().sendMessage("You can't open up a bank in the wilderness!");
      player.getPlayerAssistant().closeAllWindows();
      return;
    }
    ...
    if (player.requestPinDelete) {
      if (player.enterdBankpin) {
        ...
        player.requestPinDelete = false;
        player.getActionSender().sendMessage("[Notice] Your PIN has been deleted. It is recommended " + "to have one.");
      }
    }
        ...
        for (int i2 = 1; i2 < PlayerHandler.players.length; i2++) {
          ...
          if (p != null && p.isActive && Misc.playerNameToInt64(p.playerName) == friend) {
            ...
            if (player.playerRights >= 2 || p.privateChat == 0
                || p.privateChat == 1 && o.getPlayerAssistant().isInPM(Misc.playerNameToInt64(player.playerName))) {
              loadPM(friend, 1);
              pmLoaded = true;
            }
            break;
  ...
  public void potionPoisonHeal(int itemId, int itemSlot, int newItemId, int healType) {
    ...
    if (!player.isDead && System.currentTimeMillis() - player.foodDelay > 2000) {
      if (player.getItemAssistant().playerHasItem(itemId, 1, itemSlot)) {
        player.getActionSender().sendMessage("You drink the " + ItemAssistant.getItemName(itemId).toLowerCase() + ".");
  ...
  public void magicOnItems(int slot, int itemId, int spellId) {
    if (!player.getItemAssistant().playerHasItem(itemId, 1, slot) || itemId == 995) {
      return;
    }
    switch (spellId) {
      case 1162: // low alch
        ...
        if (System.currentTimeMillis() - player.alchDelay > 1000) {
          ...
          for (int i : ServerConstants.ITEM_UNALCHABLE) {
            if (itemId == i) {
          ...
          if (canAlch) {
            ...
            if (player.getPlayerAssistant().isPlayer()) {
              GameLogger.writeLog(player.playerName, "alchemy", player.playerName + " cast Low Alchemy on " + itemName
                  + " for " + GameLogger.formatCurrency(value) + " coins");
            }
            ...
            addSkillXP(31, 6);
            player.getActionSender().sendSound(SoundList.LOW_ALCHEMY, 100, 0);
      ...
      case 1178: // high alch
        ...
        if (System.currentTimeMillis() - player.alchDelay > 1000) {
          ...
          if (canAlch) {
            ...
            String itemName = ItemAssistant.getItemName(itemId).toLowerCase();
            if (player.getPlayerAssistant().isPlayer()) {
              GameLogger.writeLog(player.playerName, "alchemy", player.playerName + " cast High Alchemy on " + itemName
                  + " for" + GameLogger.formatCurrency(value) + " coins");
            }
            player.getItemAssistant().deleteItem(itemId, slot, 1);
            if (itemId > 854 && itemId < 857) {
            ...
            RandomEventHandler.addRandom(player);
            addSkillXP(65, 6);
            player.getActionSender().sendSound(SoundList.HIGH_ALCHEMY, 100, 0);
            refreshSkill(6);
          }
        }
        break;
  ...
  public void vengMe() {
    if (System.currentTimeMillis() - player.lastVeng > 30000) {
      if (player.getItemAssistant().playerHasItem(557, 10) && player.getItemAssistant().playerHasItem(9075, 4)
          && player.getItemAssistant().playerHasItem(560, 2)) {
        player.vengOn = true;
        player.lastVeng = System.currentTimeMillis();
        player.startAnimation(4410);
        player.gfx100(726);
        player.getItemAssistant().deleteItem(557, player.getItemAssistant().getItemSlot(557), 10);
        player.getItemAssistant().deleteItem(560, player.getItemAssistant().getItemSlot(560), 2);
        player.getItemAssistant().deleteItem(9075, player.getItemAssistant().getItemSlot(9075), 4);
      } else {
        player.getActionSender()
            .sendMessage("You do not have the required runes to cast this spell. (9075 for astrals)");
      }
    } else {
  ...
  public void applyDead() {
    ...
    if (player.duelStatus != 6) {
      ...
      if (opponent != null) {
        ...
        if (player.getPlayerAssistant().isPlayer() && player.inWild() && player.npcIndex < 1) {
          GameLogger.writeLog(opponent.playerName, "pkingkiller", opponent.playerName + " killed " + player.playerName
              + " absX: " + player.absX + " absY: " + player.absY + "");
        }
        if (opponent.getPlayerAssistant().isPlayer() && player.inWild() && player.npcIndex < 1) {
          GameLogger.writeLog(player.playerName, "pkingkilled", player.playerName + " was killed by "
              + opponent.playerName + " absX: " + opponent.absX + " absY: " + opponent.absY + "");
        }
    ...
    if (player.inCw()) {
      ...
      if (duelOpponent.getPlayerAssistant().isPlayer()) {
        GameLogger.writeLog(player.playerName, "duelingkilled",
            player.playerName + " was killed by " + duelOpponent.playerName + " in the duel arena.");
      }
    }
  ...
  public void giveLife() {
    ...
    if (player.duelStatus <= 4) {
      if (!CastleWars.isInCw(player) && !PestControl.isInGame(player) && !PestControl.isInPcBoat(player)
          && player.tutorialProgress > 35 && FightPits.getState(player) == null && !player.inFightCaves()) {
    ...
    if (FightPits.getState(player) != null) {
      ...
      if (CastleWars.getTeamNumber(player) == 1) {
        player.getPlayerAssistant().movePlayer(2426 + Misc.random(3), 3076 - Misc.random(3), 1);
      } else {
        player.getPlayerAssistant().movePlayer(2373 + Misc.random(3), 3131 - Misc.random(3), 1);
      }
      ...
      player.getDialogueHandler().sendStatement2("Oh dear you died! Go back to the step you",
      ...
      player.getPlayerAssistant().resetTzhaar();
                                                             ...
                                                             // repawn to
                                                             // wildy
      movePlayer(ServerConstants.RESPAWN_X, ServerConstants.RESPAWN_Y, 0);
      ...
      player.getPlayerAssistant().movePlayer(ServerConstants.DUELING_RESPAWN_X + 5, ServerConstants.DUELING_RESPAWN_Y + 5, 0);
      assert o != null;
      if (o != null) {
        o.getActionSender().sendSound(122, 100, 0);
        o.getPlayerAssistant().movePlayer(ServerConstants.DUELING_RESPAWN_X + 5, ServerConstants.DUELING_RESPAWN_Y + 5, 0);
      }
  ...
  public void followPlayer() {
    ...
    boolean castingMagic = (player.usingMagic || player.mageFollow || player.autocasting || player.spellId > 0)
        && mageDistance;
  ...
  public void followNpc() {
    if (NpcHandler.npcs[player.followId] == null || NpcHandler.npcs[player.followId].isDead) {
      resetFollow();
      return;
    }
  ...
  public boolean fullVeracs() {
    return player.playerEquipment[player.playerHat] == 4753 && player.playerEquipment[player.playerChest] == 4757
        && player.playerEquipment[player.playerLegs] == 4759 && player.playerEquipment[player.playerWeapon] == 4755;
  }

  public boolean fullGuthans() {
    return player.playerEquipment[player.playerHat] == 4724 && player.playerEquipment[player.playerChest] == 4728
        && player.playerEquipment[player.playerLegs] == 4730 && player.playerEquipment[player.playerWeapon] == 4726;
  }
  ...
  public void firstTimeTutorial() {
    if (ServerConstants.TUTORIAL_ISLAND && player.tutorialProgress == 0) {
    ...
    } else if (player.tutorialProgress == 0 && !ServerConstants.TUTORIAL_ISLAND) {
      ...
      LightSources.saveBrightness(player);
      if (!player.hasBankpin) {
        player.getActionSender().sendMessage("You do not, have a bank pin it is highly recommened you get one.");
      }
    }
  }
  ...
  public void levelUp(int skill) {
    ...
    int totalLevel = getLevelForXP(player.playerXP[0]) + getLevelForXP(player.playerXP[1])
        + getLevelForXP(player.playerXP[2]) + getLevelForXP(player.playerXP[3]) + getLevelForXP(player.playerXP[4])
        + getLevelForXP(player.playerXP[5]) + getLevelForXP(player.playerXP[6]) + getLevelForXP(player.playerXP[7])
        + getLevelForXP(player.playerXP[8]) + getLevelForXP(player.playerXP[9]) + getLevelForXP(player.playerXP[10])
        + getLevelForXP(player.playerXP[11]) + getLevelForXP(player.playerXP[12]) + getLevelForXP(player.playerXP[13])
        + getLevelForXP(player.playerXP[14]) + getLevelForXP(player.playerXP[15]) + getLevelForXP(player.playerXP[16])
        + getLevelForXP(player.playerXP[17]) + getLevelForXP(player.playerXP[18]) + getLevelForXP(player.playerXP[19])
    ...
    switch (skill) {
      ...
      case 3:
        ...
        if (player.playerLevel[3] < player.getPlayerAssistant().getLevelForXP(player.playerXP[3])) {
  ...
  public boolean addSkillXP(int amount, int skill) {
    ...
    if (player.tutorialProgress < 36 && player.playerLevel[skill] == 3 && ServerConstants.TUTORIAL_ISLAND == true) {
      return false;
    }
    amount *= ServerConstants.SERVER_EXP_BONUS;
    ...
    player.getActionSender().setSkillLevel(skill, player.playerLevel[skill], player.playerXP[skill]);
    refreshSkill(skill);
    return true;
  }
  ...
  public static final int[][] RUNES = { { 560, 1, 1 }, { 565, 1, 2 }, { 562, 1, 3 }, { 558, 1, 4 } };
  ...
  public int findKiller() {
    ...
    for (int j = 0; j < PlayerHandler.players.length; j++) {
      ...
      if (player.goodDistance(player.absX, player.absY, PlayerHandler.players[j].absX, PlayerHandler.players[j].absY,
          40)
  ...
  public void checkPouch(int i) {
    ...
    player.getActionSender().sendMessage("This pouch has " + player.pouches[i] + " rune ess in it.");
  }
