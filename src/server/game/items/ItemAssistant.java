...
import server.game.items.impl.Weight;
...
public class ItemAssistant {
  ...
  private static int COMMON = Misc.random(5), UNCOMMON = Misc.random(25), RARE = Misc.random(100);

  private static final int[][] CASKET = { { 995, Misc.random(3000), COMMON }, { 1621, 1, UNCOMMON },
      { 1619, 1, UNCOMMON }, { 1617, 1, RARE }, { 987, 1, RARE }, { 985, 1, RARE }, { 1454, 1, COMMON },
  ...
  public void addCasketRewards(int itemId) {
    ...
    for (int[] element : CASKET) {
      ...
      if (Misc.random(chance) == 0 && System.currentTimeMillis() - clickTimer > 1800) {
        ...
        c.getActionSender().sendMessage("You find " + amount + " " + getItemName(item) + ".");
      } else {
  ...
  private final int[] TREE_SEEDS = { 5291, 5292, 5293, 5294, 5295, 5296, 5297, 5298, 5299, 5300, 5301, 5302, 5303, 5304,
      5315, 5316, 5313, 5314 };
  ...
  public void destroyInterface(int itemId) {
    ...
    String[][] info = { { "Are you sure you want to destroy this item?", "14174" }, { "Yes.", "14175" },
        { "No.", "14176" }, { "", "14177" }, { "You probably won't be able to", "14182" },
        { "get this item back once lost.", "14183" }, { itemName, "14184" } };// make
                                                                              // some
                                                                              // kind
                                                                              // of
                                                                              // c.getItemInfo
  ...
  public void addOrDropItem(int item, int amount) {
    if (isStackable(item) && hasFreeSlots(1)) {
      ...
      c.getActionSender().sendMessage("You have no inventory space, so the item(s) appear beneath you.");
    } else {
      addItem(item, amount);
    }
  }
  ...
  public static int getTotalAmountEquipment(Client c) {
    ...
    for (int element : c.playerEquipment) {
      total = (int) Math.floor(c.getShopAssistant().getItemShopValue(element));
    }
    return total;
  }
  ...
  public static int getTotalAmountItems(Client c) {
    ...
    for (int playerItem : c.playerItems) {
      total = (int) Math.floor(c.getShopAssistant().getItemShopValue(playerItem));
    }
    return total;
  }
  ...
  public int[][] brokenBarrows = { { 4708, 4860 }, { 4710, 4866 }, { 4712, 4872 }, { 4714, 4878 }, { 4716, 4884 },
      { 4720, 4896 }, { 4718, 4890 }, { 4720, 4896 }, { 4722, 4902 }, { 4732, 4932 }, { 4734, 4938 }, { 4736, 4944 },
      { 4738, 4950 }, { 4724, 4908 }, { 4726, 4914 }, { 4728, 4920 }, { 4730, 4926 }, { 4745, 4956 }, { 4747, 4926 },
  ...
  public void keepItem(int keepItem, boolean deleteItem) {
    ...
    for (int i = 0; i < c.playerItems.length; i++) {
      if (c.playerItems[i] - 1 > 0) {
        int inventoryItemValue = c.getShopAssistant().getItemShopValue(c.playerItems[i] - 1);
    ...
    for (int i1 = 0; i1 < c.playerEquipment.length; i1++) {
      if (c.playerEquipment[i1] > 0) {
        int equipmentItemValue = c.getShopAssistant().getItemShopValue(c.playerEquipment[i1]);
      ...
      if (o != null) {
        if (tradeable(c.playerItems[i] - 1)) {
          ...
          if (specialCase(c.playerItems[i] - 1)) {
            Server.itemHandler.createGroundItem(o, 995, c.getX(), c.getY(), getUntradePrice(c.playerItems[i] - 1),
                c.killerId);
          }
    ...
    for (int e = 0; e < c.playerEquipment.length; e++) {
      if (o != null) {
        if (tradeable(c.playerEquipment[e])) {
          Server.itemHandler.createGroundItem(o, c.playerEquipment[e], c.getX(), c.getY(), c.playerEquipmentN[e],
              c.killerId);
        } else {
          if (specialCase(c.playerEquipment[e])) {
            Server.itemHandler.createGroundItem(o, 995, c.getX(), c.getY(), getUntradePrice(c.playerEquipment[e]),
                c.killerId);
          }
  ...
  public boolean tradeable(int itemId) {
    for (int element : ServerConstants.ITEM_TRADEABLE) {
      if (itemId == element) {
        return false;
      }
    }
    ...
    if ((freeSlots() >= 1 || playerHasItem(item, 1)) && Item.itemStackable[item]
      ...
      for (int i = 0; i < c.playerItems.length; i++) {
        if (c.playerItems[i] == item + 1 && Item.itemStackable[item] && c.playerItems[i] > 0) {
          ...
          if (c.playerItemsN[i] + amount < ServerConstants.MAXITEM_AMOUNT && c.playerItemsN[i] + amount > -1) {
            c.playerItemsN[i] += amount;
          } else {
            c.playerItemsN[i] = ServerConstants.MAXITEM_AMOUNT;
          }
          if (c.getOutStream() != null && c != null) {
      ...
      for (int i = 0; i < c.playerItems.length; i++) {
        if (c.playerItems[i] <= 0) {
          c.playerItems[i] = item + 1;
          if (amount < ServerConstants.MAXITEM_AMOUNT && amount > -1) {
            c.playerItemsN[i] = 1;
            if (amount > 1) {
  ...
  public void getBonus() {
    for (int element : c.playerEquipment) {
      if (element > -1) {
        for (int j = 0; j < ServerConstants.ITEM_LIMIT; j++) {
  ...
  public void getRequirements(String itemName, int itemId) {
    ...
    if (itemName.contains("black")) {
      if (!itemName.contains("knife") && !itemName.equalsIgnoreCase("Black skirt") && !itemName.contains("dart")
          && !itemName.contains("javelin") && !itemName.contains("thrownaxe") && !itemName.contains("vamb")
          && !itemName.contains("chap") && !itemName.equalsIgnoreCase("Black robe")) {
  ...
  public void updateSpecialBar() {
    if (c.usingSpecial) {
      c.getPlayerAssistant()
          .sendFrame126("" + (c.specAmount >= 2 ? "@yel@S P" : "@bla@S P") + ""
              + (c.specAmount >= 3 ? "@yel@ E" : "@bla@ E") + "" + (c.specAmount >= 4 ? "@yel@ C I" : "@bla@ C I") + ""
              + (c.specAmount >= 5 ? "@yel@ A L" : "@bla@ A L") + "" + (c.specAmount >= 6 ? "@yel@  A" : "@bla@  A")
              + "" + (c.specAmount >= 7 ? "@yel@ T T" : "@bla@ T T") + "" + (c.specAmount >= 8 ? "@yel@ A" : "@bla@ A")
              + "" + (c.specAmount >= 9 ? "@yel@ C" : "@bla@ C") + "" + (c.specAmount >= 10 ? "@yel@ K" : "@bla@ K"),
              c.specBarId);
    } else {
      c.getPlayerAssistant().sendFrame126("@bla@S P E C I A L  A T T A C K", c.specBarId);
    }
  }
  ...
  public boolean wearItem(int wearID, int slot) {
    ...
    if (c.tutorialProgress == 22) {
      ...
      c.getDialogueHandler().chatboxText(c, "Clothes, armour, weapons and many other items are equipped",
          ...
          "small x. Speak to the Combat Instructor to continue.", "You're now holding your dagger");
      ...
      c.getActionSender().chatbox(6180);
      c.getDialogueHandler().chatboxText(c, "", "Click on the flashing crossed swords icon to see the combat",
    ...
    if (c.playerItems[slot] == wearID + 1) {
      ...
      if (ServerConstants.itemRequirements) {
        if (targetSlot == 10 || targetSlot == 7 || targetSlot == 5 || targetSlot == 4 || targetSlot == 0
          ...
          if (c.defenceLevelReq > 0) {
            if (c.getPlayerAssistant().getLevelForXP(c.playerXP[1]) < c.defenceLevelReq) {
              c.getActionSender()
                  .sendMessage("You need a defence level of " + c.defenceLevelReq + " to wear this item.");
              canWearItem = false;
            }
          }
        ...
        if (targetSlot == 3) {
          if (c.attackLevelReq > 0) {
            if (c.getPlayerAssistant().getLevelForXP(c.playerXP[0]) < c.attackLevelReq) {
              c.getActionSender()
                  .sendMessage("You need an attack level of " + c.attackLevelReq + " to wield this weapon.");
              canWearItem = false;
            }
          }
          if (c.rangeLevelReq > 0) {
            if (c.getPlayerAssistant().getLevelForXP(c.playerXP[4]) < c.rangeLevelReq) {
              c.getActionSender()
                  .sendMessage("You need a range level of " + c.rangeLevelReq + " to wield this weapon.");
              canWearItem = false;
            }
          }
          if (c.magicLevelReq > 0) {
            if (c.getPlayerAssistant().getLevelForXP(c.playerXP[6]) < c.magicLevelReq) {
              c.getActionSender()
                  .sendMessage("You need a magic level of " + c.magicLevelReq + " to wield this weapon.");
              canWearItem = false;
            }
          }
        }
      ...
      /*
       * if (slot >= 0 && wearID >= 0) { int toEquip = c.playerItems[slot]; int
       * toEquipN = c.playerItemsN[slot]; int toRemove =
       * c.playerEquipment[targetSlot]; int toRemoveN =
  ...
  public void wearItem(int wearID, int wearAmount, int targetSlot) {
    synchronized (c) {
      if (c.getOutStream() != null && c != null) {
        ...
        c.getItemAssistant();
        c.getItemAssistant().sendWeapon(c.playerEquipment[c.playerWeapon],
            ItemAssistant.getItemName(c.playerEquipment[c.playerWeapon]));
        resetBonus();
        getBonus();
        /*
         * for (int bowId : RangeData.BOWS) { if
         * (c.playerEquipment[c.playerWeapon] == bowId) { for (int arrowId :
         * RangeData.ARROWS) { if (c.playerEquipment[c.playerArrows] == arrowId
         * && c.playerEquipment[c.playerWeapon] == bowId) { writeBonus(); } else
         * if (c.playerEquipment[c.playerWeapon] == bowId &&
         * c.playerEquipment[c.playerArrows] != arrowId) { writeBonus(); } else
         * if (c.playerEquipment[c.playerArrows] != arrowId &&
         * c.playerEquipment[c.playerWeapon] != bowId) { writeBonus(); } } } }
         */
        writeBonus();
        c.getCombatAssistant().getPlayerAnimIndex();
        c.updateRequired = true;
  ...
  public void resetBank() {
    synchronized (c) {
      ...
      c.getOutStream().writeWord(ServerConstants.BANK_SIZE);
      for (int i = 0; i < ServerConstants.BANK_SIZE; i++) {
        ...
        if (c.bankItems[i] > ServerConstants.ITEM_LIMIT || c.bankItems[i] < 0) {
          c.bankItems[i] = ServerConstants.ITEM_LIMIT;
        }
  ...
  public void resetTempItems() {
    ...
    for (int i = 0; i < itemCount + 1; i++) {
      ...
      if (c.playerItems[i] > ServerConstants.ITEM_LIMIT || c.playerItems[i] < 0) {
        c.playerItems[i] = ServerConstants.ITEM_LIMIT;
      }
      c.getOutStream().writeWordBigEndianA(c.playerItems[i]);
    }
    c.getOutStream().endFrameVarSizeWord();
  ...
  public boolean bankItem(int itemID, int fromSlot, int amount) {
    ...
    for (int i = 0; i < ServerConstants.ITEM_BANKABLE.length; i++) {
      if (itemID == ServerConstants.ITEM_BANKABLE[i]) {
    ...
    if (!Item.itemIsNote[c.playerItems[fromSlot] - 1]) {
      ...
      if (Item.itemStackable[c.playerItems[fromSlot] - 1] || c.playerItemsN[fromSlot] > 1) {
        ...
        if (!alreadyInBank && freeBankSlots() > 0) {
          ...
          if (c.bankItemsN[toBankSlot] + amount <= ServerConstants.MAXITEM_AMOUNT && c.bankItemsN[toBankSlot] + amount > -1) {
  ...
  public void moveItems(int from, int to, int moveWindow, boolean insertMode) {
    ...
    if (moveWindow == 5382 && from >= 0 && to >= 0 && from < ServerConstants.BANK_SIZE && to < ServerConstants.BANK_SIZE
  ...
  public void deleteArrow() {
    ...
    if (c.playerEquipmentN[c.playerArrows] == 1) {
      deleteEquipment(c.playerEquipment[c.playerArrows], c.playerArrows);
    }
  ...
  public void deleteEquipment() {
    ...
    if (c.playerEquipmentN[c.playerWeapon] == 1) {
      deleteEquipment(c.playerEquipment[c.playerWeapon], c.playerWeapon);
    }
