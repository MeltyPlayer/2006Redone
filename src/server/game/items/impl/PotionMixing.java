...
public class PotionMixing {
    ...
    if (id11.substring(0, id11.indexOf("(")).equalsIgnoreCase(id22.substring(0, id22.indexOf("(")))) {
      try {
        ...
        if (totalAmount > 4) {
          ...
          c.getItemAssistant().deleteItem(id2, c.getItemAssistant().getItemSlot(id2), 1);
          c.getItemAssistant().addItem(c.getItemAssistant().getItemId(item1), 1);
          c.getItemAssistant().addItem(c.getItemAssistant().getItemId(item2), 1);
        } else {
          amount1 = totalAmount;
          String item1 = id11.substring(0, id11.indexOf("(") + 1) + amount1 + ")";
          c.getItemAssistant().deleteItem(id, c.getItemAssistant().getItemSlot(id), 1);
  ...
  private final int[][] potMixing = { { 117, 117, 113, 227 }, { 119, 115, 113, 227 }, { 115, 119, 113, 227 },
  ...
  public void mixPotion(int id, int id2) {
    for (int[] element : potMixing) {
      if (element[0] == id && element[1] == id || element[1] == id && element[0] == id2) {
        c.getItemAssistant().deleteItem(id, c.getItemAssistant().getItemSlot(id), 1);
        c.getItemAssistant().deleteItem(id2, c.getItemAssistant().getItemSlot(id2), 1);
