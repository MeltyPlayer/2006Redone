...
public class Npc {
  ...
  public void requestTransformTime(Client player, int itemId, int animation, final int currentId, final int newId,
      int transformTime) {
    if (!player.getItemAssistant().playerHasItem(itemId)) {
      player.getActionSender()
          .sendMessage("You need " + ItemAssistant.getItemName(itemId).toLowerCase() + " to do that.");
      return;
    }
  ...
  public int getNextWalkingDirection() {
    ...
    for (Npc npc : NpcHandler.npcs) {
      ...
      if (npc.absX == nextX && npc.absY == nextY && npc.heightLevel == heightLevel) {
        return -1;
      }
    }
    for (Player p : PlayerHandler.players) {
      ...
      if (p.absX == nextX && p.absY == nextY && p.heightLevel == heightLevel) {
        return -1;
      }
    }
    if (dir == -1) {
