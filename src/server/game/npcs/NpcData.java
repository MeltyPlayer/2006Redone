...
public class NpcData {
   ...
   * return true; } if (NPCHandler.npcs[i].inWild() && NPCHandler.npcs[i].MaxHP
   * > 0 && !onlyMage(NPCHandler.npcs[i].npcType)) { return true; } return
   * false; }
   */
  ...
  public static int getCloseRandomPlayer(int i) {
    ...
    for (int j = 0; j < PlayerHandler.players.length; j++) {
      if (PlayerHandler.players[j] != null) {
        if (NpcHandler.goodDistance(PlayerHandler.players[j].absX, PlayerHandler.players[j].absY,
            NpcHandler.npcs[i].absX, NpcHandler.npcs[i].absY,
            2 + NpcHandler.distanceRequired(i) + NpcHandler.followDistance(i)) || FightCaves.isFightCaveNpc(i)) {
