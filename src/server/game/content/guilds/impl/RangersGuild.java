package server.game.content.guilds.impl;
...
public class RangersGuild {
	...
	public final int ARROWS_REQ = 882, HIT_CHANCE = 55, RANGED_LV = 4;

	public final String MISSED = "Missed!", BLACK = "Hit Black!",
			YELLOW = "Hit Yellow!", BLUE = "Hit Blue!", RED = "Hit Red!",
			BULLSEYE = "Bulls-Eye";

	public int arrowsLeft = 0, playerScore = 0, hitChance;
	...
	public void fireAtTarget() {
		...
		hitChance = Misc.random(HIT_CHANCE)
				+ Misc.random(c.playerLevel[RANGED_LV]);
		if (arrowsLeft != 0) {
			for (int bowId : RangeData.BOWS) {
				if (c.playerEquipment[c.playerWeapon] == bowId) {
					c.usingBow = true;
					if (c.playerEquipment[c.playerArrows] == ARROWS_REQ
							&& c.usingBow) {
						if (isInTargetArea()) {
							if (hitChance >= 10) {
						...
						c.getDialogueHandler().sendStatement(
								"You need some bronze arrows and a bow to shoot the target.");
						c.nextChat = 0;
					}
				}
			}
			...
			c.getDialogueHandler()
					.sendStatement("You should talk to the competition judge.");
			c.nextChat = 0;
		}
	}
	...
	public boolean isInTargetArea() {
		if (c.absX >= 2669 && c.absX <= 2674 && c.absY >= 3415
				&& c.absY <= 3421) {
			return true;
		}
		return false;
	...
	public void exchangePoints() {
		if (arrowsLeft == 0 && playerScore > 0) {
			if (c.getItemAssistant().freeSlots() > 0) {
				...
				c.getDialogueHandler().sendNpcChat2(
						"Well done. Your score is : " + playerScore + ".",
						"You have earned " + ticketsAmt + " Archery tickets.",
						c.talkingNpc, "Tutor");
				playerScore = 0;
				c.nextChat = 0;
			} else {
				c.getDialogueHandler().sendStatement(
						"You need 1 free slot to exchange tickets.");
				c.nextChat = 0;
			}
		} else {
			c.getDialogueHandler().sendStatement(
					"You still have " + arrowsLeft + " shots left.");
			c.nextChat = 0;
		}
	}
	...
	public void howAmIDoing() {
		if (playerScore == 0) {
			c.getDialogueHandler().sendNpcChat2(
					"You haven't started yet. Stand behind the hay bales and",
					"shoot those arrows at the targets.", c.talkingNpc,
					"Tutor");
			c.nextChat = 0;
		} else {
			c.getDialogueHandler().sendNpcChat2(
					"Your score is : " + playerScore, "Your doing very well!",
					c.talkingNpc, "Tutor");
			c.nextChat = 0;
		}
	}
		...
		if (arrowsLeft == 0) {
			if (c.getItemAssistant().playerHasItem(995, 200)) {
				if (c.getItemAssistant().freeSlots() > 1) {
					...
					c.getDialogueHandler().sendStatement(
							"The archer hands you 10 bronze arrows and a bow.");
					c.nextChat = 0;
				} else {
					c.getDialogueHandler().sendStatement(
							"You need 2 free slots to receive arrows and a bow.");
				}
			} else {
				c.getDialogueHandler().sendStatement(
						"You need at least 200 gp to buy 10 arrows and a bow.");
				c.nextChat = 0;
			}
		} else {
			c.getDialogueHandler().sendStatement(
