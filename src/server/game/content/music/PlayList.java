...
public class PlayList {
	...
	@SuppressWarnings("unused")
	private final int[] buttons = { 41, 155, 99, 167, 296, 105, 81, 123, 2, 170,
			83, 59, 73, 63, 29, 24, 53, 71, 251, 28, 25, 66, 33, 171, 130, 40,
			187, 113, 112, 135, 183, 86, 248, 168, 84, 61, 435, 436, 437, 434,
			116, 253, 263, 47, 159, 108, 117, 199, 49 };
	...
	private final String[] names = { "Autumn Voyage", "Unknown Land",
			...
			"Nightfall", "Newbie Melody", "Spirit", "Scape Soft", "Inspiration",
			"Wildwood", "Wonder", "Iban", "Egypt", "Dogs of War",
			"Food for Thought", "Malady", "Dance of Death", "Parade",
			"Down To Earth", "Faithless", "Book Of Spells", "Vision",
	...
	private static enum Songs {
		...
		AUTUMN_VOYAGE(16208, 2, 4304, 0), UNKNOWN_LAND(17066, 3, 4418,
				1), LONG_WAY_HOME(17010, 12, 4362, 2), WITCHING(17078, 14, 4430,
						3), KINGDOM(32243, 9, 8435, 4), MEDIEVAL(17016, 157,
								4368,
								5), HARMONY(16248, 76, 4344, 6), RUNE_ESSENCE(
										17034, 57, 4386,
										7), PHEASANT_PEASANT(55108, 419, 14188,
												8), WORKSHOP(17081, 15, 4433,
														9), HORIZON(16250, 18,
																4346,
																10), DREAM(
																		35014,
		...
		SEA_SHANTY_2(17041, 35, 4393, 24), ATTENTION(16207, 180, 4303,
				25), TOMORROW(24153, 105, 6297, 26), NIGHTFALL(17024, 127, 4376,
						27), NEWBIE_MELODY(17023, 62, 4375, 28), SPIRIT(17046,
								175, 4398, 29), SCAPE_SOFT(23100, 54, 5988,
										30), INSPIRATION(16253, 96, 4349,
												31), WILDWOOD(27071, 278, 6983,
														32), WONDER(17079, 34,
																4431,
																33), IBAN(16251,
																		1, 4347,
		...
		try {
			if (Songs.get(songID) == null) {
				...
				if (auto) {
					player.getPlayerAssistant()
							.sendFrame126("Song Unavailable!", 4439);
					updateList(4439);
				}
				return;
			}
			if (!unlocked[Songs.get(songID).arraySlot]) {
				player.getActionSender()
						.sendMessage("You have unlocked the song "
				...
				player.getPlayerAssistant()
						.sendFrame126(names[Songs.get(songID).arraySlot], 4439);
				updateList(4439);
			} else {
				if (auto) {
	...
	public void handleButton(int button) {
		...
		if (button == 9925) {
			...
			if (songId == -1) {
				...
				player.getActionSender()
						.sendMessage("You have to unlock that song first!");
			} else {
				auto = false;
				player.getPlayerAssistant().sendConfig(18, 0);
				playSong(songId);
