...
public class Music {
  ...
  public static int[][] MUSIC_DATA = { // all the songs added
      { 50, 51, 2 }, { 49, 51, 2 }, // Flour mill place
      { 48, 50, 3 }, { 46, 56, 8 },
      // {51, 60, 9},
      { 46, 50, 12 }, { 31, 74, 16 }, // mime event
      // {46, 50, 49},
      { 52, 57, 14 }, { 47, 52, 15 }, { 45, 53, 18 }, { 45, 54, 18 }, { 47, 50, 35 }, { 50, 150, 139 }, // harmony
                                                                                                        // 2
      { 46, 54, 23 }, // goblin village
      // {47, 50, 36},
      { 51, 50, 36 }, { 47, 51, 49 }, { 51, 49, 50 }, { 44, 75, 52 }, { 46, 53, 54 }, { 49, 54, 111 }, { 51, 60, 255 }, // scape
                                                                                                                        // sad
  ...
  public static void checkMusic(Client c) {
    for (int[] aMUSIC_DATA : MUSIC_DATA) {
      if (c.getX() / 64 == aMUSIC_DATA[0] && c.getY()
          / 64 == aMUSIC_DATA[1]/*
                                 * && player . getPlayList ( ) . auto
                                 */) {
        if (c.getTemporary("CURRENT_SONG") == null || (Integer) c.getTemporary("CURRENT_SONG") != aMUSIC_DATA[2]) {
          c.getActionSender().sendMessage("@gre@Play Music Has Music:@bla@ " + aMUSIC_DATA[0] + " : " + aMUSIC_DATA[1]);
        }
      }
    }
    c.getActionSender().sendMessage("@red@Play Music No Music:@bla@ " + c.getX() / 64 + " : " + c.getY() / 64);
  }
  ...
  public static void playMusic(Client player) {
    for (int[] aMUSIC_DATA : MUSIC_DATA) {
      if (player.getX() / 64 == aMUSIC_DATA[0] && player.getY()
          / 64 == aMUSIC_DATA[1]/*
                                 * && player . getPlayList ( ) . auto
                                 */) {
        if (player.getTemporary("CURRENT_SONG") == null
          ...
          /*
           * int[] battlefield = {148, 17}; int randomSong = Misc.random(1); if
           * (aMUSIC_DATA[0] == 39 && aMUSIC_DATA[1] == 50) { aMUSIC_DATA[2] =
