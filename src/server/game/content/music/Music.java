		...
		for (int[] aMUSIC_DATA : MUSIC_DATA) {
			if (c.getX() / 64 == aMUSIC_DATA[0] && c.getY()
					/ 64 == aMUSIC_DATA[1]/*
											 ...
											 * auto
											 */) {
				if (c.getTemporary("CURRENT_SONG") == null || (Integer) c
						.getTemporary("CURRENT_SONG") != aMUSIC_DATA[2]) {
					c.getActionSender()
							.sendMessage("@gre@Play Music Has Music:@bla@ "
									+ aMUSIC_DATA[0] + " : " + aMUSIC_DATA[1]);
				}
			}
		}
		c.getActionSender().sendMessage("@red@Play Music No Music:@bla@ "
				+ c.getX() / 64 + " : " + c.getY() / 64);
	}
		...
		for (int[] aMUSIC_DATA : MUSIC_DATA) {
			if (player.getX() / 64 == aMUSIC_DATA[0] && player.getY()
				...
				if (player.getTemporary("CURRENT_SONG") == null
						|| (Integer) player.getTemporary(
								"CURRENT_SONG") != aMUSIC_DATA[2]) {
					player.getPlayList();
