...
public class Slayer {
	...
	public enum Task {
		ABERRANT_SPECTRE(1604, 60, 90, 3, "Slayer Tower"), ABYSSAL_DEMON(1615,
				85, 150, 4, "Slayer Tower"), BANSHEE(1612, 15, 22, 0 + r(2),
						"Slayer Tower"), BASILISK(1616, 40, 75, 2,
								"Fremennik Slayer Dungeon"), BAT(412, 1, 8, 1,
										"Road to Paterdomus"), BLACK_DEMON(84,
												1, 157, 3,
												"Taverly Dungeon"), BLACK_DRAGON(
														54, 1, 258, 4,
														"Taverly Dungeon"), BLOODVELD(
																1618, 50, 120,
																3 + r(1),
																"Slayer Tower"), BLUE_DRAGON(
																		55, 1,
																		107, 3,
																		"Taverly Dungeon"), BRONZE_DRAGON(
																				1590,
																				1,
																				125,
																				3,
																				"Brimhaven Dungeon"), CAVE_CRAWLER(
																						1600,
																						10,
																						22,
																						1,
																						"Fremennik Slayer Dungeon"), COCKATRICE(
																								1620,
																								25,
																								37,
																								2,
																								"Fremennik Slayer Dungeon"), CRAWLING_HAND(
																										1648,
																										5,
																										16 + r(3),
																										1 + r(1),
																										"Slayer Tower"), DAGANNOTH_74(
																												1338,
																												1,
																												70 + r(50),
																												3,
																												"Lighthouse Basement"), DAGANNOTH_92(
																														1342,
																														1,
																														70 + r(50),
																														3,
																														"Lighthouse Basement"), DARK_BEAST(
																																2783,
																																90,
																																180,
																																4,
		...
		private Task(int npcId, int levelReq, int exp, int difficulty,
				String location) {
			this.npcId = npcId;
			this.exp = exp;
	...
	public boolean canAttackNpc(int i) {
		if (c.playerLevel[c.playerSlayer] < getRequiredLevel(
				NpcHandler.npcs[i].npcType)) {
			c.getActionSender()
					.sendMessage("You need a slayer level of "
							+ getRequiredLevel(NpcHandler.npcs[i].npcType)
							+ " to attack this npc.");
			...
			if (c.combatLevel < slayermasters.getCombatRequirement()
				...
				c.getActionSender()
						.sendMessage("You need "
								+ slayermasters.getCombatRequirement()
			...
			if (slayerTask.getDifficulty() == taskLevel) {
				if (c.playerLevel[18] >= slayerTask.getLevelReq()) {
					...
					if (!c.needsNewTask) {
						...
						c.slayerTask = task;
						c.taskAmount = getTaskAmount(
								getDifficulty(c.slayerTask) - 1);
						c.needsNewTask = false;
					}
					c.getDialogueHandler().sendDialogues(1237, c.npcType);// assign
																			// task
					c.getActionSender().sendMessage("You have been assigned "
							+ c.taskAmount + " " + getTaskName(c.slayerTask)
	...
	public void cancelTask() {
		if (!hasTask()) {
			c.getActionSender()
					.sendMessage("You must have a task to cancel first.");
			return;
		}
		...
		c.getActionSender()
				.sendMessage("You have cancelled your current task of "
						+ c.taskAmount + " " + getTaskName(c.slayerTask) + ".");
		c.slayerTask = -1;
		c.taskAmount = 0;
		c.slayerPoints -= 30;
	...
	public void removeTask() {
		...
		if (!hasTask()) {
			c.getActionSender()
					.sendMessage("You must have a task to remove first.");
			return;
		}
	...
	public void updateCurrentlyRemoved() {
		...
		for (int i = 0; i < c.removedTasks.length; i++) {
			if (c.removedTasks[i] != -1) {
				c.getPlayerAssistant()
						.sendFrame126(getTaskName(c.removedTasks[i]), line[i]);
			} else {
