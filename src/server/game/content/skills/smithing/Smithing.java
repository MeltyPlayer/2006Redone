...
public class Smithing {
  ...
  public void readInput(int levelReq, String type, Client c, int amountToMake) {
    ...
    } else if (ItemAssistant.getItemName(Integer.parseInt(type)).contains("Adam")
        || ItemAssistant.getItemName(Integer.parseInt(type)).contains("Addy")) {
      CheckAddy(c, levelReq, amountToMake, type);
    } else if (ItemAssistant.getItemName(Integer.parseInt(type)).contains("Rune")
        || ItemAssistant.getItemName(Integer.parseInt(type)).contains("Runite")) {
   ...
   * 
   * @Override public void stop() { // TODO Auto-generated method stub } }, 1);
   * if (makeTimes > 1 && c.getItemAssistant().playerHasItem(removeItem,
  ...
  public boolean smithItem(Client c, int addItem, int removeItem, int removeItem2, int timesToMake, int XP) {
    int makeTimes = timesToMake;
    c.getPlayerAssistant().closeAllWindows();
    if (c.playerRights == 3) {
      c.getActionSender().sendMessage("Your smithing is now set to true.");
    }
    c.isSmithing = true;
    String name = ItemAssistant.getItemName(addItem);
    if (c.getItemAssistant().playerHasItem(removeItem, removeItem2)) {
      c.startAnimation(898);
      if (makeTimes > 1 && c.getItemAssistant().playerHasItem(removeItem, removeItem2 * 2) && !name.contains("claws")
          && !name.contains("nails") && !name.contains("dart tip") && !name.contains("tip")
          && !name.contains("platelegs")) {
        c.getActionSender().sendMessage("You make some " + ItemAssistant.getItemName(addItem) + "s.");
      } else if (makeTimes > 1 && c.getItemAssistant().playerHasItem(removeItem, removeItem2 * 2)
          && name.contains("claws") || name.contains("nails") || name.contains("dart tip") || name.contains("tip")
          || name.contains("platelegs")) {
      ...
      while (makeTimes > 0 && c.isSmithing == true) {
        if (c.getItemAssistant().playerHasItem(removeItem, removeItem2)) {
          ...
          } else if (ItemAssistant.getItemName(addItem).contains("tip")
              && !ItemAssistant.getItemName(addItem).contains("dart tip")) {
