      ...
      if (bartype == data[i][0]) {
        if (c.playerLevel[c.playerSmithing] < data[i][1]) { // Smithing
          ...
          c.getDialogueHandler()
              .sendStatement("You need a smithing level of at least " + data[i][1] + " in order smelt this bar.");
          return;
        }
        ...
        if (data[i][3] > 0 && data[i][4] > 0) { // All OTHER bars
          if (!c.getItemAssistant().playerHasItem(data[i][3]) || !c.getItemAssistant().playerHasItem(data[i][4])) {
            c.getActionSender()
                .sendMessage("You need an " + ItemAssistant.getItemName(data[i][3]).toLowerCase() + " and " + data[i][5]
                    + " " + ItemAssistant.getItemName(data[i][4]).toLowerCase() + " to make this bar.");
        ...
        if (data[i][4] < 0) { // Iron bar, Gold & Silver requirements
          if (!c.getItemAssistant().playerHasItem(data[i][3])) {
            c.getActionSender().sendMessage(
                "You need an " + ItemAssistant.getItemName(data[i][3]).toLowerCase() + " to make this bar.");
            c.getPlayerAssistant().removeAllWindows();
            return;
          }
        }
        ...
        if (data[i][5] > 0) { // Bars with more than 1 coal requirement
          if (!c.getItemAssistant().playerHasItem(data[i][4], data[i][5])) {
            c.getActionSender().sendMessage("You need an " + ItemAssistant.getItemName(data[i][3]).toLowerCase()
                + " and " + data[i][5] + " coal ores to make this bar.");
            c.getPlayerAssistant().removeAllWindows();
            return;
          }
        }
        ...
        CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
          ...
          public void execute(CycleEventContainer container) {
            ...
            c.getActionSender().sendMessage("You receive an " // Message
                + ItemAssistant.getItemName(c.playerSkillProp[13][6]).toLowerCase() + ".");
            if (c.playerSkillProp[13][3] == GOLD && c.playerSkillProp[13][4] == -1
