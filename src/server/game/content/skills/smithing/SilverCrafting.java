...
public class SilverCrafting {
	...
	public static enum SilverCraft {
		UNBLESSED(34205, 2355, 1716, 1, 16, 50), UNBLESSED5(34204, 2355, 1716,
				5, 16,
				50), UNBLESSED10(34203, 2355, 1716, 10, 16, 50), UNBLESSEDX(
						34202, 2355, 1716, 0, 16,
						50), UNHOLY(34209, 2355, 1724, 1, 17, 50), UNHOLY5(
								34208, 2355, 1724, 5, 17, 50), UNHOLY10(34207,
										2355, 1724, 10, 17, 50), UNHOLYX(34206,
												2355, 1724, 0, 17, 50), SICKLE(
														34213, 2355, 2961, 1,
														18, 50), SICKLE5(34212,
																2355, 2961, 5,
																18,
	...
	public static void makeSilver(final Client player, int buttonId,
		...
		if (silverCraft == null
				|| silverCraft.getAmount() == 0 && amount == 0) {
			return;
		}
		if (silverCraft.getUsed() == SILVER_BAR && player.isCrafting) {
			...
			if (!player.getItemAssistant().playerHasItem(SILVER_BAR)) {
				player.getDialogueHandler()
						.sendStatement("You need a silver bar to do this.");
				return;
			}
			if (player.playerLevel[player.playerCrafting] < silverCraft
					.getLevel()) {
				player.getDialogueHandler()
						.sendStatement("You need a crafting level of "
			...
			CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
				...
				int amnt = silverCraft.getAmount() != 0
						? silverCraft.getAmount()
						: amount;
