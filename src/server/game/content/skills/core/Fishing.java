...
public class Fishing extends SkillHandler {
      ...
      if (npcId == data[i][0]) {
        if (c.playerLevel[c.playerFishing] < data[i][1]) {
          c.getDialogueHandler()
              .sendStatement("You need a fishing level of at least " + data[i][1] + " in order to fish at this spot.");
          return;
        }
        ...
        if (data[i][3] > 0) {
          if (!c.getItemAssistant().playerHasItem(data[i][3])) {
            c.getDialogueHandler()
                .sendStatement("You need more " + ItemAssistant.getItemName(data[i][3]).toLowerCase().toLowerCase()
        ...
        if (c.tutorialProgress == 6) { // if tutorial prog = 6
          ...
          c.getDialogueHandler().chatboxText(c, "This should only take a few seconds.",
              "As you gain Fishing experience you'll find that there are many",
              "types of fish and many ways to catch them.", "", "Please wait");
          ...
          CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
            ...
            public void execute(CycleEventContainer container) {
              ...
              if (c.playerSkillProp[10][2] > 0) {
                c.getPlayerAssistant().addSkillXP(c.playerSkillProp[10][2], c.playerFishing);
              }
              if (c.playerSkillProp[10][1] > 0) {
        ...
        CycleEventHandler.getSingleton().addEvent(eventId, c, new CycleEvent() {
          ...
          public void execute(CycleEventContainer container) {
            if (c.playerSkillProp[10][3] > 0) {
              ...
              if (!c.getItemAssistant().playerHasItem(c.playerSkillProp[10][3])) {
                c.getActionSender().sendMessage(
                    "You don't have any " + ItemAssistant.getItemName(c.playerSkillProp[10][3]) + " left!");
                c.getActionSender()
                    .sendMessage("You need " + ItemAssistant.getItemName(c.playerSkillProp[10][3]) + " to fish here.");
                resetFishing(c);
                container.stop();
              }
            }
        ...
        CycleEventHandler.getSingleton().addEvent(eventId, c, new CycleEvent() {
          ...
          public void execute(CycleEventContainer container) {
            ...
            if (c.playerSkillProp[10][1] > 0) {
              c.getActionSender()
                  .sendMessage("You catch "
                      + (c.playerSkillProp[10][1] == 321 || c.playerSkillProp[10][1] == 317
                          || c.playerSkillProp[10][1] == 7944 ? "some " : "a ")
                      + ItemAssistant.getItemName(c.playerSkillProp[10][1]).toLowerCase().replace("raw ", "") + ".");
            }
            if (c.playerSkillProp[10][1] > 0) {
              randomEvents(c);
            }
            ...
            if (c.playerSkillProp[10][1] > 0) {
              c.getItemAssistant().deleteItem(c.playerSkillProp[10][3],
                  c.getItemAssistant().getItemSlot(c.playerSkillProp[10][3]), 1);
              c.getItemAssistant().addItem(c.playerSkillProp[10][1], 1);
              c.startAnimation(c.playerSkillProp[10][0]);
            }
            if (c.playerSkillProp[10][5] > 0 && c.playerLevel[c.playerFishing] >= c.playerSkillProp[10][6]) {
              c.getPlayerAssistant().addSkillXP(c.playerSkillProp[10][8], c.playerFishing);
            } else if (c.playerSkillProp[10][7] > 0) {
              c.getPlayerAssistant().addSkillXP(c.playerSkillProp[10][2], c.playerFishing);
            }
            if (c.playerSkillProp[10][3] > 0) {
              if (!c.getItemAssistant().playerHasItem(c.playerSkillProp[10][3])) {
                c.getDialogueHandler().sendStatement("You have run out of "
                    + ItemAssistant.getItemName(c.playerSkillProp[10][3]).toLowerCase().toLowerCase() + ".");
                // c.getPacketDispatcher().sendMessage("You
                // don't have any "+
  ...
  public static boolean hasFishingEquipment(Client c, int equipment) {
    if (!c.getItemAssistant().playerHasItem(equipment)) {
      if (equipment == 311) {
        if (!c.getItemAssistant().playerHasItem(311) && !c.getItemAssistant().playerHasItem(10129)
            && c.playerEquipment[3] != 10129) {
          c.getActionSender()
              .sendMessage("You need a " + ItemAssistant.getItemName(equipment).toLowerCase() + " to fish here.");
          resetFishing(c);
          return false;
        }
      } else {
        resetFishing(c);
        c.getActionSender().sendMessage("You need a " + ItemAssistant.getItemName(equipment) + " to fish here.");
        return false;
      }
    }
    return true;
  ...
  public static String messages(Client c) {
    ...
    if (c.playerSkillProp[10][10] == 2 || c.playerSkillProp[10][10] == 3 || c.playerSkillProp[10][10] == 4
        || c.playerSkillProp[10][10] == 5 || c.playerSkillProp[10][10] == 6) {
      return messages[1][0];
    }
