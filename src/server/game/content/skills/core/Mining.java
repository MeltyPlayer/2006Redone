...
public class Mining {
  ...
  public final int[][] Pick_Settings = { { 1265, 1, 1, 625 }, // Bronze
      { 1267, 1, 2, 626 }, // Iron
      { 1269, 6, 3, 627 }, // Steel
      { 1271, 31, 5, 629 }, // Addy
  ...
  public final int[][] Rock_Settings = { { 2091, 1, 18, 3, 436 }, // Copper
      { 2095, 1, 18, 3, 438 }, // Tin
  ...
  public void startMining(final Client player, final int objectNumber, final int objectX, final int objectY,
      final int type) {
    if (player.isMining)
      return;
    ...
    if (Rock_Settings[objectNumber][1] > miningLevel) {
      player.getActionSender()
          .sendMessage("You need a Mining level of " + Rock_Settings[objectNumber][1] + " to mine this rock.");
      return;
    }
    for (int i = 0; i < Pick_Settings.length; i++) {
      if (player.getItemAssistant().playerHasItem(Pick_Settings[i][0])
          || player.playerEquipment[player.playerWeapon] == Pick_Settings[i][0]) {
    ...
    if (player.tutorialProgress == 17 || player.tutorialProgress == 18) {
      ...
      player.getDialogueHandler().chatboxText(player, "", "Your character is now attempting to mine the rock.",
          "This should only take a few seconds.", "", "Please wait");
    ...
    CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
      ...
      public void execute(CycleEventContainer container) {
        ...
        if (player.isMining) {
          ...
          player.getActionSender().sendMessage("You manage to mine some "
              + ItemAssistant.getItemName(Rock_Settings[objectNumber][4]).toLowerCase() + ".");
        }
        if (player.tutorialProgress == 17) {
          ...
          if (player.getItemAssistant().playerHasItem(438)) {
            ...
            player.getDialogueHandler().chatboxText(player,
                ...
                "riger click on the copper rock and select 'mine'.", "", "Mining");
        ...
        mineRock(Rock_Settings[objectNumber][3] + getRandomAdd(), objectX, objectY, type,
            Rock_Settings[objectNumber][0]);
  ...
  public int getTimer(int b, int c, int level) {
    double timer = (int) ((Rock_Settings[b][1] * 2) + 20 + Misc.random(20))
        - ((Pick_Settings[c][2] * (Pick_Settings[c][2] * 0.75)) + level);
    if (timer < 2.0) {
      return 2;
    } else {
      return (int) timer;
  ...
  public static void prospectRock(final Client c, final String itemName) {
    if (c.tutorialProgress == 15 || c.tutorialProgress == 16) {
      ...
      c.getDialogueHandler().chatboxText(c, "Please wait.",
      ...
      CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
        ...
        public void execute(CycleEventContainer container) {
          if (c.tutorialProgress == 15) {
            c.getActionSender().sendMessage("This rock contains " + itemName.toLowerCase() + ".");
            c.getActionSender().chatbox(6180);
            c.getDialogueHandler().chatboxText(c, "", "So now you know there's tin in the grey rocks. Try prospecting",
                "the brown ones next.", "", "It's tin");
            c.getActionSender().createArrow(3086, 9501, c.getH(), 2);
            c.getActionSender().chatbox(6179);
            ...
            c.getActionSender().sendMessage("This rock contains " + itemName.toLowerCase() + ".");
            c.getActionSender().chatbox(6180);
            c.getDialogueHandler().chatboxText(c, "Talk to the Mining Instructor to find out about these types of",
                "ore and how you can mine them. He'll even give you the", "required tools.", "", "It's copper");
  ...
  public static enum rockData {
    ESSENCE(new int[] { 2491 }, 1, 5, 3, 1, new int[] { 1436 }), CLAY(
        new int[] { 2108, 2109, 11189, 11190, 11191, 9713, 9711, 14905, 14904 }, 1, 5, 1, 5, new int[] { 434 }), COPPER(
            new int[] { 3042, 2091, 2090, 9708, 9709, 9710, 11960, 14906, 14907 }, 1, 18, 1, 8, new int[] { 436 }), TIN(
                new int[] { 2094, 2095, 3043, 9716, 9714, 11958, 11957, 11959, 11933, 11934, 11935, 14903, 14902 }, 1,
                18, 1, 8,
                new int[] { 438 }), BLURITE(new int[] { 10574, 10583, 2110 }, 10, 20, 1, 8, new int[] { 668 }), IRON(
                    new int[] { 2093, 2092, 9717, 9718, 9719, 11962, 11956, 11954, 14856, 14857, 14858, 14914, 14913 },
                    15, 35, 2, 5, new int[] { 440 }), SILVER(new int[] { 2101, 11186, 11187, 11188, 2100 }, 20, 40, 3,
                        20, new int[] { 442 }), COAL(
                            new int[] { 2096, 2097, 11963, 11964, 14850, 14851, 14852, 11930, 11931 }, 30, 50, 4, 25,
                            new int[] { 453 }), GOLD(new int[] { 2099, 2098, 11183, 11184, 11185, 9720, 9722 }, 40, 65,
                                6, 33, new int[] { 444 }), MITHRIL(new int[] { 2103, 2102, 14853, 14854, 14855 }, 55,
                                    80, 8, 50, new int[] { 447 }), ADAMANT(
                                        new int[] { 2104, 2105, 14862, 14863, 14864 }, 70, 95, 10, 83,
                                        new int[] { 449 }), RUNE(new int[] { 14859, 14860, 2106, 2107 }, 85, 125, 20,
                                            166, new int[] { 451 }), GRANITE(new int[] { 10947 }, 45, 75, 10, 10,
                                                new int[] { 6979, 6981, 6983 }), SANDSTONE(new int[] { 10946 }, 35, 60,
                                                    5, 5, new int[] { 6971, 6973, 6975, 6977 }), GEM(new int[] { 2111 },
                                                        40, 65, 6, 120, new int[] { 1 });

    private final int levelReq, mineTimer, respawnTimer, xp;
    private final int[] oreIds;
    private final int[] objectId;

    private rockData(final int[] objectId, final int levelReq, final int xp, final int mineTimer,
        final int respawnTimer, final int... oreIds) {
