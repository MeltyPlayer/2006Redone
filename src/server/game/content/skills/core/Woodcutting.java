...
public class Woodcutting {
  ...
  public final static int[][] Axe_Settings = { { 1351, 1, 1, 879 }, { 1349, 1, 2, 877 }, { 1353, 6, 3, 875 },
      { 1361, 6, 4, 873 }, { 1355, 21, 5, 871 }, { 1357, 31, 6, 869 }, { 1359, 41, 7, 867 }, { 6739, 61, 8, 2846 },
      { 13661, 41, 8, 10251 } };

  public final static int[][] Tree_Settings = { { 1276, 1342, 1, 25, 1511, 45, 100 },
      { 1278, 1342, 1, 25, 1511, 45, 100 }, { 1286, 1342, 1, 25, 1511, 45, 100 }, { 1281, 1356, 15, 38, 1521, 11, 20 },
      { 1308, 7399, 30, 68, 1519, 11, 8 }, { 5552, 7399, 30, 68, 1519, 11, 8 }, { 1307, 1343, 45, 100, 1517, 48, 8 },
      { 1309, 7402, 60, 175, 1515, 79, 5 }, { 1306, 7401, 75, 250, 1513, 150, 3 }, { 5551, 7399, 30, 68, 1519, 11, 8 },
      { 5553, 7399, 30, 68, 1519, 11, 8 }, { 3033, 1342, 1, 25, 1511, 45, 100 }, { 3037, 1356, 15, 38, 1521, 11, 20 },
      { 1282, 1342, 1, 25, 1511, 45, 100 }, { 1383, 1342, 1, 25, 1511, 45, 100 }, { 2023, 3371, 1, 25, 2862, 45, 100 },
      { 1319, 1341, 1, 25, 1511, 45, 100 }, { 1318, 1341, 1, 25, 1511, 45, 100 }, { 1315, 1341, 1, 25, 1511, 45, 100 },
      { 1316, 1341, 1, 25, 1511, 45, 100 }, { 1332, 1341, 1, 25, 1511, 45, 100 }, { 1292, 1341, 36, 1, 771, 45, 100 } };

  public static int[][] FIX_AXE = { { 492, 508, 1351 }, { 492, 510, 1349 }, { 492, 512, 1353 }, { 492, 514, 1361 },
      { 492, 516, 1355 }, { 492, 518, 1357 }, { 492, 520, 1359 }, };
  ...
  public static void handleCanoe(final Client player, final int objectId) {
    ...
    for (int axes[] : Axe_Settings) {
      ...
      int level = axes[1];
      int anim = axes[3];
      if (player.playerLevel[player.playerWoodcutting] >= level && player.getItemAssistant().playerHasItem(type)
  ...
  public static void startWoodcutting(final Client player, final int j, final int x, final int y, final int type) {
    ...
    if (Tree_Settings[j][2] > wcLevel) {
      player.getActionSender()
          .sendMessage("You need a Woodcutting level of " + Tree_Settings[j][2] + " to cut this tree.");
      return;
    }
    for (int i = 0; i < Axe_Settings.length; i++) {
      if (player.getItemAssistant().playerHasItem(Axe_Settings[i][0])
      ...
      if (player.tutorialProgress == 3) {
        ...
        if (player.playerAppearance[0] == 0) {
          player.getDialogueHandler().chatboxText(player, "",
              ...
              "for a moment while he does all the hard work.", "", "Please wait");
        } else {
          player.getDialogueHandler().chatboxText(player, "",
              "Your character is now attempting to cut down the tree. Sit back",
              "for a moment while she does all the hard work.", "", "Please wait");
        }
      ...
      CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
        ...
        public void execute(CycleEventContainer container) {
          ...
          if (player.isWoodcutting) {
            ...
            player.getActionSender().sendMessage("You manage to get some "
                + ItemAssistant.getItemName(Tree_Settings[j][4]).toLowerCase() + " from the tree.");
          }
          if (player.tutorialProgress == 3) {
  ...
  public static int[][] trees = { { // NORMAL
      1276, 1277, 1278, 1279, 1280, 1282, 1283, 1284, 1285, 1286, 1287, 1288, 1289, 1290, 1291, 1301, 1303, 1304, 1305,
      1318, 1319, 1315, 1316, 1330, 1331, 1332, 1333, 1383, 1384, 2409, 2447, 2448, 3033, 3034, 3035, 3036, 3879, 3881,
      3883, 3893, 3885, 3886, 3887, 3888, 3892, 3889, 3890, 3891, 3928, 3967, 3968, 4048, 4049, 4050, 4051, 4052, 4053,
      4054, 4060, 5004, 5005, 5045, 5902, 5903, 5904, 8973, 8974, 10041, 10081, 10082, 10664, 11112, 11510, 12559,
      12560, 12732, 12895, 12896, 13412, 13411, 13419, 13843, 13844, 13845, 13847, 13848, 13849, 13850, 14308, 14309,
      14513, 14514, 14515, 14521, 14564, 14565, 14566, 14593, 14594, 14595, 14600, 14635, 14636, 14637, 14642, 14664,
      14665, 14666, 14693, 14694, 14695, 14696, 14701, 14738, 14796, 14797, 14798, 14799, 14800, 14801, 14802, 14803,
      14804, 14805, 14806, 14807, 15489, 15776, 15777, 16264, 16265, 19165, 19166, 19167, 23381,

      }, { // OAK
          1281, 3037, 8462, 8463, 8464, 8465, 8466, 8467, 10083, 13413, 13420, },
      { // WILLOW
          1308, 5551, 5552, 5553, 8481, 8482, 8483, 8484, 8485, 8486, 8487, 8488, 8496, 8497, 8498, 8499, 8500, 8501,
          13414, 13421, },
      { // MAPLE
          1307, 4674, 8435, 8436, 8437, 8438, 8439, 8440, 8441, 8442, 8443, 8444, 8454, 8455, 8456, 8457, 8458, 8459,
          8460, 8461, 13415, 13423, },
      { // YEW
          1309, 8503, 8504, 8505, 8506, 8507, 8508, 8509, 8510, 8511, 8512, 8513, 13416, 13422, },
      { // MAGIC
