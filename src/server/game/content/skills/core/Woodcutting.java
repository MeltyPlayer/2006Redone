...
import server.game.content.randomevents.TreeSpirit;
...
public class Woodcutting {
	...
	public final static int[][] Axe_Settings = { { 1351, 1, 1, 879 },
			{ 1349, 1, 2, 877 }, { 1353, 6, 3, 875 }, { 1361, 6, 4, 873 },
			{ 1355, 21, 5, 871 }, { 1357, 31, 6, 869 }, { 1359, 41, 7, 867 },
			{ 6739, 61, 8, 2846 }, { 13661, 41, 8, 10251 } };
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				if (player.isWoodcutting) {
					...
					player.getActionSender().sendSound(SoundList.TREE_CUTTING,
							100, 0);
				} else {
					container.stop();
				}
	...
	public static void handleCanoe(final Client player, final int objectId) {
		if (player.playerLevel[player.playerWoodcutting] < 12) {
			player.getActionSender().sendMessage(
					"You need a woodcutting level of at least 12 to use the canoe station.");
			return;
		}
		for (int axes[] : Axe_Settings) {
			...
			int level = axes[1];
			int anim = axes[3];
			if (player.playerLevel[player.playerWoodcutting] >= level
					&& player.getItemAssistant().playerHasItem(type)
					|| player.playerLevel[player.playerWoodcutting] >= level
							&& player.playerEquipment[player.playerWeapon] == type) {
				player.turnPlayerTo(player.objectX, player.objectY);
				player.startAnimation(anim);
				player.getActionSender()
						.sendMessage("You swing your axe at the station.");
						...
						new CycleEvent() {
							...
							public void execute(CycleEventContainer container) {
								...
								CycleEventHandler.getSingleton()
										.addEvent(player, new CycleEvent() {
											@Override
											public void execute(
								...
								player.getActionSender().sendMessage(
										"You cut down the canoe. Please wait...");
								container.stop();
							}
	...
	public void fixAxe(final Client player) {
		for (int fix[] : FIX_AXE) {
			...
			if (player.getItemAssistant().playerHasItem(axeHandle)
					&& player.getItemAssistant().playerHasItem(axeHead)) {
				...
				player.getActionSender().sendMessage(
						"Your axe handle and axe head have been taken.");
				CycleEventHandler.getSingleton().addEvent(player,
						new CycleEvent() {
							@Override
							public void execute(CycleEventContainer container) {
	...
	public static void addFallenTree(Client player, int canoe) {
		if (canoe == player.objectId) {
			for (Player players : PlayerHandler.players) {
				if (players != null) {
					new Object(1296, player.objectX, player.objectY, 0, 0, 10,
	...
	public boolean hasAxe(Client player) {
		for (int i = 0; i < Axe_Settings.length; i++) {
			if (player.getItemAssistant().playerHasItem(Axe_Settings[i][0])
					|| player.playerEquipment[player.playerWeapon] == Axe_Settings[i][0]) {
				return true;
			}
		}
		return false;
	...
	public static void startWoodcutting(final Client player, final int j,
			final int x, final int y, final int type) {
		if (player.isWoodcutting || player.isFletching || player.isFiremaking
				|| player.playerIsFletching) {
			return;
		}
		if (player.absX == 2717 && player.absY == 3461) {
			player.getActionSender()
					.sendMessage("You can't cut the tree from here!");
			return;
		}
		...
		if (Tree_Settings[j][2] > wcLevel) {
			player.getActionSender()
					.sendMessage("You need a Woodcutting level of "
		...
		if (player.getItemAssistant().freeSlots() < 1) {
			player.getActionSender().sendMessage(
					"You do not have enough inventory slots to do that.");
			return;
		}
		if (Misc.goodDistance(player.objectX, player.objectY, player.absX,
				player.absY, 3)) {
			...
			player.startAnimation(Axe_Settings[a][3]);
			player.isWoodcutting = true;
			player.getActionSender().sendSound(SoundList.TREE_CUT_BEGIN, 100,
					0);
			...
			if (player.tutorialProgress == 3) {
				...
				if (player.playerAppearance[0] == 0) {
					...
					player.getDialogueHandler().chatboxText(player, "",
							"Your character is now attempting to cut down the tree. Sit back",
							"for a moment while she does all the hard work.",
							"", "Please wait");
				}
			...
			CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					...
					if (player.getItemAssistant().freeSlots() < 1) {
						player.getActionSender().sendMessage(
								"You have ran out of inventory slots.");
						container.stop();
					}
					int XP = Tree_Settings[j][3];
					if (player.isWoodcutting) {
						player.getItemAssistant().addItem(Tree_Settings[j][4],
								1);
						player.getPlayerAssistant().addSkillXP(XP, 8);
						player.getActionSender()
								.sendMessage("You manage to get some "
										+ ItemAssistant
												.getItemName(
														Tree_Settings[j][4])
												.toLowerCase()
					...
					if (player.isWoodcutting && player.tutorialProgress >= 36
					...
					if (player.isWoodcutting && Misc.random(350) == 69
					...
					if (Misc.random(100) <= Tree_Settings[j][6]) {
						...
						player.getActionSender().sendSound(SoundList.TREE_EMPTY,
								100, 0);
						container.stop();
					}
				}
	...
	public static int getTimer(int b, int c, int level) {
		double timer = (int) ((Tree_Settings[b][2] * 2) + 20 + Misc.random(20))
	...
	public static void birdNests(Client player) {
		if (Misc.random(200) == 69 && player.tutorialProgress >= 36) {
			player.getActionSender()
					.sendMessage("A birds nest falls from the branches.");
			dropNest(player);
		}
	}

	public static void dropNest(Client player) {
		Server.itemHandler.createGroundItem(player, 5070 + Misc.random(4),
	...
	public static int[][] trees = { { // NORMAL
			1276, 1277, 1278, 1279, 1280, 1282, 1283, 1284, 1285, 1286, 1287,
			1288, 1289, 1290, 1291, 1301, 1303, 1304, 1305, 1318, 1319, 1315,
			1316, 1330, 1331, 1332, 1333, 1383, 1384, 2409, 2447, 2448, 3033,
			3034, 3035, 3036, 3879, 3881, 3883, 3893, 3885, 3886, 3887, 3888,
			3892, 3889, 3890, 3891, 3928, 3967, 3968, 4048, 4049, 4050, 4051,
			4052, 4053, 4054, 4060, 5004, 5005, 5045, 5902, 5903, 5904, 8973,
			8974, 10041, 10081, 10082, 10664, 11112, 11510, 12559, 12560, 12732,
			12895, 12896, 13412, 13411, 13419, 13843, 13844, 13845, 13847,
			13848, 13849, 13850, 14308, 14309, 14513, 14514, 14515, 14521,
			14564, 14565, 14566, 14593, 14594, 14595, 14600, 14635, 14636,
			14637, 14642, 14664, 14665, 14666, 14693, 14694, 14695, 14696,
			14701, 14738, 14796, 14797, 14798, 14799, 14800, 14801, 14802,
			14803, 14804, 14805, 14806, 14807, 15489, 15776, 15777, 16264,
			16265, 19165, 19166, 19167, 23381,

			}, { // OAK
					1281, 3037, 8462, 8463, 8464, 8465, 8466, 8467, 10083,
					13413, 13420, },
			{ // WILLOW
					...
					13421, },
			{ // MAPLE
					1307, 4674, 8435, 8436, 8437, 8438, 8439, 8440, 8441, 8442,
					8443, 8444, 8454, 8455, 8456, 8457, 8458, 8459, 8460, 8461,
					13415, 13423, },
			{ // YEW
					1309, 8503, 8504, 8505, 8506, 8507, 8508, 8509, 8510, 8511,
					8512, 8513, 13416, 13422, },
			{ // MAGIC
					1306, 8396, 8397, 8398, 8399, 8400, 8401, 8402, 8403, 8404,
					8405, 8406, 8407, 8408, 8409, 13417, 13424, } };

	public static void cutDownTree(int respawnTime, int x, int y, int type,
			int i, int j) {
