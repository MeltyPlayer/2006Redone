...
public class Potatoes extends SkillHandler {
  ...
  public enum PotatoMaking {
    ...
    CHILLI(7062, 7054, 165, 41), CHEESE(1985, 6705, 199, 47), EGG(7064, 7056, 195, 51), MUSHROOM(7066, 7058, 27,
        64), TUNA(7068, 7060, 309, 68);
     ...
     * @param levelReq
     */
    private PotatoMaking(int ingredient, int newPotatoID, int XP, int levelReq) {
  ...
  public boolean makePotato(int id) {
    ...
    if (c.getItemAssistant().playerHasItem(potato.getIngredient(), 1)) {
      if (c.playerLevel[c.playerCooking] >= potato.getReq()) {
        ...
        c.getPlayerAssistant().addSkillXP(potato.getXP() * COOKING_EXPERIENCE, c.playerCooking);
        RandomEventHandler.addRandom(c);
      } else {
        c.getActionSender().sendMessage("You need a cooking level of " + potato.getReq() + " to make this potato.");
      }
    }
    return false;
  }
