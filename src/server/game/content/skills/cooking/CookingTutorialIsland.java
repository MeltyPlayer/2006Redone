...
public class CookingTutorialIsland extends SkillHandler {
  ...
  private static void cookFish(Client c, int itemID, int xpRecieved, int levelRequired, int burntFish, int cookedFish,
      int object) {
    if (!COOKING) {
      c.getActionSender().sendMessage("Cooking is currently disabled.");
      return;
    }
  ...
  private static void viewCookInterface(Client c, int item) {
    ...
    c.getPlayerAssistant().sendFrame126("" + ItemAssistant.getItemName(item) + "", 13717);
  }
    ...
    if (c.tutorialProgress == 6) {
      ...
      CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
        ...
        public void execute(CycleEventContainer container) {
          c.getItemAssistant().deleteItem(c.playerSkillProp[7][0],
              c.getItemAssistant().getItemSlot(c.playerSkillProp[7][0]), 1);
          if (c.Cookstage1 == 1) {
            c.getActionSender().chatbox(6180);
            c.getDialogueHandler().chatboxText(c, "You have just burned your first shrimp. This is normal. As you",
                ...
                "more shrimp then use them on a fire.", "Burning your shrimp.");
            c.getActionSender().chatbox(6179);
            c.Cookstage1 = 0;
            c.getItemAssistant().addItem(c.playerSkillProp[7][3], 1);
          } else {
            c.getActionSender().chatbox(6180);
            c.getDialogueHandler().chatboxText(c, "If you'd like a recap on anything you've learnt so far, speak to",
            ...
            c.getActionSender().chatbox(6179);
            c.getActionSender().createArrow(3089, 3092, c.getH(), 2);
            c.getPlayerAssistant().addSkillXP(c.playerSkillProp[7][1], 7);
            c.getItemAssistant().addItem(c.playerSkillProp[7][4], 1);
            c.tutorialProgress = 7;
          }
          deleteTime(c);
          if (!c.getItemAssistant().playerHasItem(c.playerSkillProp[7][0], 1) || c.doAmount <= 0) {
            container.stop();
          }
