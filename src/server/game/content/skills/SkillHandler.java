...
public class SkillHandler {
  ...
  public static boolean FISHING = true, AGILITY = true, COOKING = true, FIREMAKING = true, HERBLORE = true,
      MINING = true, RUNECRAFTING = true, THIEVING = true, WOODCUTTING = true, PRAYER = true, FLETCHING = true,
      CRAFTING = true, MAGIC = true, FARMING = false, SLAYER = true, SMITHING = true;

  public static final int WOODCUTTING_EXPERIENCE = SKILLING_EXP, MINING_EXPERIENCE = SKILLING_EXP,
      SMITHING_EXPERIENCE = SKILLING_EXP, FARMING_EXPERIENCE = SKILLING_EXP, FIREMAKING_EXPERIENCE = SKILLING_EXP,
      HERBLORE_EXPERIENCE = SKILLING_EXP, FISHING_EXPERIENCE = SKILLING_EXP, AGILITY_EXPERIENCE = SKILLING_EXP,
      PRAYER_EXPERIENCE = SKILLING_EXP, RUNECRAFTING_EXPERIENCE = SKILLING_EXP, CRAFTING_EXPERIENCE = SKILLING_EXP,
      THIEVING_EXPERIENCE = SKILLING_EXP, SLAYER_EXPERIENCE = SKILLING_EXP, COOKING_EXPERIENCE = SKILLING_EXP,
      FLETCHING_EXPERIENCE = SKILLING_EXP;

  public static boolean isSkilling(Client c) {
    if (c.playerSkilling[10] || c.playerStun || c.playerSkilling[12] || c.playerIsFletching || c.isFletching
        || c.playerIsCooking || c.isMining || c.isWoodcutting || c.isSmithing || c.isSmelting || c.isSpinning
        || c.isPotionMaking || c.isPotCrafting || c.isFiremaking || c.playerSkilling[c.playerHerblore] == true
        || c.playerSkilling[13]) {
      return true;
    }
    return false;
  ...
  public static boolean noInventorySpace(Client c, String skill) {
    if (c.getItemAssistant().freeSlots() == 0) {
      c.getActionSender().sendMessage("You don't have enough inventory space to continue " + skill + "!");
      return false;
    }
    return true;
  }
  ...
  public static void send1Item(Client c, int itemId) {
    ...
    c.getPlayerAssistant().sendFrame126(getLine(c) + "" + ItemAssistant.getItemName(itemId) + "", 2799);
  ...
  public static boolean playerHasItem(Client c, String itemName, String skill, int itemID) {
    if (!c.getItemAssistant().playerHasItem(itemID, 1)) {
      c.getActionSender().sendMessage("You dont have any " + itemName + " to continue " + skill + "!");
      c.getDialogueHandler().sendStatement("You dont have any " + itemID + " to continue " + skill + "!");
      return false;
    }
    return true;
  }
  ...
  public static boolean hasRequiredLevel(final Client c, int id, int lvlReq, String skill, String event) {
    if (c.playerLevel[id] < lvlReq) {
      c.getActionSender().sendMessage("You don't have a high enough " + skill + " level to " + event + ".");
      return false;
    }
    return true;
  }
