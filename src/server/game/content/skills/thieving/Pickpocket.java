...
public class Pickpocket extends SkillHandler {
	...
	MAN(new int[] { 1, 2, 3, 3222 }, 1, 8.0, 1, 5, new int[] { 995, 3 }), WOMEN(
			new int[] { 4, 5, 6 }, 1, 8.0, 1, 5, new int[] { 995, 3 }), FARMER(
					...
					new int[] { 5318, 4 }), HAM_FEMALE(new int[] { 1715 }, 15,
							...
							new int[] { 4302, 1 }, new int[] { 4304, 1 },
							new int[] { 4298, 1 }, new int[] { 4308, 1 },
							new int[] { 4300, 1 }, new int[] { 4310, 1 },
							new int[] { 4306, 1 }), HAM_MALE(new int[] { 1714 },
									20, 22.5, 2, 4,
									new int[] { 995, 2 + r(19) },
									new int[] { 4302, 1 },
									new int[] { 4304, 1 },
									...
									new int[] { 4300, 1 },
									new int[] { 4310, 1 },
									new int[] { 4306, 1 }), WARRIOR(
											new int[] { 15, 18 }, 25, 26.0, 2,
											5, new int[] { 995, 18 }), ROGUE(
													new int[] { 187 }, 32, 35.5,
													2, 5, new int[] { 995, 25 },
													new int[] { 995, 40 },
													new int[] { 7919, 1 },
	...
	public static void attemptPickpocket(final Client c, final int npcId) {
		...
		if (c.underAttackBy > 0 || c.underAttackBy2 > 0) {
			c.getActionSender()
					.sendMessage("You can't pickpocket while in combat!");
			return;
		}
		if (!THIEVING) {
			c.getActionSender()
			...
			if (npcId == n.getNpc(npcId)) {
				if (c.playerLevel[c.playerThieving] < n.getLevel()) {
					c.getDialogueHandler()
							.sendStatement("You need a Thieving level of "
									+ n.getLevel() + " to pickpocket the "
									+ NpcHandler.getNpcListName(n.getNpc(npcId))
											.toLowerCase()
									+ ".");
					return;
				}
				c.getActionSender()
						.sendMessage("You attempt to pick the  " + NpcHandler
								.getNpcListName(n.getNpc(npcId)).toLowerCase()
								+ "'s pocket.");
				...
				if (Misc.random(c.playerLevel[17] + 5) < Misc
					...
					CycleEventHandler.getSingleton().addEvent(c,
							new CycleEvent() {
								...
								public void execute(
									...
									NpcHandler.npcs[npcId].forceChat(
											"What do you think you're doing?");
									NpcHandler.npcs[npcId]
											.facePlayer(c.playerId);
									c.lastThieve = System.currentTimeMillis()
											+ 5000;
									c.getActionSender()
											.sendMessage("You fail to pick the "
													+ NpcHandler.getNpcListName(
															n.getNpc(npcId))
															.toLowerCase()
													+ "'s pocket.");
									container.stop();
					...
					String message = "You pick the " + NpcHandler
							.getNpcListName(n.getNpc(npcId)).toLowerCase()
							+ "'s pocket.";
							...
							new CycleEvent() {
								...
								public void execute(
									...
									c.getPlayerAssistant().addSkillXP(
											(int) n.getXp(), c.playerThieving);
									int[] random = n.getPickPockets()[Misc
											.random(n.getPickPockets().length
													- 1)];
