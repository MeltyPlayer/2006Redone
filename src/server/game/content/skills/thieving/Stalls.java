...
public class Stalls {
	...
	public static enum stallData {
		VEGETABLE_STALL(4706, 2, 10, 0, new int[] { 1965, 1 }), BAKER_STALL(
				2561, 5, 16, 3, new int[] { 2309, 1 }, new int[] { 1891, 1 },
				new int[] { 1895, 1 }), TEA_STALL(635, 5, 16, 0,
						new int[] { 712, 1 }), SILK_STALL(2560, 20, 24, 2,
								new int[] { 950, 1 }), WINE_STALL(14011, 22, 27,
										0, new int[] { 1935, 1 },
										new int[] { i("jug of water"), 1 },
										new int[] { i("jug of wine"), 1 },
										new int[] { i("grapes"),
												1 }), SEED_STALL(7053, 27, 10,
														0,
														new int[] { i(
																"potato seed"),
																1 },
		...
		private stallData(final int objectId, final int levelReq,
				final double xp, final int face, final int[]... stalls) {
	...
	public static void attemptStall(final Client c, final int objectId,
		...
		for (final stallData s : stallData.values()) {
			...
			if (c.underAttackBy > 0 || c.underAttackBy2 > 0) {
				c.getActionSender().sendMessage(
						"You can't steal from a stall while in combat!");
				return;
			}
			...
			if (objectId == s.getObject()) {
				if (c.playerLevel[c.playerThieving] >= s.getLevel()) {
					...
					Server.objectHandler.createAnObject(c, 634, x, y,
							s.getFace());
					c.getPlayerAssistant().addSkillXP((int) s.getXp(),
							c.playerThieving);
					int[] random = s.getStalls()[Misc
							.random(s.getStalls().length - 1)];
					if (c.isBotting == false) {
						c.getItemAssistant().addItem(random[0],
								random[1] + (random.length > 2
										? Misc.random(random[2])
										: 0));
					}
					c.lastThieve = System.currentTimeMillis();
					c.getActionSender()
							.sendMessage("You steal a "
									+ ItemAssistant.getItemName(random[0])
									+ " from the stall.");
					...
					c.getDialogueHandler()
							.sendStatement("You must have a thieving level of "
									+ s.getLevel()
	 ...
	 * 14011: case 4705: return 16000; case 7053: return 11000; case 2563:
	 * return 15000; case 2565: return 30000; case 2564: return 80000; case
	 * 2562: return 180000; } return 2500 + r(2500); }
	 */
		...
		for (int i = 1; i < NpcHandler.maxNPCs; i++) {
			if (NpcHandler.npcs[i] != null) {
				if (NpcHandler.npcs[i].npcType == 32
					...
					if (c.goodDistance(c.absX, c.absY, NpcHandler.npcs[i].absX,
							NpcHandler.npcs[i].absY, 7)
							&& c.heightLevel == NpcHandler.npcs[i].heightLevel) {
						if (!NpcHandler.npcs[i].underAttack) {
							NpcHandler.npcs[i].forceChat(
