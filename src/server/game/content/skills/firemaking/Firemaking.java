package server.game.content.skills.firemaking;
...
public class Firemaking {
	...
	public static boolean stopFiremaking = false, pickedUpFiremakingLog = false,
			logLit;
	...
	public static void attemptFire(final Client c, final int itemUsed,
		...
		for (int i = 0; i < firemakingItems.length; i++) {
			...
			for (final LogData l : LogData.values()) {
				final int logId = usedWith == firemakingItems[i] ? itemUsed
						: usedWith;
				if (logId == l.getLogId()) {
					if (c.playerLevel[11] < l.getLevel()) {
						c.getActionSender()
								.sendMessage("You need a firemaking level of "
										+ l.getLevel() + " to light "
										+ ItemAssistant.getItemName(logId));
						return;
					}
					if (c.inBank() || c.inLumbBuilding()
							|| c.inDraynorBuilding()) {
						c.getActionSender()
								.sendMessage("You cannot light a fire here.");
						return;
					}
					...
					boolean notInstant = System.currentTimeMillis()
							- SkillHandler.lastSkillingAction > 2500;
					int cycle = 2;
					if (notInstant) {
						c.getActionSender()
								.sendMessage("You attempt to light a fire.");
						c.getActionSender().sendSound(SoundList.FIRE_LIGHT, 100,
								0);
						if (!notInstant) {
							c.getActionSender()
									.sendSound(SoundList.FIRST_ATTEMPT, 100, 0);// testing
						}
						...
						if (groundObject == false) {
							c.getItemAssistant().deleteItem(logId,
									c.getItemAssistant().getItemSlot(logId), 1);
							Server.itemHandler.createGroundItem(c, logId,
					...
					CycleEventHandler.getSingleton().addEvent(c,
							new CycleEvent() {
								...
								public void execute(
									...
									if (c.isWoodcutting || c.playerIsFletching
											|| c.isFletching) {
										container.stop();
									}
									if (c.isFiremaking == true) {
										...
										c.getActionSender().sendSound(
												SoundList.FIRE_SUCCESSFUL, 100,
												0);
										if (itemUsed == 7331
												|| usedWith == 7331)
											new Object(11406, x, y, 0, 0, 10,
													-1, 60 + Misc.random(30));
										else if (itemUsed == 7330
												|| usedWith == 7330)
											new Object(11405, x, y, 0, 0, 10,
													-1, 60 + Misc.random(30));
										else if (itemUsed == 7329
												|| usedWith == 7329)
											new Object(11404, x, y, 0, 0, 10,
													-1, 60 + Misc.random(30));
										else
											new Object(2732, x, y, 0, 0, 10, -1,
													60 + Misc.random(30));
										c.getActionSender().sendMessage(
												"The fire catches and the log beings to burn.");
										c.getPlayerAssistant().addSkillXP(
												(int) l.getXp(), 11);
										if (c.tutorialProgress == 4) {
											c.getDialogueHandler()
													.sendDialogues(3016, 943);
										}
					...
					CycleEventHandler.getSingleton().addEvent(c,
							new CycleEvent() {
								...
								public void execute(
									...
									Server.objectHandler.createAnObject(c, -1,
											x, y);
