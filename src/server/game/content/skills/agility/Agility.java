...
public class Agility {
  ...
  public static final int LOG_EMOTE = 762, PIPES_EMOTE = 844, CLIMB_UP_EMOTE = 828, CLIMB_DOWN_EMOTE = 827,
      CLIMB_UP_MONKEY_EMOTE = 3487, WALL_EMOTE = 840;
  public int steppingStone, steppingStoneTimer = 0, agilityTimer = -1, moveHeight = -1, tropicalTreeUpdate = -1,
      zipLine = -1;
   ...
   * heightlevel was not correct while the heightlevel was not correct we could
   * not change heightlevels and use the walkToEmote directly so we need to add
   * a little timer to make it work
   */
  ...
  /**
   * a specific position the player has to stand on before the action is set to
   * true
   */
  ...
  public void agilityProcess() {
    ...
    if (zipLine == 0) {
      walk(0, -39, getAnimation(WerewolfAgility.WEREWOLF_SLING_OBJECT), 743);
      zipLine = -1;
    }

    if (steppingStone > 0 && steppingStoneTimer == 0) {
      walk(-1, 0, getAnimation(WildernessAgility.WILDERNESS_STEPPING_STONE_OBJECT), -1);
      steppingStone--;
  ...
  public boolean checkLevel(int objectId) {
    if (getLevelRequired(objectId) > c.playerLevel[c.playerAgility]) {
      c.getActionSender().sendMessage("You need atleast " + getLevelRequired(objectId) + " agility to do this.");
      return true;
    }
    return false;
  }
    ...
    if (changeObjectTimer == 0) {
      ...
      for (Player player : PlayerHandler.players) {
        if (player != null) {
          ...
          c.getActionSender().createObjectHints(newObjectX, newObjectY, 130, 2);
