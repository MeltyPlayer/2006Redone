    ...
    for (final leatherData l : leatherData.values()) {
      ...
      if (leather == l.getLeather()) {
        ...
        if (l.getLeather() == 1743) {
          ...
          c.getPlayerAssistant().sendFrame126(ItemAssistant.getItemName(1131), 2799);
          c.leatherType = leather;
        }
        if (l.getLeather() == 6289) {
          ...
          for (int i = 0; i < name.length; i++) {
            c.getPlayerAssistant().sendFrame126(name[i], 8949 + i * 4);
          }
          c.leatherType = leather;
          return;
        }
      ...
      if (buttonId == l.getButtonId(buttonId)) {
        if (c.leatherType == l.getLeather()) {
          if (c.playerLevel[12] < l.getLevel()) {
            c.getActionSender().sendMessage("You need a crafting level of " + l.getLevel() + " to make this.");
            c.getPlayerAssistant().removeAllWindows();
            return;
          }
          if (!c.getItemAssistant().playerHasItem(1734)) {
            c.getActionSender().sendMessage("You need some thread to make this.");
            c.getPlayerAssistant().removeAllWindows();
            return;
          }
          if (!c.getItemAssistant().playerHasItem(c.leatherType, l.getHideAmount())) {
            c.getActionSender().sendMessage(
                "You need " + l.getHideAmount() + " " + ItemAssistant.getItemName(c.leatherType).toLowerCase()
                    + " to make " + ItemAssistant.getItemName(l.getProduct()).toLowerCase() + ".");
            c.getPlayerAssistant().removeAllWindows();
            return;
          }
          ...
          CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
            ...
            public void execute(CycleEventContainer container) {
              if (c.isCrafting == true) {
                if (!c.getItemAssistant().playerHasItem(1734)) {
                  c.getActionSender().sendMessage("You have run out of thread.");
                  container.stop();
                  return;
                }
                if (!c.getItemAssistant().playerHasItem(c.leatherType, l.getHideAmount())) {
                  c.getActionSender().sendMessage("You have run out of leather.");
                  container.stop();
                  return;
                }
                if (amount == 0) {
                ...
                c.getItemAssistant().deleteItem(1734, c.getItemAssistant().getItemSlot(1734), 1);
                c.getItemAssistant().deleteItem(c.leatherType, l.getHideAmount());
                c.getItemAssistant().addItem(l.getProduct(), 1);
                c.getActionSender().sendMessage(
                    "You make " + (ItemAssistant.getItemName(l.getProduct()).contains("body") ? "a" : "some") + " "
                        + ItemAssistant.getItemName(l.getProduct()) + ".");
                c.getPlayerAssistant().addSkillXP((int) l.getXP(), 12);
                c.startAnimation(1249);
                amount--;
                if (!c.getItemAssistant().playerHasItem(1734)) {
                  c.getActionSender().sendMessage("You have run out of thread.");
