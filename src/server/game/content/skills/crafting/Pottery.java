...
public class Pottery {
  ...
  public static void makeUnfire(final Client c, final int id, final double xp, final int level, int amount) {
    ...
    if (c.getItemAssistant().playerHasItem(softClay) && c.playerLevel[12] >= level && c.isPotCrafting == true) {
      c.startAnimation(unFire);
      c.getItemAssistant().deleteItem2(softClay, 1);
      c.getItemAssistant().addItem(id, 1);
      c.getActionSender().sendMessage("You make the soft clay into a " + ItemAssistant.getItemName(id) + ".");
    ...
    CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
      ...
      public void execute(CycleEventContainer container) {
        if (c.getItemAssistant().playerHasItem(softClay) && c.playerLevel[12] >= level && !(c.doAmount <= 0)
            && c.isPotCrafting == true) {
          c.startAnimation(unFire);
          c.getItemAssistant().deleteItem2(softClay, 1);
          c.getItemAssistant().addItem(id, 1);
        ...
        if (c.playerLevel[12] < level) {
          container.stop();
          c.getActionSender().sendMessage("You need a crafting level of " + level + " to make this.");
        }

        if (!c.getItemAssistant().playerHasItem(softClay)) {
          container.stop();
          c.getActionSender().sendMessage("You need soft clay to do this.");
        }
  ...
  public static void makeFire(final Client c, final int startId, final int finishId, final int level, final double xp,
      int amount) {
    c.getPlayerAssistant().closeAllWindows();
    c.doAmount = amount;
    c.isPotCrafting = true;
    if (c.getItemAssistant().playerHasItem(startId) && c.playerLevel[12] >= level && c.isPotCrafting == true) {
      ...
      c.getActionSender().sendMessage("You put a " + ItemAssistant.getItemName(startId) + " into the oven.");
      c.getActionSender().sendMessage("You retrieve the " + ItemAssistant.getItemName(finishId) + " from the oven.");
      c.getPlayerAssistant().addSkillXP(xp, c.playerCrafting);
      c.doAmount--;
    }
    ...
    if (!c.getItemAssistant().playerHasItem(startId) && c.playerLevel[12] >= level) {
      c.getActionSender().sendMessage("You need an " + ItemAssistant.getItemName(startId) + " to do this.");
    }
    ...
    CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
      ...
      public void execute(CycleEventContainer container) {
        if (c.getItemAssistant().playerHasItem(startId) && c.playerLevel[12] >= level && c.isPotCrafting == true
          ...
          c.getActionSender().sendMessage("You put a " + ItemAssistant.getItemName(startId) + " into the oven.");
          c.getActionSender()
              .sendMessage("You retrieve the " + ItemAssistant.getItemName(finishId) + " from the oven.");
          c.getPlayerAssistant().addSkillXP(xp, c.playerCrafting);
          c.doAmount--;
        }

        if (c.isPotCrafting == false || !c.getItemAssistant().playerHasItem(startId) || c.playerLevel[12] < level) {
          container.stop();
        }
