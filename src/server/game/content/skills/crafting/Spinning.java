...
public class Spinning extends CraftingData {
  ...
  public static int[][] BEFORE_AFTER = { { 1737, 1759, 3, 1 }, { 1779, 1777, 15, 10 } };
          ...
          if (c.isSpinning == true) {
            if (c.getItemAssistant().playerHasItem(before)) {
              if (c.playerLevel[c.playerCrafting] < level) {
                c.getDialogueHandler().sendStatement("You need a crafting level of " + level + " to do this.");
                return;
              }
              ...
              c.getActionSender().sendMessage("You spin the " + ItemAssistant.getItemName(before) + " into a "
                  + ItemAssistant.getItemName(after) + ".");
              c.doAmount--;
            }

            if (!c.getItemAssistant().playerHasItem(before) || c.isSpinning == false) {
              container.stop();
            }
