...
public class JewelryMaking {
  ...
  private final static int[][] RINGS = { // Ring, Gem, Level, XP
      { 1635, -1, 5, 15 }, { 1637, 1607, 20, 40 }, { 1639, 1605, 27, 55 }, { 1641, 1603, 34, 70 },
      { 1643, 1601, 43, 85 }, { 1645, 1615, 55, 100 }, { 6575, 6573, 67, 115 } };
  private final static int[][] NECKLACES = { { 1654, -1, 6, 20 }, { 1656, 1607, 22, 55 }, { 1658, 1605, 29, 60 },
      { 1660, 1603, 40, 75 }, { 1662, 1601, 56, 90 }, { 1664, 1615, 72, 105 }, { 6577, 6573, 82, 120 } };
  private static int[][] AMULETS = { { 1673, -1, 8, 30 }, { 1675, 1607, 24, 65 }, { 1677, 1605, 31, 70 },
      { 1679, 1603, 50, 85 }, { 1681, 1601, 70, 100 }, { 1683, 1615, 80, 150 }, { 6579, 6573, 90, 165 } };
  ...
  public static void mouldInterface(Client c) {
    ...
    if (c.getItemAssistant().playerHasItem(1592, 1)) {
      for (int i = 0; i < MOULD_INTERFACE_IDS[0].length; i++) {
        c.getPlayerAssistant().sendFrame34(MOULD_INTERFACE_IDS[0][i], i, 4233, 1);
      }
      ...
      c.getPlayerAssistant().sendFrame126("You need a ring mould to craft rings.", 4230);
    }
    /* Necklace */
    if (c.getItemAssistant().playerHasItem(1597, 1)) {
      for (int i = 0; i < MOULD_INTERFACE_IDS[1].length; i++) {
        c.getPlayerAssistant().sendFrame34(MOULD_INTERFACE_IDS[1][i], i, 4239, 1);
      }
      ...
      c.getPlayerAssistant().sendFrame126("You need a necklace mould to craft necklaces", 4236);
      for (int i = 0; i < MOULD_INTERFACE_IDS[1].length; i++) {
    ...
    if (c.getItemAssistant().playerHasItem(1595, 1)) {
      for (int i = 0; i < MOULD_INTERFACE_IDS[2].length; i++) {
        c.getPlayerAssistant().sendFrame34(MOULD_INTERFACE_IDS[2][i], i, 4245, 1);
      }
      ...
      c.getPlayerAssistant().sendFrame126("You need a amulet mould to craft necklaces", 4242);
      for (int i = 0; i < MOULD_INTERFACE_IDS[2].length; i++) {
  ...
  public static void mouldItem(Client c, int item, int amount) {
    ...
    if (c.playerLevel[c.playerCrafting] >= lvl) {
      if (ItemAssistant.getItemName(itemAdd).toLowerCase().contains("gold")
          && !c.getItemAssistant().playerHasItem(GOLD_BAR, 1) || !c.getItemAssistant().playerHasItem(GOLD_BAR, 1)) {
        c.getActionSender().sendMessage("You need a Gold bar to make this.");
        return;
      } else if (!c.getItemAssistant().playerHasItem(gem, 1) && c.getItemAssistant().playerHasItem(GOLD_BAR, 1)) {
        c.getActionSender().sendMessage(getRequiredMessage(ItemAssistant.getItemName(gem)));
        return;
      }
      c.getPlayerAssistant().removeAllWindows();
      while (done < amount && (ItemAssistant.getItemName(gem).toLowerCase().contains("unarmed")
          && c.getItemAssistant().playerHasItem(GOLD_BAR, 1)
          || c.getItemAssistant().playerHasItem(gem, 1) && c.getItemAssistant().playerHasItem(GOLD_BAR, 1))) {
      ...
      if (done == 1) {
        c.getActionSender().sendMessage("You craft the gold to form a " + ItemAssistant.getItemName(itemAdd) + ".");
      } else if (done > 1) {
        c.getActionSender()
            .sendMessage("You craft the gold to form " + done + " " + ItemAssistant.getItemName(itemAdd) + "'s.");
      }
    } else {
      c.getActionSender().sendMessage("You need a Crafting level of " + lvl + " to craft this.");
      return;
    }
  }

  public static String getRequiredMessage(String item) {
    if (item.startsWith("A") || item.startsWith("E") || item.startsWith("I") || item.startsWith("O")
        || item.startsWith("U")) {
