    ...
    for (final cutGemData g : cutGemData.values()) {
      if (itemId == g.getUncut()) {
        if (c.playerLevel[12] < g.getLevel()) {
          c.getActionSender().sendMessage("You need a crafting level of " + g.getLevel() + " to cut this gem.");
          return false;
        }
        ...
        CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
          ...
          public void execute(CycleEventContainer container) {
            if (Misc.random(2) == 0 && itemUsed > 1624 && itemUsed < 1630
                || usedWith > 1624 && usedWith < 1630 && Misc.random(2) == 0) {
            ...
            if (c.isCrafting == true) {
              if (c.getItemAssistant().playerHasItem(itemId)) {
                ...
                c.getItemAssistant();
                c.getActionSender().sendMessage("You cut the " + ItemAssistant.getItemName(itemId).toLowerCase() + ".");
                c.startAnimation(g.getAnimation());
                c.getActionSender().sendSound(SoundList.CUT_GEM, 100, 0);
              } else {
                container.stop();
              }
            } else {
