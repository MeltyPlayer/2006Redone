			...
			if (itemId == g.getUncut()) {
				...
				CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
					...
					public void execute(CycleEventContainer container) {
						if (Misc.random(2) == 0 && itemUsed > 1624
								&& itemUsed < 1630
								|| usedWith > 1624 && usedWith < 1630
										&& Misc.random(2) == 0) {
							c.getActionSender()
									.sendMessage("You fail to cut the gem.");
								...
								c.getActionSender()
										.sendMessage(
												"You cut the "
														...
														+ ".");
								c.startAnimation(g.getAnimation());
								c.getActionSender().sendSound(SoundList.CUT_GEM,
										100, 0);
							} else {
								container.stop();
							}
