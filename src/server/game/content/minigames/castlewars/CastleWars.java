package server.game.content.minigames.castlewars;
...
public class CastleWars {
  ...
  public static boolean deleteCastleWarsItems(Client c, int itemId) {
    ...
    for (int item : items) {
      ...
      if (itemId == item && !isInCw(c)) {
        ...
        c.getActionSender()
            .sendMessage("You shouldn't have " + ItemAssistant.getItemName(itemId) + " outside of castlewars!");
        return false;
      }
    }
    return true;
  ...
  public static void collapseCave(int cave) {
    ...
    while (iterator.hasNext()) {
      ...
      if (teamPlayer.absX > COLLAPSE_ROCKS[cave][0] && teamPlayer.absX < COLLAPSE_ROCKS[cave][1]
          && teamPlayer.absY > COLLAPSE_ROCKS[cave][2] && teamPlayer.absY < COLLAPSE_ROCKS[cave][3]) {
  ...
  /**
   * Method we use to add someone to the waitinroom in a different method, this
   * will filter out some error messages
   * 
   * @param player
  ...
  public static void addToWaitRoom(Client player, int team) {
    if (player == null) {
      ...
      player.getActionSender()
          .sendMessage("There's already a Castle Wars going. Please wait a few minutes before trying again.");
      return;
    } else if (player.playerEquipment[player.playerHat] > 0 || player.playerEquipment[player.playerCape] > 0
        || player.getItemAssistant().playerHasItem(player.playerCape)
        || player.getItemAssistant().playerHasItem(player.playerHat)) {
   ...
   *          the player that wants to join
   * @param team
   *          team he wants to be in - team = 1 (saradomin), team = 2 (zamorak),
   *          team = 3 (random)
   */
  public static void toWaitingRoom(Client player, int team) {
    int[] food = { 391, 385, 379, 333, 329, 373, 361, 7946, 397, 1891, 365, 339, 1942, 6701, 6705, 7056, 7054, 7058,
        7060, 315, 347, 325, 1897, 2289, 2293, 2297, 2301, 2309, 2323, 2325, 2327, 351, 6703, 1963, 6883, 2108, 2118,
        2116, 15272, 1961, 1959, 1989, 1935, 1915, 1978, 712, 1907, 2120, 2114, 1973, 1993, 355, 3144, 1917, 1909, 2955,
        1969, 6701, 1893, 1895, 1965, 1899, 1901, 2291, 2295, 2297, 2299, 2301, 2303, 1883, 6883, 2108, 2118, 2116,
        7928, 7929, 7930, 7931, 7932, 7933, 10476, 2130, 2003, 2011, 4049 };
    ...
    if (team == 1) {
      ...
      if (getZammyPlayers() >= getSaraPlayers() || getSaraPlayers() == 0) {
        player.getActionSender().sendMessage("You have been added to the Saradomin team.");
        player.getActionSender()
        ...
        player.getPlayerAssistant().movePlayer(WAIT_ROOM[team - 1][0] + Misc.random(5),
      ...
      if (getZammyPlayers() <= getSaraPlayers() || getZammyPlayers() == 0) {
        player.getActionSender().sendMessage("Random team: You have been added to the Zamorak team.");
        player.getActionSender()
            .sendMessage("Next Game Begins In: " + (gameStartTimer * 3 + timeRemaining * 3) + " seconds.");
  ...
  public static void dropFlag(Client player, int flagId) {
    ...
    for (Client teamPlayer : gameRoom.keySet()) {
      teamPlayer.getActionSender().object(object, player.getX(), player.getY(), 0, 10);
    }
  }
  ...
  public static void pickupFlag(Client player) {
    ...
    while (iterator.hasNext()) {
      Client teamPlayer = iterator.next();
      teamPlayer.getActionSender().createObjectHints(player.objectX, player.objectY, 170, -1);
      teamPlayer.getActionSender().object(-1, player.objectX, player.objectY, 0, 10);
    }
    return;
  }
  ...
  public static void createHintIcon(Client player, int t) {
    ...
    while (iterator.hasNext()) {
      ...
      if (gameRoom.get(teamPlayer) == t) {
        ...
        teamPlayer.getActionSender().createPlayerHints(10, player.playerId);
  ...
  public static void leaveWaitingRoom(Client player) {
    ...
    if (waitingRoom.containsKey(player)) {
      ...
      player.getPlayerAssistant().movePlayer(2439 + Misc.random(4), 3085 + Misc.random(5), 0);
      return;
    }
    player.getPlayerAssistant().movePlayer(2439 + Misc.random(4), 3085 + Misc.random(5), 0);
    ...
    while (iterator.hasNext()) {
      ...
      if (player != null) {
        player.getPlayerAssistant()
            .sendFrame126("Next Game Begins In: " + (gameStartTimer * 3 + timeRemaining * 3) + " seconds.", 6570);
        player.getPlayerAssistant().sendFrame126("Zamorak Players: " + getZammyPlayers() + ".", 6572);
        player.getPlayerAssistant().sendFrame126("Saradomin Players: " + getSaraPlayers() + ".", 6664);
  ...
  public static void updateInGamePlayers() {
    if (getSaraPlayers() > 0 && getZammyPlayers() > 0) {
      ...
      while (iterator.hasNext()) {
        ...
        player.getPlayerAssistant().sendFrame126("Zamorak = " + scores[1], 11147);
        player.getPlayerAssistant().sendFrame126(scores[0] + " = Saradomin", 11148);
        player.getPlayerAssistant().sendFrame126(timeRemaining * 3 + " secs", 11155);
  ...
  public static void startGame() {
    ...
    while (iterator.hasNext()) {
      ...
      player.getPlayerAssistant().movePlayer(GAME_ROOM[team - 1][0] + Misc.random(3),
  ...
  public static void endGame() {
    ...
    while (iterator.hasNext()) {
      ...
      player.getActionSender().sendMessage("Castle Wars: The Castle Wars game has ended!");
      player.getActionSender().sendMessage("Castle Wars: Kills: " + player.cwKills + " Deaths: " + player.cwDeaths
          + " Games Played: " + player.cwGames + ".");
      player.getActionSender().createPlayerHints(10, -1);
      deleteGameItems(player);
      player.isDead = false;
      for (int i = 0; i < 25; i++) {
      ...
      if (scores[0] == scores[1]) {
        ...
        player.getActionSender().sendMessage("Tie game! You earn 1 ticket!");
      } else if (team == 1) {
        if (scores[0] > scores[1]) {
          player.getItemAssistant().addItem(4067, 2);
          player.getActionSender().sendMessage("You won the game. You received 2 Castle Wars Tickets!");
        } else if (scores[0] < scores[1]) {
          player.getItemAssistant().addItem(4067, 0);
          player.getActionSender().sendMessage("You lost the game. You received no tickets!");
        }
      } else if (team == 2) {
  ...
  public static void removePlayerFromCw(Client player) {
    ...
    if (gameRoom.containsKey(player)) {
      ...
      } else if (player.getItemAssistant().playerHasEquipped(ZAMMY_BANNER)) {
        player.getItemAssistant().removeItem(player.playerEquipment[3], 3);
        setZammyFlag(0); // safe flag
      }
      deleteGameItems(player);
      player.getPlayerAssistant().movePlayer(2440, 3089, 0);
      player.getActionSender().sendMessage("The Casle Wars game has ended for you!");
      player.getActionSender().sendMessage("Kills: " + player.cwKills + " Deaths: " + player.cwDeaths + ".");
      player.getActionSender().createPlayerHints(10, -1);
  ...
  public static void deleteGameItems(Client player) {
    ...
    for (int item : items) {
      if (player.getItemAssistant().playerHasItem(item)) {
        player.getItemAssistant().deleteItem2(item, player.getItemAssistant().getItemAmount(item));
      }
    }
  }
  ...
  public static void changeFlagObject(int objectId, int team) {
    ...
    while (iterator.hasNext()) {
      Client teamPlayer = iterator.next();
      teamPlayer.getActionSender().object(objectId, FLAG_STANDS[team][0], FLAG_STANDS[team][1], 0, 10);
    }
  }
}
