...
public class FightPits {
  ...
  public static void removePlayer(Client c, boolean forceRemove) {
    c.inPits = false;
    if (forceRemove) {
      c.getPlayerAssistant().movePlayer(EXIT_WAITING_X, EXIT_WAITING_Y, 0);
      playerMap.remove(c);
      return;
    }
    ...
    if (state.equals(PLAYING)) {
      if (getListCount(PLAYING) - 1 == 0 && !forceRemove) {
        ...
        c.getItemAssistant().addItem(TOKKUL_ID, 1500 + Misc.random(500));
  ...
  public static boolean updateWaitingRoom(Client c) {
    c.getPlayerAssistant().sendFrame126("Next Game Begins In : " + gameStartTimer, 2805);
    c.getPlayerAssistant().sendFrame126("Champion: JalYt-Ket-" + pitsChampion, 2806);
  ...
  public static boolean updateGame(Client c) {
    c.getPlayerAssistant().sendFrame126("Foes Remaining: " + getListCount(PLAYING), 2805);
    c.getPlayerAssistant().sendFrame126("Champion: JalYt-Ket-" + pitsChampion, 2806);
    c.getPlayerAssistant().sendConfig(560, 1);
    c.getPlayerAssistant().walkableInterface(2804);
    return true;
