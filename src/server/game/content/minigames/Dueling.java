...
public class Dueling {
		...
		try {
			...
			if (player.inTrade || player.isShopping) {
				player.getActionSender()
						.sendMessage("You can not stake currently.");
				return;
			}
			if (!player.inDuelArena()) {
				player.getActionSender().sendMessage(
						"You must be in the duel arena to do that.");
				return;
			}
			resetDuel();
			resetDuelItems();
			 ...
			 * player.getActionSender().
			 * sendMessage("You can't duel your own IP."); return; }
			 */
			player.duelRequested = true;
			if (player.duelStatus == 0 && o.duelStatus == 0
					&& player.duelRequested && o.duelRequested
					&& player.duelingWith == o.getId()
				...
				o.getActionSender()
						.sendMessage(player.playerName + ":duelreq:");
			}
		} catch (Exception e) {
	...
	public void openDuel() {
		...
		if (!player.inDuelArena()) {
			player.getActionSender()
					.sendMessage("You must be in the duel arena to do that.");
			return;
		}
		if (player.duelingArena()) {
			player.getActionSender()
					.sendMessage("You can't do that in a duel!");
			return;
		}
		player.duelStatus = 1;
		refreshduelRules();
		...
		for (int i = 0; i < player.playerEquipment.length; i++) {
			sendDuelEquipment(player.playerEquipment[i],
					player.playerEquipmentN[i], i);
		}
		player.getPlayerAssistant().sendFrame126("Dueling with: " + o.playerName
	...
	public void refreshDuelScreen() {
		synchronized (player) {
			...
			player.getOutStream()
					.writeWord(o.getDueling().stakedItems.toArray().length);
			current = 0;
	...
	public boolean stakeItem(int itemID, int fromSlot, int amount) {
		...
		for (int i : Constants.ITEM_TRADEABLE) {
			if (i == itemID || itemID >= 6864 && itemID <= 6882) {
				player.getActionSender()
						.sendMessage("You can't stake this item.");
				return false;
			}
		}
		if (player.inTrade || player.isShopping) {
	...
	public boolean fromDuel(int itemID, int fromSlot, int amount) {
		...
		if (Item.itemStackable[itemID]) {
			if (player.getItemAssistant().freeSlots()
					- 1 < player.duelSpaceReq) {
				player.getActionSender().sendMessage(
					...
					if (item.id == itemID) {
						if (!item.stackable) {
							...
							if (item.amount > amount) {
								item.amount -= amount;
								player.getItemAssistant().addItem(itemID,
										amount);
							} else {
								if (player.getItemAssistant().freeSlots()
										- 1 < player.duelSpaceReq) {
	...
	public void confirmDuel() {
		...
		if (!player.inDuelArena()) {
			player.getActionSender().sendMessage(
					"You must be in the duel arena to confirm a duel.");
			return;
		}
		if (player.duelingArena()) {
			player.getActionSender()
		...
		player.getPlayerAssistant().sendFrame126("Hitpoints will be restored.",
				8250);
		player.getPlayerAssistant()
				.sendFrame126("Boosted stats will be restored.", 8238);
	...
	public void startDuel() {
		...
		if (!player.inDuelArena()) {
			player.getActionSender().sendMessage(
					"You must be in the duel arena to start a duel.");
			return;
		}
		if (player.inTrade || player.isShopping) {
			player.getActionSender()
		...
		if (player.duelRule[7]) {
			for (int p = 0; p < player.getPrayer().PRAYER.length; p++) { // reset
				...
				player.getPlayerAssistant()
						.sendConfig(player.getPrayer().PRAYER_GLOW[p], 0);
			}
			player.headIcon = -1;
		...
		if (player.duelRule[19]) {
			player.getItemAssistant().removeItem(player.playerEquipment[10],
					10);
		}
		if (player.duelRule[20]) {
			player.getItemAssistant().removeItem(player.playerEquipment[12],
					12);
		}
		if (player.duelRule[21]) {
			player.getItemAssistant().removeItem(player.playerEquipment[13],
					13);
		}
		player.duelStatus = 5;
		...
		if (player.duelRule[8]) {
			if (player.duelRule[1]) {
				player.getPlayerAssistant().movePlayer(player.duelTeleX,
						player.duelTeleY, 0);
			} else {
	...
	public void duelVictory() {
		...
		if (opponent != null) {
			player.getPlayerAssistant().sendFrame126("" + opponent.combatLevel,
					6839);
		...
		if (player.getPlayerAssistant().isPlayer()) {
			GameLogger.writeLog(player.playerName, "duelingkiller",
					player.playerName + " killed " + opponent.playerName
							+ " in the duel arena.");
		}
		player.getPlayerAssistant().movePlayer(
				Constants.DUELING_RESPAWN_X + Misc.random(5),
		...
		for (GameItem item : otherStakedItems) {
			if (item.id > 0 && item.amount > 0) {
				if (Item.itemStackable[item.id]) {
					if (!player.getItemAssistant().addItem(item.id,
							item.amount)) {
						Server.itemHandler.createGroundItem(player, item.id,
								player.getX(), player.getY(), item.amount,
								player.getId());
					}
				} else {
	...
	public void checkDuelWalk() {
		Client o = (Client) PlayerHandler.players[player.duelingWith];
		if (player.duelStatus == 5 && o.duelStatus == 5 && o.duelingArena()
				&& !player.duelingArena()) {
	...
	public void selectRule(int i) { // rules
		...
		if (!player.inDuelArena()) {
			player.getActionSender().sendMessage(
					"You must be in the duel arena to change your rules.");
			return;
		}
		if (player.duelingArena()) {
			player.getActionSender()
		...
		if (i == 16
				&& (player.getItemAssistant()
						.is2handed(ItemAssistant.getItemName(
