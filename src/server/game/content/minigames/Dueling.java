...
public class Dueling {
  ...
  public void refreshDuelScreen() {
    synchronized (player) {
      ...
      player.getOutStream().writeWord(o.getDueling().stakedItems.toArray().length);
      current = 0;
  ...
  public void confirmDuel() {
    ...
    player.getPlayerAssistant().sendFrame126("Boosted stats will be restored.", 8238);
    if (player.duelRule[8]) {
      player.getPlayerAssistant().sendFrame126("There will be obstacles in the arena.", 8239);
    }
    ...
    for (int i = 0; i < 8; i++) {
      if (player.duelRule[i]) {
        player.getPlayerAssistant().sendFrame126("" + rulesOption[i], lineNumber);
        lineNumber++;
      }
    }
  ...
  public void startDuel() {
    ...
    if (player.duelRule[7]) {
      for (int p = 0; p < player.getPrayer().PRAYER.length; p++) { // reset
        ...
        player.getPlayerAssistant().sendConfig(player.getPrayer().PRAYER_GLOW[p], 0);
      }
      player.headIcon = -1;
      ...
      if (item.id > 0 && item.amount > 0) {
        if (Item.itemStackable[item.id]) {
          if (!player.getItemAssistant().addItem(item.id, item.amount)) {
            Server.itemHandler.createGroundItem(player, item.id, player.getX(), player.getY(), item.amount,
                player.getId());
          }
        } else {
  ...
  public void selectRule(int i) { // rules
    ...
    if (i == 16
        && (player.getItemAssistant().is2handed(
            ItemAssistant.getItemName(player.playerEquipment[player.playerWeapon]).toLowerCase(),
            player.playerEquipment[player.playerWeapon]) && player.getItemAssistant().freeSlots() == 0)
        || (o.getItemAssistant().is2handed(
            ItemAssistant.getItemName(player.playerEquipment[player.playerWeapon]).toLowerCase(),
            player.playerEquipment[player.playerWeapon]) && o.getItemAssistant().freeSlots() == 0)) {
