...
public class PestControl {
  ...
  private final int[][] voidMonsterData = { { shifter, 2660 + Misc.random(4), 2592 + Misc.random(4) },
   ...
   * ().sendMessage("Next game will start in: 60 seconds."); break; case 30: c.
   * getPacketDispatcher().sendMessage("Next game will start in: 30 seconds." );
   * break; } } catch (RuntimeException e) { // TODO Auto-generated catch block
   ...
   * (gameTimer > 60) {
   * player.getPlayerAssistant().sendString("Time remaining: " + (gameTimer/60)
   * + " minutes", 21117); } else {
  ...
  private void startGame() {
    ...
    for (Client player : waitingBoat.keySet()) {
      ...
      player.getActionSender().sendMessage("The Pest Control Game has begun!");
    ...
    for (Client player : gamePlayers.keySet()) {
      ...
      if (won && player.pcDamage > 50) {
        ...
        player.getActionSender().sendMessage(
            "You have won the pest control game and have been awarded " + POINT_REWARD + " Pest Control points.");
        ...
        player.getActionSender()
            .sendMessage("The void knights notice your lack of zeal. You only gain " + POINT_REWARD2 + " points.");
      } else {
        player.getDialogueHandler().sendDialogues(597, 3790);
        player.getActionSender()
            .sendMessage("You failed to kill all the portals in 3 minutes and have not been awarded points.");
      }
    }
    cleanUpPlayer();
    cleanUp();
  ...
  public static void addToWaitRoom(Client player) {
    if (player != null && player.combatLevel > 39) {
      ...
      player.getActionSender().sendMessage("You have joined the Pest Control boat.");
      player.getActionSender().sendMessage("You currently have " + player.pcPoints + " Pest Control Points.");
      player.getActionSender().sendMessage("There are currently " + playersInBoat() + " players ready in the boat.");
      player.getActionSender().sendMessage("Players needed: " + PLAYERS_REQUIRED + " to 25 players.");
