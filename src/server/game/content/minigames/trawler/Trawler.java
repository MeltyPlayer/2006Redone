package server.game.content.minigames.trawler;
...
public class Trawler extends GroupMinigame {
  ...
  /*
   * The arraylist containing all of the players that need to removed Created so
   * that an instance wouldn't have to be created every time the updating is
   * called simply just call the clear method
   */
  ...
  public enum Wall {
    ...
    North_One_Normal(0, 1885, 4826), North_Two_Normal(1, 1886, 4826), North_Three_Normal(2, 1887,
        4826), North_Four_Normal(3, 1888, 4826), North_Five_Normal(4, 1889, 4826), North_Six_Normal(5, 1890,
            4826), North_Seven_Normal(6, 1891, 4826), North_Eight_Normal(7, 1892, 4826), South_One_Normal(8, 1885,
                4823), South_Two_Normal(9, 1886, 4823), South_Three_Normal(10, 1887, 4823), South_Four_Normal(11, 1888,
                    4823), South_Five_Normal(12, 1889, 4823), South_Six_Normal(13, 1890, 4823), South_Seven_Normal(14,
                        1891, 4823), South_Eight_Normal(15, 1892, 4823), North_One_Sinking(0, 2013,
                            4826), North_Two_Sinking(1, 2014, 4826), North_Three_Sinking(2, 2015,
                                4826), North_Four_Sinking(3, 2016, 4826), North_Five_Sinking(4, 2017,
                                    4826), North_Six_Sinking(5, 2018, 4826), North_Seven_Sinking(6, 2019,
                                        4826), North_Eight_Sinking(7, 2020, 4826), South_One_Sinking(8, 2013,
                                            4823), South_Two_Sinking(9, 2014, 4823), South_Three_Sinking(10, 2015,
                                                4823), South_Four_Sinking(11, 2016, 4823), South_Five_Sinking(12, 2017,
                                                    4823), South_Six_Sinking(13, 2018, 4823), South_Seven_Sinking(14,
                                                        2019, 4823), South_Eight_Sinking(15, 2020, 4823);
  ...
  public void updateWall(int index) {
    ...
    if (wall_status[index] == true) {
      Server.objectHandler.createAnObject(leaking_wall, w.x, w.y, w.y == 4826 ? 1 : 3);
      if (isSunk)
        Server.objectHandler.createAnObject(leaking_wall, w.x - 128, w.y, w.y == 4826 ? 1 : 3);
      else
        Server.objectHandler.createAnObject(leaking_wall, w.x + 128, w.y, w.y == 4826 ? 1 : 3);
    } else {
      Server.objectHandler.createAnObject(patched_wall, w.x, w.y, w.y == 4826 ? 1 : 3);
      if (isSunk)
        Server.objectHandler.createAnObject(patched_wall, w.x - 128, w.y, w.y == 4826 ? 1 : 3);
      else
        Server.objectHandler.createAnObject(patched_wall, w.x + 128, w.y, w.y == 4826 ? 1 : 3);
    }
  }
  ...
  /*
   * Updates every play in the game's interface, if the player is null they are
  ...
  public void playerUpdates() {
    for (Client p : players) {
      if (p != null) {
        p.asClient().getPlayerAssistant().sendFrame126(net_ripped ? "@red@Ripped" : "@gre@Okay", 11935);
  ...
  public void upLadder(Client p, int obX, int obY) {
    if (doAction(p)) {
      if (!isSunk) {
        ...
        p.getPlayerAssistant().movePlayer(obX == 1884 ? 1885 : 1892, obY, 1);
      } else {
        p.startAnimation(climb_up);
  ...
  public void downLadder(Client p, int obX, int obY) {
    if (doAction(p)) {
      if (!isSunk) {
        ...
        p.getPlayerAssistant().movePlayer(obX == 1884 ? 1885 : 1892, obY, 0);
      } else {
        p.startAnimation(climb_down);
  ...
  public void fixNet(Client p) {
    if (doAction(p)) {
      ...
      if (!p.getItemAssistant().playerHasItem(rope)) {
        p.getActionSender().sendMessage("You need a rope before attempting to fix the net!");
        return;
      }
  ...
  public void bail(Client p) {
    if (doAction(p)) {
      if (p.getItemAssistant().playerHasItem(bailing_bucket_empty)) {
        ...
        p.getItemAssistant().replaceItem(bailing_bucket_empty, bailing_bucket_full);
  ...
  public void emptyBucket(Client p) {
    if (doAction(p)) {
      if (p.getItemAssistant().playerHasItem(bailing_bucket_full)) {
        p.startAnimation(832);
        p.getItemAssistant().replaceItem(bailing_bucket_full, bailing_bucket_empty);
      }
    }
  }
    ...
    if (water_level > 25 && !isSunk) {
      ...
      for (int j = 0; j < players.size(); j++) {
        if (players.get(j) != null) {
          ...
          players.get(j).getPlayerAssistant().movePlayer(players.get(j).absX + 128, players.get(j).absY,
    ...
    CycleEventHandler.getSingleton().addEvent(this, new CycleEvent() {
      ...
      public void execute(CycleEventContainer container) {
        if (game_time == 0) {
          ...
          game_time--;
          waiting_room.messageWaiting(
              "The trawler will return in " + game_time + (game_time == 1 ? " minute" : " minutes") + "!");
        }
      }
  ...
  public int chanceByLevel(Player p, int fish) {
    switch (fish) {
      case 381:
        if (p.playerLevel[p.playerFishing] >= 81 && p.playerLevel[p.playerFishing] < 90) {
          return 5;
        } else if (p.playerLevel[p.playerFishing] >= 90 && p.playerLevel[p.playerFishing] < 99) {
          return 9;
        ...
        return 0;
      case 395:
        if (p.playerLevel[p.playerFishing] >= 79 && p.playerLevel[p.playerFishing] < 85) {
          return 8;
        } else if (p.playerLevel[p.playerFishing] >= 85 && p.playerLevel[p.playerFishing] < 95) {
          return 13;
  ...
  public void showReward(Client c) {
    ...
    for (int j = 0; j < c.fishingTrawlerReward.size(); j++) {
      c.getPlayerAssistant().sendFrame34(c.fishingTrawlerReward.get(j).id, j, 4640,
          c.fishingTrawlerReward.get(j).amount);
    }
  }

  public void updateRewardSlot(Client c, int slot) {
    c.inFishingTrawlerRewardsInterface = true;
    c.getPlayerAssistant().sendFrame34(c.fishingTrawlerReward.get(slot).id, slot, 4640,
        c.fishingTrawlerReward.get(slot).amount);
    if (slot != 4 && c.fishingTrawlerReward.size() == 5) {
      c.getPlayerAssistant().sendFrame34(c.fishingTrawlerReward.get(4).id, 4, 4640,
          c.fishingTrawlerReward.get(4).amount);
    }
  }
