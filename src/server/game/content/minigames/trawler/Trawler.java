...
public class Trawler extends GroupMinigame {
	...
	public enum Wall {
		...
		North_One_Normal(0, 1885, 4826), North_Two_Normal(1, 1886,
				4826), North_Three_Normal(2, 1887, 4826), North_Four_Normal(3,
						1888, 4826), North_Five_Normal(4, 1889,
								4826), North_Six_Normal(5, 1890,
										4826), North_Seven_Normal(6, 1891,
												4826), North_Eight_Normal(7,
														1892,
														4826), South_One_Normal(
																8, 1885,
																4823), South_Two_Normal(
																		9, 1886,
																		4823), South_Three_Normal(
																				10,
																				1887,
																				4823), South_Four_Normal(
																						11,
																						1888,
																						4823), South_Five_Normal(
																								12,
																								1889,
																								4823), South_Six_Normal(
																										13,
																										1890,
																										4823), South_Seven_Normal(
																												14,
																												1891,
																												4823), South_Eight_Normal(
																														15,
																														1892,
																														4823), North_One_Sinking(
																																0,
																																2013,
																																4826), North_Two_Sinking(
																																		1,
																																		2014,
																																		4826), North_Three_Sinking(
																																				2,
																																				2015,
																																				4826), North_Four_Sinking(
																																						3,
																																						2016,
																																						4826), North_Five_Sinking(
																																								4,
																																								2017,
																																								4826), North_Six_Sinking(
																																										5,
																																										2018,
																																										4826), North_Seven_Sinking(
																																												6,
																																												2019,
																																												4826), North_Eight_Sinking(
																																														7,
																																														2020,
																																														4826), South_One_Sinking(
																																																8,
																																																2013,
																																																4823), South_Two_Sinking(
																																																		9,
																																																		2014,
																																																		4823), South_Three_Sinking(
																																																				10,
																																																				2015,
																																																				4823), South_Four_Sinking(
																																																						11,
																																																						2016,
																																																						4823), South_Five_Sinking(
																																																								12,
																																																								2017,
																																																								4823), South_Six_Sinking(
																																																										13,
																																																										2018,
																																																										4823), South_Seven_Sinking(
																																																												14,
																																																												2019,
																																																												4823), South_Eight_Sinking(
																																																														15,
																																																														2020,
																																																														4823);
	...
	public void resetWalls() {
		for (Wall w : Wall.values()) {
			if (w != null) {
				Server.objectHandler.removeObject(
						Server.objectHandler.getObjectByPosition(w.x, w.y));
				Server.objectHandler.createAnObject(perfect_wall, w.x, w.y,
						w.y == 4826 ? 1 : 3);
			}
		}
	}
	...
	public void updateWall(int index) {
		...
		Server.objectHandler.removeObject(
				Server.objectHandler.getObjectByPosition(w.x, w.y));
		Server.objectHandler.removeObject(Server.objectHandler
				.getObjectByPosition(w.x + (isSunk ? -128 : 128), w.y));
	...
	public void playerUpdates() {
		for (Client p : players) {
			if (p != null) {
				p.asClient().getPlayerAssistant().sendFrame126(
						net_ripped ? "@red@Ripped" : "@gre@Okay", 11935);
	...
	public void increaseWaterLevel() {
		...
		water_level += (leaks / 2)
				+ random_gen.nextInt(leaks * random_gen.nextInt(2) + 1);
	}
	...
	public void fixHole(Client p, int x, int y) {
		if (doAction(p)) {
			if (p.getItemAssistant().playerHasItem(swamp_paste)) {
				...
				p.getActionSender()
						.sendMessage("You don't have any swamp paste.");
			}
		}
	}
	...
	public void upLadder(Client p, int obX, int obY) {
		if (doAction(p)) {
			if (!isSunk) {
				...
				p.getPlayerAssistant().movePlayer(obX == 1884 ? 1885 : 1892,
						obY, 1);
			} else {
				p.startAnimation(climb_up);
				p.getPlayerAssistant().movePlayer(obX == 2021 ? 2020 : 2013,
						obY, 1);
			}
		}
	}
	...
	public void fixNet(Client p) {
		if (doAction(p)) {
			...
			if (skillCheck(p.playerLevel[p.playerCrafting], 1, 0)) {
				...
				playerUpdates();
				p.getActionSender()
						.sendMessage("You successfully fix the net!");
			} else {
				p.getItemAssistant().deleteItem(rope, 1);
				p.getActionSender()
						.sendMessage("You failed to repair the net!");
			}
		}
	}
	...
	public boolean doAction(Player p) {
		...
		if (System.currentTimeMillis()
				- p.lastFishingTrawlerInteraction >= 1600) {
	...
	public void bail(Client p) {
		if (doAction(p)) {
			if (p.getItemAssistant().playerHasItem(bailing_bucket_empty)) {
				...
				p.getItemAssistant().replaceItem(bailing_bucket_empty,
						bailing_bucket_full);
	...
	public void emptyBucket(Client p) {
		if (doAction(p)) {
			if (p.getItemAssistant().playerHasItem(bailing_bucket_full)) {
				p.startAnimation(832);
				p.getItemAssistant().replaceItem(bailing_bucket_full,
						bailing_bucket_empty);
			}
		}
	}
		...
		if (water_level > 25 && !isSunk) {
			...
			for (int j = 0; j < players.size(); j++) {
				if (players.get(j) != null) {
					...
					players.get(j).getPlayerAssistant().movePlayer(
							players.get(j).absX + 128, players.get(j).absY,
		...
		CycleEventHandler.getSingleton().addEvent(this, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				if (game_time == 0) {
					...
					game_time--;
					waiting_room.messageWaiting("The trawler will return in "
							+ game_time
							+ (game_time == 1 ? " minute" : " minutes") + "!");
				}
			}
	...
	public void showReward(Client c) {
		...
		for (int j = 0; j < c.fishingTrawlerReward.size(); j++) {
			c.getPlayerAssistant().sendFrame34(c.fishingTrawlerReward.get(j).id,
					j, 4640, c.fishingTrawlerReward.get(j).amount);
		}
	}

	public void updateRewardSlot(Client c, int slot) {
		c.inFishingTrawlerRewardsInterface = true;
		c.getPlayerAssistant().sendFrame34(c.fishingTrawlerReward.get(slot).id,
				slot, 4640, c.fishingTrawlerReward.get(slot).amount);
		if (slot != 4 && c.fishingTrawlerReward.size() == 5) {
			c.getPlayerAssistant().sendFrame34(c.fishingTrawlerReward.get(4).id,
					4, 4640, c.fishingTrawlerReward.get(4).amount);
		}
	}
