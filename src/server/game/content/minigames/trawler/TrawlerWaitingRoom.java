...
public class TrawlerWaitingRoom extends WaitingRoom {
	...
	public void onStart() {
		...
		/*
		 * for(Iterator<Player> i = waiting.iterator(); i.hasNext();) { Player p
		 * = i.next(); if(!boat.playerInArea(p)) { i.remove(); // Allowed with
		 * an iterator } }
		 */
	...
	public void onJoin(Client p) {
		...
		if (!isActive()) {
			p.getActionSender().sendMessage(trawler.getGameTime() == 0
					? "The trawler will be returning in less than a minute!"
					: "The trawler will return in " + trawler.getGameTime()
							+ (trawler.getGameTime() == 1 ? " minute"
									: " minutes")
							+ "!");
		} else {
			p.getActionSender().sendMessage(getTimeRemaining() == 0
					? "The trawler will be leaving in less than a minute!"
					: "The trawler will leave in " + getTimeRemaining()
							+ (getTimeRemaining() == 1 ? " minute" : " minutes")
							+ "!");
		}
	}
