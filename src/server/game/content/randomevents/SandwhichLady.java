...
public class SandwhichLady {
	...
	public static void handleOptions(Client player, int actionbuttonId) {
			...
			case 63013:
				if (itemType == 0) {
					...
					player.getActionSender()
							.sendMessage("You have chosen the wrong item!");
		...
		} else if (player.hasSandwhichLady == false && actionbuttonId > 63008
				&& actionbuttonId < 63116) {
			player.getActionSender().sendMessage(
					"You have improperly opened the sandwhich lady interface.");
		}
	}
	...
	public static void openSandwhichLady(Client player) {
		...
		case 2:
			player.getPlayerAssistant()
					.sendFrame126("Please select the chocolate.", 16145);
			itemType = 2;
			break;
		case 3:
		...
		case 5:
			player.getPlayerAssistant()
					.sendFrame126("Please select the square sandwich.", 16145);
			itemType = 5;
			break;
		case 6:
