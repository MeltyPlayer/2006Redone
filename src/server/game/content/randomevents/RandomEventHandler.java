...
public class RandomEventHandler {
	...
	public static void failEvent(Client player) {
		...
		player.getActionSender()
				.sendMessage("You wake up in a strange location...");
		resetEvent(player);
	...
	public static void callRandom(Client player) { // add all random events here
		if (player.inFightCaves()
				|| player.playerEquipment[player.playerWeapon] == 4024
		...
		default:
			System.out.println("Error no random event called for "
					+ player.playerName + ".");
			break;
		}
	}
	...
	public static void addRandom(Client player) {
		if (player.randomActions >= CALL_RANDOM) {
			...
			if (player.playerRights == 3) {
				player.getActionSender().sendMessage("Next random will be in "
						+ nextRandom + " more random actions.");
			}
