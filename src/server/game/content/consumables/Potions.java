...
public class Potions {
  ...
  public void handlePotion(int itemId, int slot) {
    if (c.duelRule[5]) {
      c.getActionSender().sendMessage("You may not drink potions in this duel.");
      return;
    }
    ...
    if (System.currentTimeMillis() - c.potDelay >= 1200) {
      ...
      c.attackTimer++;
      c.getActionSender().sendMessage("You drink some of your " + Item.getItemName(itemId) + ".");
  ...
  public void restoreStats() {
    for (int j = 0; j <= 6; j++) {
      ...
      if (c.playerLevel[j] < c.getLevelForXP(c.playerXP[j])) {
        ...
        c.getActionSender().setSkillLevel(j, c.playerLevel[j], c.playerXP[j]);
      }
    }
  }
  ...
  public void doTheBrewzam(int itemId, int replaceItem, int slot) {
    ...
    for (int tD : toDecrease) {
      ...
      c.getActionSender().setSkillLevel(tD, c.playerLevel[tD], c.playerXP[tD]);
    }
  ...
  public void antifirePot(int itemId, int replaceItem, int slot) {
    ...
    c.antiFirePot = true;
    c.antiFirePotion();
    c.getActionSender().sendMessage("Your immunity against dragon fire has been increased.");
    c.getItemAssistant().resetItems(3214);
  ...
  public boolean potionNames(int itemId) {
    String name = ItemAssistant.getItemName(itemId);
    return name.endsWith("potion(4)") || name.endsWith("potion(3)") || name.endsWith("potion(2)")
        || name.endsWith("potion(1)") || name.contains("saradomin brew") || name.contains("zamorak brew");
  }
}
