...
public class CastRequirements {
  ...
  public static boolean hasRunes(Client c, int[] runes, int[] amount) {
    ...
    c.getActionSender().sendMessage("You don't have enough required runes to cast this spell!");
    return false;
  }
  ...
  public static void deleteRunes(Client c, int[] runes, int[] amount) {
    for (int i = 0; i < runes.length; i++) {
      c.getItemAssistant().deleteItem(runes[i], c.getItemAssistant().getItemSlot(runes[i]), amount[i]);
    }
  }
