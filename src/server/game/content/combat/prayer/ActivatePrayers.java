  ...
  public static void activatePrayer(Client c, int i) {
    if (c.duelRule[7]) {
      for (int p = 0; p < c.getPrayer().PRAYER.length; p++) { // reset
        ...
        c.getPlayerAssistant().sendConfig(c.getPrayer().PRAYER_GLOW[p], 0);
      }
      c.getActionSender().sendMessage("Prayer has been disabled in this duel!");
      return;
    }
    if (i == 24 && c.playerLevel[1] < 65) {
      c.getPlayerAssistant().sendConfig(c.getPrayer().PRAYER_GLOW[i], 0);
      c.getActionSender().sendMessage("You may not use this prayer yet.");
      return;
    }
    ...
    if (c.playerLevel[5] > 0 || !Constants.PRAYER_POINTS_REQUIRED) {
      if (c.getPlayerAssistant().getLevelForXP(c.playerXP[5]) >= c.getPrayer().PRAYER_LEVEL_REQUIRED[i]
        ...
        switch (i) {
            ...
            if (c.getPrayer().prayerActive[i] == false) {
              for (int j = 0; j < defencePrayer.length; j++) {
                if (defencePrayer[j] != i) {
                  ...
                  c.getPlayerAssistant().sendConfig(c.getPrayer().PRAYER_GLOW[defencePrayer[j]], 0);
                }
              }
            }
            break;
            ...
            if (c.getPrayer().prayerActive[i] == false) {
              for (int j = 0; j < strengthPrayer.length; j++) {
                if (strengthPrayer[j] != i) {
                  ...
                  c.getPlayerAssistant().sendConfig(c.getPrayer().PRAYER_GLOW[strengthPrayer[j]], 0);
                }
              }
              for (int j = 0; j < rangePrayer.length; j++) {
                if (rangePrayer[j] != i) {
                  c.getPrayer().prayerActive[rangePrayer[j]] = false;
                  c.getPlayerAssistant().sendConfig(c.getPrayer().PRAYER_GLOW[rangePrayer[j]], 0);
                }
              }
              for (int j = 0; j < magePrayer.length; j++) {
                if (magePrayer[j] != i) {
                  c.getPrayer().prayerActive[magePrayer[j]] = false;
                  c.getPlayerAssistant().sendConfig(c.getPrayer().PRAYER_GLOW[magePrayer[j]], 0);
                }
              }
            }
            break;
            ...
            if (c.getPrayer().prayerActive[i] == false) {
              for (int j = 0; j < attackPrayer.length; j++) {
                if (attackPrayer[j] != i) {
                  ...
                  c.getPlayerAssistant().sendConfig(c.getPrayer().PRAYER_GLOW[attackPrayer[j]], 0);
                }
              }
              for (int j = 0; j < rangePrayer.length; j++) {
                if (rangePrayer[j] != i) {
          ...
          case 17:
          case 18:
            if (System.currentTimeMillis() - c.getPrayer().stopPrayerDelay < 5000) {
              c.getActionSender().sendMessage("You have been injured and can't use this prayer!");
              c.getPlayerAssistant().sendConfig(c.getPrayer().PRAYER_GLOW[16], 0);
              c.getPlayerAssistant().sendConfig(c.getPrayer().PRAYER_GLOW[17], 0);
              c.getPlayerAssistant().sendConfig(c.getPrayer().PRAYER_GLOW[18], 0);
              return;
            }
            if (i == 16) {
        ...
        if (!headIcon) {
          if (c.getPrayer().prayerActive[i] == false) {
            ...
            c.getPlayerAssistant().sendConfig(c.getPrayer().PRAYER_GLOW[i], 1);
          } else {
        ...
        c.getPlayerAssistant().sendConfig(c.getPrayer().PRAYER_GLOW[i], 0);
        c.getPlayerAssistant().sendFrame126("You need a @blu@Prayer level of " + c.getPrayer().PRAYER_LEVEL_REQUIRED[i]
            + " to use " + c.getPrayer().PRAYER_NAME[i] + ".", 357);
        c.getPlayerAssistant().sendFrame126("Click here to continue", 358);
        c.getPlayerAssistant().sendChatInterface(356);
      }
    } else {
      c.getPlayerAssistant().sendConfig(c.getPrayer().PRAYER_GLOW[i], 0);
      c.getActionSender().sendMessage("You have run out of prayer points!");
