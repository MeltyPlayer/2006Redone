...
import server.game.content.combat.magic.MagicMaxHit;
import server.game.content.combat.magic.MagicRequirements;
import server.game.content.combat.magic.MagicSpells;
import server.game.content.combat.melee.MeleeData;
import server.game.content.combat.melee.MeleeMaxHit;
...
import server.game.content.combat.range.RangeMaxHit;
...
import server.game.content.skills.slayer.SlayerRequirements;
...
public class CombatAssistant {
  ...
  public void delayedHit(int i) { // npc hit delay
    if (NpcHandler.npcs[i] != null) {
      ...
      if (NpcHandler.npcs[i].attackTimer <= 3
          || NpcHandler.npcs[i].attackTimer == 0 && NpcHandler.npcs[i].hitDelayTimer > 0 && !c.castingMagic) { // block
                                                                                                               // animation
      ...
      if (!c.castingMagic && c.projectileStage > 0) { // range hit
        ...
        if (NpcHandler.npcs[i].npcType == FightCaves.TZTOK_JAD && NpcHandler.npcs[i].spawnedBy == c.getId()
            && ((NpcHandler.npcs[i].HP < (FightCaves.getHp(FightCaves.TZTOK_JAD) / 2)) && (NpcHandler.npcs[i].HP
                - damage + (damage2 > -1 ? damage2 : 0) < (FightCaves.getHp(FightCaves.TZTOK_JAD) / 2)))) {
        ...
        if (NpcHandler.npcs[i].npcType == FightCaves.TZTOK_JAD && NpcHandler.npcs[i].spawnedBy == c.getId()
            && ((NpcHandler.npcs[i].HP > (FightCaves.getHp(FightCaves.TZTOK_JAD) / 2))
  ...
  public void fireProjectileNpc() {
    if (c.oldNpcIndex > 0) {
      if (NpcHandler.npcs[c.oldNpcIndex] != null) {
        ...
        int offX = (pY - nY) * -1;
        int offY = (pX - nX) * -1;
        c.getPlayerAssistant().createPlayersProjectile(pX, pY, offX, offY, 50, RangeData.getProjectileSpeed(c),
            RangeData.getRangeProjectileGFX(c), 43, 31, c.oldNpcIndex + 1, MagicSpells.getStartDelay(c));
        if (RangeData.usingDbow(c)) {
          c.getPlayerAssistant().createPlayersProjectile2(pX, pY, offX, offY, 50, RangeData.getProjectileSpeed(c),
              RangeData.getRangeProjectileGFX(c), 60, 31, c.oldNpcIndex + 1, MagicSpells.getStartDelay(c), 35);
        }
      }
    }
  }
    ...
    if (NpcHandler.npcs[i] != null) {
      ...
      if (NpcHandler.npcs[i].npcType == 9) {
        if (c.absX == 3225 && c.absY > 3459 && c.absY < 3465
            || c.absX > 3222 && c.absX < 3226 && c.absY > 3456 && c.absY < 3460
            || c.absX > 3213 && c.absX < 3223 && c.absY == 3457 || c.absX > 3202 && c.absX < 3212 && c.absY == 3457
            || c.absX > 3199 && c.absX < 3203 && c.absY > 3456 && c.absY < 3460
      ...
      if (NpcHandler.npcs[i].npcType == 134 || NpcHandler.npcs[i].npcType == 1267
      ...
      if (c.attackTimer <= 0) {
        ...
        if (armaNpc(i) && !usingCross && !usingBow && !c.usingMagic && !RangeData.usingCrystalBow(c)
        ...
        if (!c.goodDistance(c.getX(), c.getY(), NpcHandler.npcs[i].getX(), NpcHandler.npcs[i].getY(), 2)
            && RangeData.usingHally(c) && !usingOtherRangeWeapons && !usingBow && !c.usingMagic
            ...
            || !c.goodDistance(c.getX(), c.getY(), NpcHandler.npcs[i].getX(), NpcHandler.npcs[i].getY(), 1)
                && !usingOtherRangeWeapons && !RangeData.usingHally(c) && !usingBow && !c.usingMagic
            || !c.goodDistance(c.getX(), c.getY(), NpcHandler.npcs[i].getX(), NpcHandler.npcs[i].getY(), 8)
        ...
        if (!usingCross && !usingArrows && usingBow
          ...
          c.getActionSender().sendMessage("There is no ammo left in your quiver.");
          c.stopMovement();
          c.npcIndex = 0;
          return;
        }
        if (RangeData.correctBowAndArrows(c) < c.playerEquipment[c.playerArrows] && Constants.CORRECT_ARROWS && usingBow
          ...
          c.getActionSender()
              .sendMessage("You can't use " + ItemAssistant.getItemName(c.playerEquipment[c.playerArrows]).toLowerCase()
                  + "s with a " + ItemAssistant.getItemName(c.playerEquipment[c.playerWeapon]).toLowerCase() + ".");
          c.stopMovement();
          c.npcIndex = 0;
          return;
        }
        ...
        if (c.playerEquipment[c.playerWeapon] == 9185 && !properBolts()) {
          c.getActionSender().sendMessage("You must use bolts with a crossbow.");
          c.stopMovement();
          resetPlayerAttack();
          return;
        }
        ...
        if (c.usingSpecial && !c.usingMagic) {
          if (c.getCombatAssistant().checkSpecAmount(c.playerEquipment[c.playerWeapon])) {
            c.lastWeaponUsed = c.playerEquipment[c.playerWeapon];
            c.lastArrowUsed = c.playerEquipment[c.playerArrows];
            c.getSpecials().activateSpecial(c.playerEquipment[c.playerWeapon], i);
            return;
          } else {
        ...
        if (c.usingMagic) { // magic hit delay
          ...
          if (MagicData.MAGIC_SPELLS[c.spellId][4] > 0) {
            c.getPlayerAssistant().createPlayersProjectile(pX, pY, offX, offY, 50, 78,
                MagicData.MAGIC_SPELLS[c.spellId][4], MagicSpells.getStartHeight(c), MagicSpells.getEndHeight(c), i + 1,
                50);
          }
          c.hitDelay = getHitDelay();
          c.oldNpcIndex = i;
          ...
          if (c.crystalBowArrowCount >= 250) {
            switch (c.playerEquipment[c.playerWeapon]) {
              ...
              case 4223: // 1/10 bow
                ...
                c.getActionSender().sendMessage("Your crystal bow has fully degraded.");
              ...
              default:
                c.getItemAssistant().wearItem(++c.playerEquipment[c.playerWeapon], 1, 3);
                c.getActionSender().sendMessage("Your crystal bow degrades.");
                c.crystalBowArrowCount = 0;
                break;
  ...
  public void attackPlayer(int i) {
    ...
    if (PlayerHandler.players[i] != null) {
      ...
      if (CastleWars.isInCw(PlayerHandler.players[i]) && CastleWars.isInCw(c)) {
      ...
      if (!CastleWars.isInCw(PlayerHandler.players[i]) && CastleWars.isInCw(c)) {
        c.getActionSender().sendMessage("You cannot attack people outside castle wars.");
        resetPlayerAttack();
        return;
      }
      ...
      /*
       * if( c.getPlayerAssistant().getWearingAmount() < 4 && c.duelStatus < 1
       * && ! c.inCw() && c.inPits == false ) {
      ...
      if (c.attackTimer <= 0) {
        ...
        if (c.duelRule[9]) {
          ...
          if (!canUseWeapon) {
            c.getActionSender().sendMessage("You can only use fun weapons in this duel!");
            resetPlayerAttack();
            return;
          }
        }
        ...
        if (c.duelRule[2] && (usingBow || usingOtherRangeWeapons)) {
          c.getActionSender().sendMessage("Range has been disabled in this duel!");
          return;
        }
        if (c.duelRule[3] && !usingBow && !usingOtherRangeWeapons && !c.usingMagic) {
          c.getActionSender().sendMessage("Melee has been disabled in this duel!");
          return;
        }

        if (c.duelRule[4] && c.usingMagic) {
          c.getActionSender().sendMessage("Magic has been disabled in this duel!");
          resetPlayerAttack();
          return;
        }
        ...
        if (!c.goodDistance(c.getX(), c.getY(), PlayerHandler.players[i].getX(), PlayerHandler.players[i].getY(), 4)
            ...
            || !c.goodDistance(c.getX(), c.getY(), PlayerHandler.players[i].getX(), PlayerHandler.players[i].getY(), 2)
                && !usingOtherRangeWeapons && RangeData.usingHally(c) && !usingBow && !c.usingMagic
            || !c.goodDistance(c.getX(), c.getY(), PlayerHandler.players[i].getX(), PlayerHandler.players[i].getY(),
                getRequiredDistance()) && !usingOtherRangeWeapons && !RangeData.usingHally(c) && !usingBow
                && !c.usingMagic
            || !c.goodDistance(c.getX(), c.getY(), PlayerHandler.players[i].getX(), PlayerHandler.players[i].getY(), 10)
                && (usingBow || c.usingMagic)) {
        ...
        if (usingBow || c.usingMagic || usingOtherRangeWeapons || RangeData.usingHally(c)) {
          c.stopMovement();
        }
        ...
        if (c.duelStatus != 5 && !PlayerHandler.players[c.playerIndex].inCwGame() && FightPits.getState(c) == null) {
        ...
        if (c.usingMagic) { // magic hit delay
          ...
          if (MagicData.MAGIC_SPELLS[c.spellId][4] > 0) {
            c.getPlayerAssistant().createPlayersProjectile(pX, pY, offX, offY, 50, 78,
                MagicData.MAGIC_SPELLS[c.spellId][4], MagicSpells.getStartHeight(c), MagicSpells.getEndHeight(c),
                -i - 1, MagicSpells.getStartDelay(c));
          }
          if (c.autocastId > 0) {
            c.followId = c.playerIndex;
            c.followDistance = 5;
          ...
          c.oldSpellId = c.spellId;
          c.spellId = 0;
          if (MagicData.MAGIC_SPELLS[c.oldSpellId][0] == 12891 && o.isMoving) {
            // c.getPacketDispatcher().sendMessage("Barrage
            // projectile..");
            c.getPlayerAssistant().createPlayersProjectile(pX, pY, offX, offY, 50, 85, 368, 25, 25, -i - 1,
  ...
  public void playerDelayedHit(int i) {
    if (PlayerHandler.players[i] != null) {
      ...
      if (!c.castingMagic && c.projectileStage > 0) { // range hit
        ...
        if (c.lastWeaponUsed == 11235 || c.bowSpecShot == 1) {
          if (Misc.random(10 + o.getCombatAssistant().calculateRangeDefence()) > Misc
        ...
        if (o.getPrayer().prayerActive[17] && System.currentTimeMillis() - o.protRangeDelay > 1500) { // if
                                                                                                      // prayer
                                                                                                      // active
                                                                                                      // reduce
                                                                                                      // damage
        ...
        if (o.getPrayer().prayerActive[16] && System.currentTimeMillis() - o.protMageDelay > 1500) { // if
                                                                                                     // prayer
                                                                                                     // active
                                                                                                     // reduce
                                                                                                     // damage
        ...
        if (!c.magicFailed) {
          if (System.currentTimeMillis() - PlayerHandler.players[i].reduceStat > 35000) {
            ...
            switch (MagicData.MAGIC_SPELLS[c.oldSpellId][0]) {
              ...
              case 12999:
              case 13023:
                PlayerHandler.players[i].playerLevel[0] -= o.getPlayerAssistant()
          ...
          switch (MagicData.MAGIC_SPELLS[c.oldSpellId][0]) {
            case 12445: // teleblock
              if (System.currentTimeMillis() - o.teleBlockDelay > o.teleBlockLength) {
                ...
                o.getActionSender().sendSound(SoundList.TELEBLOCK_HIT, 100, 0);
            ...
            case 1157:
              PlayerHandler.players[i].playerLevel[2] -= o.getPlayerAssistant()
            ...
            case 1161:
              PlayerHandler.players[i].playerLevel[1] -= o.getPlayerAssistant()
        ...
        if (o.inMulti() && MagicSpells.multis(c)) {
          ...
          for (int j = 0; j < PlayerHandler.players.length; j++) {
            if (PlayerHandler.players[j] != null) {
              ...
              if (o.goodDistance(o.getX(), o.getY(), PlayerHandler.players[j].getX(), PlayerHandler.players[j].getY(),
                  1)) {
  ...
  public void applyPlayerMeleeDamage(int i, int damageMask) {
    ...
    if (o.getPrayer().prayerActive[18] && System.currentTimeMillis() - o.protMeleeDelay > 1500 && !veracsEffect) { // if
                                                                                                                   // prayer
                                                                                                                   // active
                                                                                                                   // reduce
                                                                                                                   // damage
      ...
      case 1: // dragon scimmy special
        if (damage > 0) {
          if (o.getPrayer().prayerActive[16] || o.getPrayer().prayerActive[17] || o.getPrayer().prayerActive[18]) {
            o.headIcon = -1;
            o.getPlayerAssistant().sendConfig(c.getPrayer().PRAYER_GLOW[16], 0);
            o.getPlayerAssistant().sendConfig(c.getPrayer().PRAYER_GLOW[17], 0);
            o.getPlayerAssistant().sendConfig(c.getPrayer().PRAYER_GLOW[18], 0);
          }
  ...
  public void fireProjectilePlayer() {
    if (c.oldPlayerIndex > 0) {
      if (PlayerHandler.players[c.oldPlayerIndex] != null) {
        ...
        if (!c.msbSpec) {
          c.getPlayerAssistant().createPlayersProjectile(pX, pY, offX, offY, 50, RangeData.getProjectileSpeed(c),
              RangeData.getRangeProjectileGFX(c), 43, 31, -c.oldPlayerIndex - 1, MagicSpells.getStartDelay(c));
        } else if (c.msbSpec) {
          c.getPlayerAssistant().createPlayersProjectile2(pX, pY, offX, offY, 50, RangeData.getProjectileSpeed(c),
              RangeData.getRangeProjectileGFX(c), 43, 31, -c.oldPlayerIndex - 1, MagicSpells.getStartDelay(c), 10);
          c.msbSpec = false;
        }
        if (RangeData.usingDbow(c)) {
          c.getPlayerAssistant().createPlayersProjectile2(pX, pY, offX, offY, 50, RangeData.getProjectileSpeed(c),
              RangeData.getRangeProjectileGFX(c), 60, 31, -c.oldPlayerIndex - 1, MagicSpells.getStartDelay(c), 35);
        }
      }
    }
  }
  ...
  public boolean checkReqs() {
    ...
    if (!PlayerHandler.players[c.playerIndex].inWild() && !PlayerHandler.players[c.playerIndex].inCwGame()
        && !CastOnOther.castOnOtherSpells(c)) {
      c.getActionSender().sendMessage("That player is not in the wilderness.");
      c.stopMovement();
      resetPlayerAttack();
      return false;
    }
    if (!c.inWild() && !PlayerHandler.players[c.playerIndex].inCwGame() && !CastOnOther.castOnOtherSpells(c)) {
    ...
    if (Constants.SINGLE_AND_MULTI_ZONES) {
      if (!PlayerHandler.players[c.playerIndex].inMulti()) { // single
        ...
        if (PlayerHandler.players[c.playerIndex].underAttackBy != c.playerId
          ...
          c.getActionSender().sendMessage("That player is already in combat.");
          c.stopMovement();
          resetPlayerAttack();
          return false;
        }
        if (PlayerHandler.players[c.playerIndex].playerId != c.underAttackBy && c.underAttackBy != 0
            || c.underAttackBy2 > 0) {
          c.getActionSender().sendMessage("You are already in combat.");
          c.stopMovement();
          resetPlayerAttack();
          return false;
        }
  ...
  public void handleDfs() {
    if (System.currentTimeMillis() - c.dfsDelay > 30000) {
      if (c.playerIndex > 0 && PlayerHandler.players[c.playerIndex] != null) {
        ...
        c.getActionSender().sendMessage("I should be in combat before using this.");
      }
    } else {
      c.getActionSender().sendMessage("My shield hasn't finished recharging yet.");
    }
  }
   ...
   * c.setHitUpdateRequired2(true); } c.dealDamage(recDamage); c.updateRequired
   * = true; } }
   */
  ...
  public static void removeRecoil(Client c) {
    if (c.recoilHits >= 400) {
      ...
      c.getItemAssistant().deleteItem(2550, c.getItemAssistant().getItemSlot(2550), 1);
  ...
  public void handleGmaulPlayer() {
    ...
    if (c.playerIndex > 0) {
      ...
      if (c.goodDistance(c.getX(), c.getY(), o.getX(), o.getY(), getRequiredDistance())) {
        if (checkReqs()) {
          if (checkSpecAmount(equippedWeapon)) {
            boolean hit = Misc.random(calcAtt()) > Misc.random(o.getCombatAssistant().calcDef());
            int damage = 0;
            if (hit) {
