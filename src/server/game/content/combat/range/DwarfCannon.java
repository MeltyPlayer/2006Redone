...
public class DwarfCannon {
  ...
  public boolean nearCannon() {
    for (int i = 0; i < Server.cannonsX.length; i++) {
      if ((player.absX >= Server.cannonsX[i] - 1) && (player.absX <= Server.cannonsX[i] + 1)
          && (player.absY >= Server.cannonsY[i] - 2) && (player.absY <= Server.cannonsY[i] + 1)) {
        return true;
      }
    }
    return false;
  ...
  public void loadCannon(int x, int y) {
    ...
    if (myBalls <= 29) {
      ...
      if (player.getItemAssistant().playerHasItem(ballsID)) {
        player.getActionSender().sendMessage(amount > 1 ? "You load the cannon with " + amount + " cannonballs."
  ...
  public boolean noSetUpArea() {
    return (player.absX >= 2024 && player.absX <= 2047 && player.absY >= 2047 && player.absY <= 4542) || player.inBank()
        || player.inFightCaves()
        || (player.absX >= 3161 && player.absX <= 3168 && player.absY >= 3486 && player.absY <= 3493);
  }
  ...
  private Npc targetNpc() {
    for (int i = 0; i < NpcHandler.maxNPCs; i++) {
      ...
      int theirX = npc.absX;
      int theirY = npc.absY;
      if (!npc.isDead && !npc.isDead && npc.HP != 0 && npc.npcType != 1266 && npc.npcType != 1268
          && inDistance(theirX, theirY)) {
        switch (rotation) {
          case 1:
  ...
  public boolean inDistance(int npcX, int npcY) {
    return (npcX >= player.cannonX - maxDistance && npcX <= player.cannonX + maxDistance
        && npcY >= player.cannonY - maxDistance && npcY <= player.cannonY + maxDistance);
  }
  ...
  private void cannonProjectile(Npc n) {
    ...
    player.getPlayerAssistant().createPlayersProjectile(oX, oY, offY, offX, 50, 60, 53, 20, 20, -player.oldNpcIndex + 1,
        30);
  }
