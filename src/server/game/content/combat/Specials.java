...
public class Specials {
  ...
  private enum specialAttack {
    ...
    ABYSSAL_WHIP(4151, 1, 1, 5, 1658, 341, -1, false, false, 0), DRAGON_DAGGER(1215, .95, 1.15, 2.5, 1062, -1, 252,
        true, false,
        0), DRAGON_DAGGER_P(1231, .85, 1.15, 2.5, 1062, -1, 252, true, false, 0), DRAGON_DAGGER_PP(5698, .85, 1.15, 2.5,
            1062, -1, 252, true, false, 0), DRAGON_DAGGER_PPP(5680, .85, 1.15, 2.5, 1062, -1, 252, true, false,
                0), DRAGON_LONG(1305, 1.20, 1.10, 2.5, 1058, -1, 248, false, false, 0), DRAGON_MACE(1434, 1.55, .85,
                    2.5, 1060, -1, 251, false, false, 0), DRAGON_SCIMITAR(4587, 1, 1, 5.5, 1872, -1, 347, false, false,
                        1), DRAGON_HALBERD(3204, 1.25, .85, 3.3, 1203, -1, 282, true, false, 0), GRANITE_MAUL(4153,
                            1.10, .85, 5, 1667, -1, 337, false, false, 0), MAGIC_SHORTBOW(861, 1.05, .95, 5.5, 1074, -1,
                                -1, true, false, 0), MAGIC_LONGBOW(859, 1.20, 1.05, 5.5, 426, -1, -1, false, false, 0);
    ...
    private specialAttack(int weapon, double specDamage, double specAccuracy, double specAmount, int anim, int gfx1,
  ...
  public void activateSpecial(int weapon, int i) {
    ...
    if (NpcHandler.npcs[i] == null && player.npcIndex > 0
        || PlayerHandler.players[player.playerIndex] == null && player.playerIndex > 0) {
      return;
    }
  ...
  public void handleGmaul() {
    if (player.npcIndex > 0 && NpcHandler.npcs[player.npcIndex] != null) {
      if (player.goodDistance(player.getX(), player.getY(), NpcHandler.npcs[player.npcIndex].getX(),
          NpcHandler.npcs[player.npcIndex].getY(), player.getCombatAssistant().getRequiredDistance())) {
      ...
      if (player.goodDistance(player.getX(), player.getY(), o.getX(), o.getY(),
  ...
  public static void specialClicking(Client player, int actionButtonId) {
    switch (actionButtonId) {
      ...
      case 29063:
        if (player.playerEquipment[player.playerWeapon] == 1377) {
          if (player.getCombatAssistant().checkSpecAmount(1377)) {
            ...
            player.playerLevel[2] = player.getLevelForXP(player.playerXP[2])
                + player.getLevelForXP(player.playerXP[2]) * 15 / 100;
      ...
      case 29138:
        if (player.playerEquipment[player.playerWeapon] == 1215 || player.playerEquipment[player.playerWeapon] == 1231
