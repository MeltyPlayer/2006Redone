...
public class MemberShipHandler {
	...
	private static void updateBank(Client player) {
		if (player.membership && Constants.MEMBERSHIP) {
			...
			player.getActionSender()
					.sendMessage("Error in the updating bank process.");
		}
	}
	...
	public void teleHome(Client player) {
		if (player.membership == true) {
			...
			player.getActionSender()
					.sendMessage("Teleporting is disabled for non members.");
		}
	}
			...
			public void execute(CycleEventContainer container) {
				if (Constants.MEMBERSHIP) {
					...
					other.getActionSender()
							.sendMessage("You have been awarded membership!");
					other.getActionSender().sendMessage("You now have "
							+ Constants.BANK_SIZE + " bank slots!");// to test
					...
					player.getActionSender().sendMessage(
							"Awarding membership has been disabled.");
					container.stop();
				}
			}
