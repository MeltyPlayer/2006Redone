				...
				if (player != null) {
					...
					if (holiday.getHoliday()) {
						if (p1.playerRights > 2) {
							p1.getActionSender()
									.sendMessage("Currently dropping "
											+ HolidayDrops.dropAmount()
											+ " items.");
						}
						p1.getActionSender()
								.sendMessage("The " + holiday.getName()
										+ " event has started, goodluck!");
						dropItems(c);
					}
				}
			}
				...
				for (int j = 0; j < PlayerHandler.players.length; j++) {
					if (PlayerHandler.players[j] != null) {
						...
						for (HolidayDrops holiday : HolidayDrops.values()) {
							if (holiday.count >= HolidayDrops.dropAmount()
									&& holiday.getHoliday()) {
								stop();
								p1.getActionSender().sendMessage("The "
										+ holiday.getName()
										+ " event has ended, good luck finding the rest of the items!");
							} else if (holiday.count < HolidayDrops.dropAmount()
									&& holiday.getHoliday()) {
								switch (Misc.random(holiday.drops)) {
								case 0:// Varrock
									Server.itemHandler.createGroundItem(p1,
											holiday.getItem(),
											3214 + Misc.random(
													holiday.DROP_DISTANCE),
											3424 - Misc.random(
													holiday.DROP_DISTANCE),
											1, j);
									holiday.count++;
