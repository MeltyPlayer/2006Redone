...
public class BankPin {
  ...
  public void bankPinEnter(int button) {
    if (allowTimer > 0 && allowTimer <= 300000) {
      ...
      if (time >= 2) {
        client.getActionSender().sendMessage("Please wait " + time + " minutes before attempting your bank pin again.");
      } else if (time == 1) {
        client.getActionSender().sendMessage("Please wait " + time + " minute before attempting your bank pin again.");
      } else if (time <= 0) {
        client.getActionSender()
            .sendMessage("Please wait less " + "than a minute before attempting your bank pin again.");
      }
      return;
    }
    sendPins();
  ...
  public void openPin() {
    ...
    randomizeNumbers();
    client.getPlayerAssistant().sendFrame126("First click the FIRST digit", 15313);
  ...
  private void sendPins() {
    ...
    for (int i = 0; i < getBankPins().length; i++) {
      client.getPlayerAssistant().sendFrame126("" + getBankPins()[i], stringIds[i]);
    }
  }

  private void handleButtonOne(int button) {
    client.getPlayerAssistant().sendFrame126("Now click the SECOND digit", 15313);
  ...
  private void handleButtonTwo(int button) {
    client.getPlayerAssistant().sendFrame126("Now click the THIRD digit", 15313);
  ...
  private void handleButtonThree(int button) {
    client.getPlayerAssistant().sendFrame126("Now click the LAST digit", 15313);
  ...
  private void handleButtonFour(int button) {
    ...
    if (!client.hasBankpin) {
      ...
      client.getActionSender().sendMessage("You have just created a bank pin.");
      client.getActionSender()
          .sendMessage("Your new Bank PIN is: " + firstPin + " - " + secondPin + " - " + thirdPin + " - " + fourthPin);
    ...
    if (client.bankPin1 == one && client.bankPin2 == two && client.bankPin3 == three && client.bankPin4 == four) {
      ...
      if (client.attemptsRemaining > 1) {
        client.getActionSender()
            .sendMessage("Invalid pin. You have " + client.attemptsRemaining + " attempts remaining.");
        ...
        client.getActionSender().sendMessage("Invalid pin. You must wait 5 minutes before attempting again.");
      }
      client.getPlayerAssistant().removeAllWindows();
      falseButtons();
    }
  ...
  public String toString() {
    return "BankPin{" + "recovery_Delay=" + recovery_Delay + ", client=" + client + ", allowTimer=" + allowTimer
        + ", bankPins=" + bankPins + ", stringIds=" + stringIds + ", actionButtons=" + actionButtons + ", firstPin="
        + firstPin + ", secondPin=" + secondPin + ", thirdPin=" + thirdPin + ", fourthPin=" + fourthPin + '}';
  }
}
