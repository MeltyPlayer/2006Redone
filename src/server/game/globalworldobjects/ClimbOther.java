...
public class ClimbOther {
  ...
  public enum ClimbData {
    ...
    VARROCK_MANHOLE(881, 882), LUMBRIDGE_TRAPDOOR(14879, 10698), VARROCK_TRAPDOOR(1568, 10698);
  ...
  public static void handleOpenOther(Client player, int objectType) {
    for (ClimbData t : ClimbData.values()) {
      if (objectType == t.getClosed()) {
        new Object(t.getOpen(), player.objectX, player.objectY, player.heightLevel,
            ObjectDefaults.getObjectFace(player, t.getClosed()), 10, t.getClosed(), 100);
        Region.addObject(t.getOpen(), player.objectX, player.objectY, player.heightLevel, 10,
            ObjectDefaults.getObjectFace(player, t.getClosed()), false);
      }
    }
  }
