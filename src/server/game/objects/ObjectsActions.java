...
import server.game.content.guilds.Guilds;
...
import server.game.content.skills.agility.AgilityShortcut;
import server.game.content.skills.core.Mining;
import server.game.content.skills.core.Woodcutting;
import server.game.content.skills.crafting.Spinning;
import server.game.content.skills.farming.Farming;
import server.game.content.skills.fletching.LogCutting;
import server.game.content.skills.runecrafting.RuneCraftingActions;
import server.game.content.skills.smithing.Smelting;
import server.game.content.skills.thieving.Stalls;
import server.game.content.skills.thieving.ThieveOther;
import server.game.content.traveling.Desert;
import server.game.globalworldobjects.ClimbOther;
import server.game.globalworldobjects.DoubleGates;
import server.game.globalworldobjects.PassDoor;
import server.game.globalworldobjects.SingleGates;
import server.game.globalworldobjects.ClimbOther.ClimbData;
import server.game.items.impl.LightSources;
import server.game.npcs.NpcHandler;
import server.game.npcs.impl.MilkCow;
import server.game.objects.impl.AxeInLog;
import server.game.objects.impl.BrimhavenVines;
import server.game.objects.impl.Climbing;
import server.game.objects.impl.FlourMill;
import server.game.objects.impl.Levers;
import server.game.objects.impl.OtherObjects;
import server.game.objects.impl.Pickable;
import server.game.objects.impl.Searching;
import server.game.objects.impl.SpecialObjects;
import server.game.objects.impl.UseOther;
import server.game.objects.impl.Webs;
import server.game.players.Client;
import server.game.players.Position;
  ...
  public void firstClickObject(int objectType, int objectX, int objectY) {
    ...
    switch (objectType) {
      ...
      case 5947:
        ...
        if (player.getItemAssistant().playerHasItem(954) && LightSources.playerHasLightSource(player)) {
          player.getItemAssistant().deleteItem2(954, 1);
          player.getPlayerAssistant().movePlayer(3168, 9572, 0);
          return;
        } else if (player.getItemAssistant().playerHasItem(954) && !LightSources.playerHasLightSource(player)) {
          player.getItemAssistant().deleteItem2(954, 1);
          player.getPlayerAssistant().movePlayer(3168, 9572, 0);
          return;
        } else {
          player.getActionSender().sendMessage("You need a rope to go down there.");
        }
        break;
      ...
      case 2112:
        if (player.absY == 9756 && player.playerLevel[player.playerMining] >= 60) {
          player.getPlayerAssistant().movePlayer(3046, 9757, 0);
          player.getActionSender().sendMessage("You enter the guild.");
        } else if (player.absY == 9757 && player.playerLevel[player.playerMining] >= 60) {
          player.getPlayerAssistant().movePlayer(3046, 9756, 0);
          player.getActionSender().sendMessage("You enter the guild.");
        } else if (player.playerLevel[player.playerMining] < 60) {
          player.getActionSender().sendMessage("You need 60 mining to enter this guild");
        }
        break;
      ...
      case 10596:
        if (player.playerLevel[player.playerSlayer] < 72) {
          player.getActionSender().sendMessage("You need 72 slayer to enter.");
          return;
        } else {
          player.getActionSender().sendMessage("You enter the icy cavern.");
      ...
      case 12982:
        if (player.absY == 3278) {
          player.getPlayerAssistant().movePlayer(player.absX, player.absY - 3, 0);
      ...
      case 2634:
        if (player.absX == 2837 && player.playerLevel[player.playerMining] >= 50) {
          player.getPlayerAssistant().movePlayer(player.absX + 3, player.absY, 0);
        } else if (player.absX == 2840 && player.playerLevel[player.playerMining] >= 50) {
          player.getPlayerAssistant().movePlayer(player.absX - 3, player.absY, 0);
      ...
      case 1804:
        if (player.absY == 3449 && player.getItemAssistant().playerHasItem(983, 1)) {
          player.getPlayerAssistant().movePlayer(player.absX, player.absY + 1, 0);
        } else if (player.absY == 3450 && player.getItemAssistant().playerHasItem(983, 1)) {
          player.getPlayerAssistant().movePlayer(player.absX, player.absY - 1, 0);
      ...
      case 5812:
      case 2341:
        player.getActionSender().sendMessage("Will be added later with the quest!");
        break;
      ...
      case 24:
        if (player.absX == 2764) {
          player.getPlayerAssistant().movePlayer(player.absX + 1, player.absY, 1);
      ...
      case 2670:
        Desert.cutCactus(player, Desert.getCacCutter(player), objectType, objectX, objectY);
        break;
      ...
      case 1516:
        if (player.tutorialProgress == 28 && player.absX == 3129) {
          player.getPlayerAssistant().movePlayer(player.absX - 1, player.absY, player.heightLevel);
      ...
      case 3016:
        if (player.tutorialProgress == 7 || player.diedOnTut == true) {
          if (player.diedOnTut == true) {
            ...
            player.getDialogueHandler().sendStatement2("You have died and have already beat this step",
                "you may continue.");
      ...
      case 3025:
        if (player.tutorialProgress >= 28) {
          ...
          player.getDialogueHandler().chatboxText(player, "Follow the path to the chapel and enter it.",
              "Once inside talk to the monk. He'll tell you all about the skill.", "", "", "Prayer");
      ...
      case 3024:
        if (player.tutorialProgress >= 27) {
          ...
          player.getDialogueHandler().chatboxText(player,
              "The guide here will tell you all about making cash. Just click on", "him to hear what he's got to say.",
              "", "", "Financial advice");
          player.getActionSender().chatbox(6179);
      ...
      case 3045:
        if (player.tutorialProgress == 26) {
          ...
          // 13);
          player.getActionSender().createArrow(3125, 3124, player.getH(), 2);
          player.getActionSender().chatbox(6180);
          player.getDialogueHandler().chatboxText(player,
              ...
              "window and move on through the door indicated.", "This is your bank box");
          player.getActionSender().chatbox(6179);
      ...
      case 3021:
        if (player.diedOnTut == true && (player.getY() == 9502 || player.getY() == 9503)) {
          player.getDialogueHandler().sendStatement2("You have died so now all you need to do is continue",
              "onto the next step.");
          player.getActionSender().createArrow(3111, 9518, player.getH(), 2);
        } else if (player.diedOnTut == false && player.tutorialProgress >= 21
            && (player.getY() == 9502 || player.getY() == 9503)) {
          player.getActionSender().chatbox(6180);
          player.getDialogueHandler().chatboxText(player, "In this area you will find out about combat with swords and",
              "bows. Speak to the guide and he will tell you all about it.", "", "", "Combat");
          player.getActionSender().chatbox(6179);
          ...
          CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
            @Override
            public void execute(CycleEventContainer container) {
              player.getActionSender().object(3020, 3094, 9503, 2, 0);
              player.getActionSender().object(3021, 3094, 9502, 2, 0);
              // others
              player.getActionSender().object(-1, 3095, 9502, 0, 0);
              player.getActionSender().object(-1, 3095, 9503, 0, 0);
      ...
      case 3023:
      case 3022:
        if (player.tutorialProgress >= 24 && (player.getY() == 9519 || player.getY() == 9518)
            || player.diedOnTut == true) {
          if (player.diedOnTut == true) {
            player.getDialogueHandler().sendStatement2("Be more careful this time",
                "now continue to kill the rat and talk to the guide.");
          }
          player.getActionSender().chatbox(6180);
          player.getDialogueHandler().chatboxText(player, "",
              "To attack the rat, right click it and select the attack option. you",
              "will then walk over to it and start hitting it.", "", "Attacking");
          player.getActionSender().chatbox(6179);
          ...
          CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
            ...
            public void execute(CycleEventContainer container) {
              ...
              player.getActionSender().object(3022, 3111, 9518, 0, 0);
              player.getActionSender().object(3023, 3111, 9519, 0, 0);
              // others
              player.getActionSender().object(-1, 3110, 9518, 7, 0);
              player.getActionSender().object(-1, 3110, 9519, 1, 0);
              player.getActionSender().createArrow(1, 6); // draws
                                                          // headicon
              // to combat ude
        ...
        } else if (player.tutorialProgress >= 25 && (player.getY() == 9519 || player.getY() == 9518)) {
          player.getActionSender().object(-1, 3111, 9518, 0, 0);
          player.getActionSender().object(3022, 3110, 9518, 7, 0);
      ...
      case 11719:
        if (player.absY == 3342) {
          player.getPlayerAssistant().movePlayer(player.absX, player.absY + 1, 2);
      ...
      case 9295:
        if (player.playerLevel[player.playerAgility] < 51) {
          player.getActionSender().sendMessage("You need 51 agility to use this shortcut.");
          return;
      ...
      case 8717:
        player.getActionSender().sendMessage("Feature currently disabled will be added in a later release.");
        break;
      ...
      case 3029:
        if (player.tutorialProgress >= 14 || player.diedOnTut == true) {
          if (player.diedOnTut == true) {
            ...
            player.getActionSender().createArrow(3094, 9503, player.getH(), 2);
            player.getDialogueHandler().sendStatement2("You have died and have already beat this step",
                "you may continue.");
          } else if (player.diedOnTut == false) {
      ...
      case 3028:
        if (player.tutorialProgress >= 14) {
          player.getActionSender().sendMessage("You have already completed this step.");
          return;
        }
        break;
      ...
      case 2287:
        if (player.playerLevel[16] < 35) {
          player.getActionSender().sendMessage("You need 35 agility to enter here!");
          return;
        }
      ...
      case 14315:
        if (!PestControl.waitingBoat.containsKey(player) && player.absX == 2657 && player.absY > 2638
            && player.absY < 264) {
      ...
      case 2320:
        long clickTimer = 0;
        if (player.absY <= 9963 && player.playerLevel[player.playerAgility] > 14
        ...
        } else if (player.absY <= 9970 && player.playerLevel[player.playerAgility] > 14
            && System.currentTimeMillis() - clickTimer > 2000) {
      ...
      case 4427:
        player.getPlayerAssistant().movePlayer(2373, player.absY == 3120 ? 3119 : 3120, 0);
        break;

      case 4428:
        player.getPlayerAssistant().movePlayer(2372, player.absY == 3120 ? 3119 : 3120, 0);
        break;

      case 4465:
        player.getPlayerAssistant().movePlayer(player.absX == 2414 ? 2415 : 2414, 3073, 0);
        break;
      ...
      case 4423:
        ...
        player.getPlayerAssistant().movePlayer(2427, player.absY == 3087 ? 3088 : 3087, 0);
        break;
      ...
      case 2071:
        if (player.pirateTreasure == 2) {
          ...
          player.getActionSender().sendMessage("You find a bottle of rum and 10 bananas.");
          player.getItemAssistant().addItem(431, 1);
          player.getItemAssistant().addItem(1963, 10);
          player.nextChat = 0;
        } else {
          player.getActionSender().sendMessage("You aren't on this step right now.");
        }
        break;
      ...
      case 2024: // WP quest
        if (player.witchspot == 2) {
          ...
          player.getActionSender().sendMessage("You are not on this part of the quest.");
        }
        break;
      ...
      case 2072: // crate
        if (player.getItemAssistant().playerHasItem(1963, 10) && player.luthas == true) {
          player.getItemAssistant().deleteItem2(1963, 10);
          player.getDialogueHandler().sendStatement("You pack your bananas in the crate...");
          player.getActionSender().sendMessage("Talk to luthas for your reward.");
          player.bananas = 2;
        } else if (player.getItemAssistant().playerHasItem(431, 1) && player.pirateTreasure == 1) {
          ...
          player.getDialogueHandler().sendStatement("I should talk to luthas and see what to do.");
          player.getActionSender().sendMessage("I think I need to put some bannanas in this crate.");
        }
        break;
      ...
      case 6727:
        if (objectX == 3551 && objectY == 9684) {
          player.getActionSender().sendMessage("You cant open this door..");
        }
        break;
      case 6746:
      ...
      case 2640:
        if (player.inWild()) {
          player.getActionSender().sendMessage("You can't use this in the wilderness.");
          return;
        }
        if (player.playerLevel[5] < player.getPlayerAssistant().getLevelForXP(player.playerXP[5])) {
          player.startAnimation(645);
          player.playerLevel[5] = player.getPlayerAssistant().getLevelForXP(player.playerXP[5]) + 2;
          player.getActionSender().sendMessage("You recharge your prayer points.");
          player.getPlayerAssistant().refreshSkill(5);
        } else {
          player.getActionSender().sendMessage("You already have full prayer points.");
      ...
      case 2407:
        if (player.inWild()) {
          ...
          player.getPlayerAssistant().movePlayer(3171, 3609 + Misc.random(10), 0);
        }
        break;
