...
public class FlourMill {
  ...
  public final static int EMPTY_POT = 1931, POT_OF_FLOUR = 1933, GRAIN = 1947, EMPTY_FLOUR_BIN = 1781,
      FULL_FLOUR_BIN = 1782;
  /**
  ...
  public static void grainOnHopper(Client c, int objectID, int itemId) {
    if (itemId == GRAIN) {
      ...
      if (c.grain == LIMIT - c.flourAmount || c.flourAmount == LIMIT) {
        c.getActionSender().sendMessage("You can't put anymore grain into the hopper.");
        return;
      }
  ...
  public static void hopperControl(final Client c) {
    if (c.grain > 0) {
      if (c.flourAmount == LIMIT) {
        c.getActionSender().sendMessage("There is currently too much grain in the hopper.");
        return;
      }
      c.getActionSender().sendMessage("You operate the hopper. The grain slides down the chute.");
      c.startAnimation(832);
      ...
      c.startAnimation(832);
      c.getActionSender().sendMessage("You operate the hopper. Nothing interesting happens.");
    }
  }
  ...
  public static void emptyFlourBin(Client c) {
    if (c.getItemAssistant().playerHasItem(EMPTY_POT, 1) && c.flourAmount > 0) {
      c.getItemAssistant().deleteItem(EMPTY_POT, 1);
      c.getItemAssistant().addItem(POT_OF_FLOUR, 1);
      c.getActionSender().sendMessage("You fill a pot with flour from the bin.");
      c.flourAmount--;
      if (c.flourAmount < 0) {
        c.flourAmount = 0;
      }
      ...
      c.getActionSender().sendMessage("You don't have an empty pot to fill flour with.");
    }
  }
}
