...
import server.game.objects.ObjectDefaults;
...
public class OtherObjects {
	...
	private final static int[] SPECIAL_OBJECTS = { 160, 155, 156, 298, 299, 300,
			304, 1181, 5253, 5254, 5255, 5256, 5257, 5258 };
		...
		for (int i = 0; i < SPECIAL_OBJECTS.length; i++) {
			if (objectType == SPECIAL_OBJECTS[i]) {
				if (System.currentTimeMillis() - player.searchObjectDelay < 1200
						|| objectType == 160 && player.absX != 3096
						|| objectType > 154 && objectType < 157
								&& player.absX != 3098
						|| player.absY == 3301) {
					return;
				}
	...
	private static void object(Client player, int id, int x, int y) {
		player.getActionSender().object(id, x, y, 0,
				ObjectDefaults.getObjectFace(player, id), 10);
	}

	private static void handleSpecialObject(final Client player,
		...
		if (objectType == 160 && player.absX == 3096) {
			...
			CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					player.getPlayerAssistant().movePlayer(3098, player.absY,
							0);
					container.stop();
				}
		...
		} else if (objectType > 154 && objectType < 157
				&& player.absX == 3098) {
			...
			if (player.playerEquipment[player.playerHands] > 0) {
				...
				player.getActionSender()
						.sendMessage("You have been stung by the nettles.");
				player.stopPlayerPacket = false;
			}
		} else if (objectName.startsWith("Hay")
				|| objectName.startsWith("hay")) {
			final int damage = 1, random = Misc.random(15);
			player.startAnimation(832);
			player.getActionSender().sendMessage(
					"You search the " + objectName.toLowerCase() + "...");
			CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
				@Override
				public void execute(CycleEventContainer container) {
					if (random == 1) {
						...
						player.getActionSender()
								.sendMessage("You find nothing of interest.");
						container.stop();
					}
				}
