...
public class CrystalChest {
  ...
  public static void makeKey(Client c) {
    if (c.getItemAssistant().playerHasItem(toothHalf(), 1) && c.getItemAssistant().playerHasItem(loopHalf(), 1)) {
  ...
  public static void searchChest(final Client c, final int id, final int x, final int y) {
    if (canOpen(c)) {
      c.getActionSender().sendMessage("You unlock the chest with your key.");
      ...
      CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
        ...
        public void execute(CycleEventContainer container) {
          ...
          c.getItemAssistant().addItem(CHEST_REWARDS[Misc.random(getLength() - 1)], 1);
          c.getActionSender().sendMessage("You find some treasure in the chest.");
