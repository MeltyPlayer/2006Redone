...
public class SpecialObjects {
  ...
  public static void openShantayChest(Client c, int objectId, int obX, int obY, String type) {
    if (type == "open") {
  ...
  public static void initKharid(Client c, int objectId) {
    if (!c.getItemAssistant().playerHasItem(995, 10)) {
      c.getDialogueHandler().itemMessage(c, "You need 10 coins to pass through this gate.", 995, 200);
      c.nextChat = 0;
      return;
    }
    ...
    coords[0] = c.objectX;
    coords[1] = c.objectY;
    c.getItemAssistant().deleteItem(995, c.getItemAssistant().getItemSlot(995), 10);
  }
