...
public class Levers {
	 ...
	 */
	private final static int[][] LEVERS = { { 3090, 3956, 2539, 4712 },
			{ 2539, 4712, 3090, 3956 }, { 2271, 4680, 3067, 10253 },
			{ 3067, 10253, 2271, 4680 }, { 3153, 3923, 2561, 3311 },
			{ 2561, 3311, 3153, 3923 } };
	...
	public static void pullLever(final Client player, int objectType) {
		...
		for (final int[] element : LEVERS) {
			if (player.objectX == element[0] && player.objectY == element[1]
					&& objectName.equalsIgnoreCase("Lever")) {
								...
								public void execute(
									...
									if (player.wildLevel > 20) {
										player.getPlayerAssistant()
												.startTeleport2(element[2],
														element[3], 0);
										container.stop();
									} else {
										player.getPlayerAssistant()
												.startTeleport(element[2],
														element[3], 0,
														"modern");
										container.stop();
									}
