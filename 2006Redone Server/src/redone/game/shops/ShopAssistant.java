...
public class ShopAssistant {
	...
	public static final int RANGE_SHOP = 111, PEST_SHOP = 175,
			CASTLE_SHOP = 112;
	...
	public void openShop(int ShopID) {
		...
		player.getPlayerAssistant().sendFrame126(ShopHandler.ShopName[ShopID],
				3901);
	}
	...
	public double getItemShopValue(int ItemID, int Type, int fromSlot) {
		...
		for (int i = 0; i < Constants.ITEM_LIMIT; i++) {
			if (ItemDefinitions.getDef()[i] != null) {
				ShopValue = (int) ItemDefinitions.getDef()[ItemID].highAlch / 3
						* 5;
	...
	public void buyFromShopPrice(int removeId, int removeSlot) {
		...
		String ShopAdd = "";
		if (player.myShopId == 138 || player.myShopId == 139
				|| player.myShopId == 58) {
			player.getActionSender()
					.sendMessage(ItemAssistant.getItemName(removeId)
							+ ": currently costs " + SpecialValue + " tokkul.");
			return;
		}
		if (player.myShopId == PEST_SHOP) {
			player.getActionSender()
					.sendMessage(ItemAssistant.getItemName(removeId)
							+ ": currently costs " + getPestItemValue(removeId)
							+ " pest control points.");
			return;
		}
		...
		if (player.myShopId == RANGE_SHOP) {
			player.getActionSender()
					.sendMessage(ItemAssistant.getItemName(removeId)
							+ ": currently costs " + getRGItemValue(removeId)
							+ " archery tickets.");
			return;
		}
		...
		player.getActionSender().sendMessage(ItemAssistant.getItemName(removeId)
	...
	public void sellToShopPrice(int removeId, int removeSlot) {
		for (int i : Constants.ITEM_SELLABLE) {
			if (i == removeId) {
				player.getActionSender().sendMessage("You can't sell "
		...
		if (ShopHandler.ShopSModifier[player.myShopId] > 1) {
			for (int j = 0; j <= ShopHandler.ShopItemsStandard[player.myShopId]; j++) {
				if (removeId == (ShopHandler.ShopItems[player.myShopId][j]
						- 1)) {
					IsIn = true;
					break;
				}
		...
		if (IsIn == false) {
			player.getActionSender()
					.sendMessage("You can't sell "
							+ ItemAssistant.getItemName(removeId).toLowerCase()
							+ " to this store.");
		} else {
			int ShopValue = (int) Math
					.floor(getItemShopValue(removeId, 1, removeSlot) * .85);
			String ShopAdd = "";
			if (ShopValue >= 1000 && ShopValue < 1000000) {
				ShopAdd = " (" + (ShopValue / 1000) + "K)";
			} else if (ShopValue >= 1000000) {
			...
			if (player.myShopId != RANGE_SHOP && player.myShopId != PEST_SHOP
					&& player.myShopId != CASTLE_SHOP && player.myShopId != 138
					&& player.myShopId != 58 && player.myShopId != 139) {
				player.getActionSender()
						.sendMessage(ItemAssistant.getItemName(removeId)
				...
				player.getActionSender()
						.sendMessage(ItemAssistant.getItemName(removeId)
								+ ": shop will buy for "
								+ getRGItemValue(removeId) + " archery tickets."
								+ ShopAdd);
			} else if (player.myShopId == PEST_SHOP) {
				player.getActionSender()
						.sendMessage(ItemAssistant.getItemName(removeId)
								+ ": shop will buy for "
								+ getPestItemValue(removeId)
								+ " pest control points." + ShopAdd);
			} else if (player.myShopId == CASTLE_SHOP) {
				player.getActionSender()
						.sendMessage(ItemAssistant.getItemName(removeId)
								+ ": shop will buy for "
								+ getCastleItemValue(removeId)
								+ " castle war tickets." + ShopAdd);
			}
			player.getActionSender()
					.sendMessage(ItemAssistant.getItemName(removeId)
							+ ": shop will buy for " + ShopValue + " coins."
	...
	public boolean sellItem(int itemID, int fromSlot, int amount) {
		...
		if (player.playerRights == 2 && !Constants.ADMIN_CAN_SELL_ITEMS) {
			player.getActionSender().sendMessage(
					"Selling items as an admin has been disabled.");
			return false;
		}
		...
		if (amount > 0 && itemID == (player.playerItems[fromSlot] - 1)) {
			if (ShopHandler.ShopSModifier[player.myShopId] > 1) {
				...
				for (int i = 0; i <= ShopHandler.ShopItemsStandard[player.myShopId]; i++) {
					if (itemID == (ShopHandler.ShopItems[player.myShopId][i]
							- 1)) {
						IsIn = true;
						break;
					}
			...
			if (amount > player.playerItemsN[fromSlot] && (ItemDefinitions
					.getDef()[player.playerItems[fromSlot]
							- 1].isNoteable == true
					|| ItemDefinitions.getDef()[player.playerItems[fromSlot]
							- 1].isStackable == true)) {
			...
			for (int i = amount; i > 0; i--) {
				TotPrice2 = (int) Math
						.floor(getItemShopValue(itemID, 1, fromSlot) * .85);
				if (player.getItemAssistant().freeSlots() > 0
						|| player.getItemAssistant().playerHasItem(995)) {
					if (ItemDefinitions.getDef()[itemID].isNoteable == false) {
						player.getItemAssistant().deleteItem(itemID,
								player.getItemAssistant().getItemSlot(itemID),
								1);
					} else {
						player.getItemAssistant().deleteItem(itemID, fromSlot,
								1);
					}
					player.getItemAssistant().addItem(995, TotPrice2);
					addShopItem(itemID, 1);
					if (player.getPlayerAssistant().isPlayer()) {
						GameLogger.writeLog(player.playerName, "shopselling",
								player.playerName + " sold " + itemName
										+ " to store id: " + player.myShopId
										+ " for"
										+ GameLogger.formatCurrency(TotPrice2)
										+ " coins");
					}
				} else {
					player.getActionSender().sendMessage(
	...
	private static final int FISHING_ITEMS[] = { 383, 371, 377, 359, 321, 341,
			353, 345, 327, 317 };
		...
		if (amount > 0) {
			...
			for (int i = 0; i < FISHING_ITEMS.length; i++) {
				if (player.myShopId == 32 && itemID == FISHING_ITEMS[i]) {
					player.getActionSender().sendMessage(
							"You can't buy that item from this store!");
					return false;
				}
			}
			...
			int castleSlot = 0;
			for (int i = amount; i > 0; i--) {
				if (player.myShopId != 138 && player.myShopId != 58
						&& player.myShopId != 139
						&& player.myShopId != RANGE_SHOP
						&& player.myShopId != PEST_SHOP
						&& player.myShopId != CASTLE_SHOP) {
					TotPrice2 = (int) Math
							.floor(getItemShopValue(itemID, 0, fromSlot));
				} else if (player.myShopId == 138 || player.myShopId == 58
						|| player.myShopId == 139) {
				...
				if (rangeSlot == -1) {
					if (player.myShopId == RANGE_SHOP) {
						player.getActionSender().sendMessage(
								"You don't have enough archery tickets to buy that.");
						break;
					}
				}
				if (castleSlot == -1) {
					if (player.myShopId == CASTLE_SHOP) {
						player.getActionSender().sendMessage(
								"You don't have enough castle wars tickets to buy that.");
						break;
					}
				}
				if (tokkulSlot == -1) {
					if (player.myShopId == 138 || player.myShopId == 58
							|| player.myShopId == 139) {
						player.getActionSender().sendMessage(
								"You don't have enough tokkul to buy that.");
						break;
					}
				}
				...
				String itemName = ItemAssistant.getItemName(itemID)
						.toLowerCase();
				if (player.getPlayerAssistant().isPlayer()) {
					GameLogger.writeLog(player.playerName, "shopbuying",
							player.playerName + " bought " + itemName
									+ " from store id: " + player.myShopId
									+ " for"
									+ GameLogger.formatCurrency(TotPrice2)
									+ " coins");
				}
				...
				if (player.myShopId == 138 || player.myShopId == 139
					...
					if (player.playerItemsN[tokkulSlot] >= TotPrice2) {
						if (player.getItemAssistant().freeSlots() > 0) {
							player.getItemAssistant().deleteItem(6529,
									tokkulSlot, TotPrice2);
							...
							ShopHandler.ShopItemsRestock[player.myShopId][fromSlot] = System
									.currentTimeMillis();
							if (fromSlot
									+ 1 > ShopHandler.ShopItemsStandard[player.myShopId]) {
								ShopHandler.ShopItems[player.myShopId][fromSlot] = 0;
							}
						} else {
							player.getActionSender().sendMessage(
					...
					if (player.playerItemsN[rangeSlot] >= TotPrice2) {
						if (player.getItemAssistant().freeSlots() > 0) {
							player.getItemAssistant().deleteItem(1464,
									rangeSlot, TotPrice2);
							player.getItemAssistant().addItem(itemID, 1);
							ShopHandler.ShopItemsN[player.myShopId][fromSlot] -= 1;
							ShopHandler.ShopItemsDelay[player.myShopId][fromSlot] = 0;
					...
					if (player.playerItemsN[castleSlot] >= TotPrice2) {
						if (player.getItemAssistant().freeSlots() > 0) {
							player.getItemAssistant().deleteItem(4067,
									castleSlot, TotPrice2);
							player.getItemAssistant().addItem(itemID, 1);
							ShopHandler.ShopItemsN[player.myShopId][fromSlot] -= 1;
							ShopHandler.ShopItemsDelay[player.myShopId][fromSlot] = 0;
							if (fromSlot
