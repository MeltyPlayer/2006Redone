...
public class CookingTutorialIsland extends SkillHandler {
	...
	private static void viewCookInterface(Client c, int item) {
		...
		c.getPlayerAssistant()
				.sendFrame126("" + ItemAssistant.getItemName(item) + "", 13717);
	}
			...
			CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					c.getItemAssistant().deleteItem(c.playerSkillProp[7][0],
							c.getItemAssistant()
									.getItemSlot(c.playerSkillProp[7][0]),
							1);
					if (c.Cookstage1 == 1) {
						...
						c.Cookstage1 = 0;
						c.getItemAssistant().addItem(c.playerSkillProp[7][3],
								1);
					} else {
						c.getActionSender().chatbox(6180);
						c.getDialogueHandler().chatboxText(c,
						...
						c.getActionSender().chatbox(6179);
						c.getActionSender().createArrow(3089, 3092, c.getH(),
								2);
						c.getPlayerAssistant()
								.addSkillXP(c.playerSkillProp[7][1], 7);
						c.getItemAssistant().addItem(c.playerSkillProp[7][4],
								1);
						c.tutorialProgress = 7;
					}
					deleteTime(c);
					if (!c.getItemAssistant()
							.playerHasItem(c.playerSkillProp[7][0], 1)
							|| c.doAmount <= 0) {
						container.stop();
					}
			...
			CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					if (c.playerSkillProp[7][5] > 0) {
						...
						// cook sound
						c.startAnimation(
								c.playerSkillProp[7][5] == 2732 ? 897 : 896);
					}
					if (!c.stopPlayerSkill) {
						container.stop();
					}
