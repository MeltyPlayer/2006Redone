...
public class Runecrafting {
	...
	public static void locate(Client c, int xPos, int yPos) {
		...
		c.getActionSender()
				.sendMessage("You need to travel " + Y + "-" + X + ".");
	}

	private enum Altars {
		AIR_ALTAR(2452, new int[] { 1438, 5527 },
				new int[] { 2842, 4829 }), MIND_ALTAR(2453,
						new int[] { 1448, 5529 },
						new int[] { 2793, 4828 }), WATER_ALTAR(2454,
								new int[] { 1444, 5531 },
								new int[] { 2713, 4836 }), EARTH_ALTAR(2455,
										new int[] { 1440, 5535 },
										new int[] { 2655, 4831 }), FIRE_ALTAR(
												2456, new int[] { 1442, 5537 },
												new int[] { 2577,
														4845 }), BODY_ALTAR(
																2457,
																new int[] {
	...
	public void enterAltar(int objId, int itemUse) {
		...
		if (a != null) {
			 ...
			 * "You need to beat rune mysteries first to do this."); c.nextChat
			 * = 0; return; }
			 */
			if (a.getKeys()[1] == c.playerEquipment[c.playerHat]
				...
				c.getPlayerAssistant().movePlayer(a.getNewLoc()[0],
						a.getNewLoc()[1], 0);
				c.getActionSender()
						.sendMessage("You enter the mysterious ruins.");
			} else {
	...
	private enum Altar_Data {
		AIR(2478, 1, 6, 556, new int[][] {
				{ 11, 2 }, { 22, 3 }, { 33, 4 }, { 44, 5 }, { 55, 6 },
				{ 66, 7 }, { 77, 8 }, { 88, 9 }, { 99, 9 } }), MIND(2479, 5, 6,
						558,
						new int[][] {
								{ 14, 2 }, { 28, 3 }, { 42, 4 }, { 56, 5 },
								{ 70, 6 }, { 84, 7 }, { 98, 8 } }), WATER(2480,
										9, 7, 555,
										new int[][] { { 19, 2 }, { 38, 3 },
												{ 57, 4 }, { 76, 5 },
												{ 95, 6 } }), EARTH(2481, 9, 7,
		...
		if (ad != null) {
			...
			if (amount > 0) {
				...
				c.getItemAssistant().addItem(ad.rewardedRune,
						amount * (getMultiplier(ad) == 1 ? getMultiplier(ad)
								: getMultiplier(ad) - 1));
				...
				c.getItemAssistant().addItem(ad.rewardedRune,
						amount2 * (getMultiplier(ad) == 1 ? getMultiplier(ad)
								: getMultiplier(ad) - 1));
				...
				c.getActionSender()
						.sendMessage("You don't have any essence left.");
			}
		}
	}
