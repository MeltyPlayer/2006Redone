...
public class GlassBlowing {
	...
	public int[][] data = { { 44210, 229, 33, 35 }, { 48108, 567, 46, 53 },
			{ 48112, 1919, 1, 18 }, { 48116, 4527, 4, 19 },
			{ 48120, 4522, 12, 25 }, { 24059, 6667, 42, 42 },
			{ 48124, 4542, 49, 55 } };
	...
	public void makeItem(final int id, final int amount, final int slot) {
		if (c.playerLevel[Constants.CRAFTING] < data[slot][2]) {
			c.getActionSender()
					.sendMessage("You need a crafting level of " + data[slot][2]
							+ " make a " + ItemAssistant.getItemName(id) + "");
			return;
		}
		if (!started) {
				...
				public void execute(CycleEventContainer container) {
					...
					if (c.getItemAssistant().playerHasItem(1775)) {
						if (c.getItemAssistant().playerHasItem(1785)) {
							...
							c.getPlayerAssistant().addSkillXP(data[slot][3],
									Constants.CRAFTING);
