...
public class Mining {
	...
	public void startMining(final Client player, final int objectNumber,
			final int objectX, final int objectY, final int type) {
		if (player.isMining)
			return;
		...
		if (Rock_Settings[objectNumber][1] > miningLevel) {
			player.getActionSender().sendMessage("You need a Mining level of "
					+ Rock_Settings[objectNumber][1] + " to mine this rock.");
			return;
		}
			...
			public void execute(CycleEventContainer container) {
				...
				if (player.isMining) {
					player.getItemAssistant()
							.addItem(Rock_Settings[objectNumber][4], 1);
					player.getPlayerAssistant().addSkillXP(
							Rock_Settings[objectNumber][2],
							player.playerMining);
					player.getActionSender().sendMessage(
							"You manage to mine some " + ItemAssistant
									.getItemName(Rock_Settings[objectNumber][4])
									.toLowerCase() + ".");
				}
				if (player.tutorialProgress == 17) {
					if (objectNumber == 18) {
						player.getDialogueHandler()
								.sendStatement("You should mine tin first.");
						resetMining(player);
						return;
					}
					if (player.getItemAssistant().playerHasItem(438)) {
						player.getActionSender().createArrow(3086, 9501,
								player.getH(), 2);
						player.getDialogueHandler().chatboxText(player,
					...
					if (objectNumber == 19) {
						player.getDialogueHandler().sendStatement(
								"You have already mined this type of ore, now try the other.");
						resetMining(player);
						return;
					}
				...
				mineRock(Rock_Settings[objectNumber][3] + getRandomAdd(),
						objectX, objectY, type, Rock_Settings[objectNumber][0]);
	...
	public int getTimer(int b, int c, int level) {
		double timer = (int) ((Rock_Settings[b][1] * 2) + 20 + Misc.random(20))
				- ((Pick_Settings[c][2] * (Pick_Settings[c][2] * 0.75))
						+ level);
		if (timer < 2.0) {
			return 2;
		} else {
	...
	public static void prospectRock(final Client c, final String itemName) {
			...
			CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					if (c.tutorialProgress == 15) {
						c.getActionSender().sendMessage("This rock contains "
						...
						c.getActionSender().createArrow(3086, 9501, c.getH(),
								2);
						c.getActionSender().chatbox(6179);
		...
		CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				c.getActionSender()
						.sendMessage("This rock contains " + itemName + ".");
				container.stop();
			}
	...
	public static enum rockData {
		ESSENCE(new int[] { 2491 }, 1, 5, 3, 1, new int[] { 1436 }), CLAY(
				new int[] { 2108, 2109, 11189, 11190, 11191, 9713, 9711, 14905,
						14904 },
				1, 5, 1, 5, new int[] { 434 }), COPPER(
						new int[] { 3042, 2091, 2090, 9708, 9709, 9710, 11960,
								14906, 14907 },
						1, 18, 1, 8, new int[] { 436 }), TIN(
								new int[] { 2094, 2095, 3043, 9716, 9714, 11958,
										11957, 11959, 11933, 11934, 11935,
										14903, 14902 },
								1, 18, 1, 8, new int[] { 438 }), BLURITE(
										new int[] { 10574, 10583, 2110 }, 10,
										20, 1, 8,
										new int[] { 668 }), IRON(new int[] {
												2093, 2092, 9717, 9718, 9719,
												11962, 11956, 11954, 14856,
												14857, 14858, 14914, 14913 },
												15, 35, 2, 5,
												new int[] { 440 }), SILVER(
														new int[] { 2101, 11186,
																11187, 11188,
		...
		private rockData(final int[] objectId, final int levelReq, final int xp,
				final int mineTimer, final int respawnTimer,
				final int... oreIds) {
