...
public class DwarfCannon {
	...
	public void placeCannon() {
		...
		if (noSetUpArea()) {
			player.getActionSender().sendMessage(
					"You are not allowed to set up a cannon here!");
			return;
		}
		if (hasCannon()) {
			player.getActionSender()
					.sendMessage("You already have a cannon placed!");
			return;
		}
		if (nearCannon()) {
			player.getActionSender().sendMessage(
					"You must be farther away from an existing cannon to set a new one up!");
			return;
		}
		if (!canSetUp()) {
			player.getActionSender().sendMessage(
					"You need all the parts of the cannon to set a cannon up.");
			return;
		}
		for (int i = 0; i < 50; i++) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void stop() {
				...
				player.getItemAssistant().deleteItem(ballsID,
						player.getItemAssistant().getItemSlot(ballsID), amount);
				myBalls = cballs;
			}
		}, 2);
	}
	...
	public void loginCheck() {
		if (needsCannon()) {
			player.getActionSender().sendMessage(
					"@red@You can collect your cannon at home from Nulodion.");
		}
	}

	private boolean canSetUp() {
		if (setUpStage == 0) {
			if (player.getItemAssistant().playerHasItem(ITEM_PARTS[0])
					&& player.getItemAssistant().playerHasItem(ITEM_PARTS[1])
					&& player.getItemAssistant().playerHasItem(ITEM_PARTS[2])
					&& player.getItemAssistant().playerHasItem(ITEM_PARTS[3])) {
				return true;
			}
		} else if (setUpStage == 1) {
			if (player.getItemAssistant().playerHasItem(ITEM_PARTS[1])
					&& player.getItemAssistant().playerHasItem(ITEM_PARTS[2])
					&& player.getItemAssistant().playerHasItem(ITEM_PARTS[3])) {
				return true;
			}
		} else if (setUpStage == 2) {
			if (player.getItemAssistant().playerHasItem(ITEM_PARTS[2])
					&& player.getItemAssistant().playerHasItem(ITEM_PARTS[3])) {
				return true;
			}
	...
	public boolean hasCannon() {
		return (player.cannonX > 0) && (player.cannonY > 0)
				|| (player.cannonX > 0 && player.cannonY > 0);
	}
	...
	public boolean nearCannon() {
		for (int i = 0; i < Server.cannonsX.length; i++) {
			if ((player.absX >= Server.cannonsX[i] - 1)
					&& (player.absX <= Server.cannonsX[i] + 1)
					&& (player.absY >= Server.cannonsY[i] - 2)
					&& (player.absY <= Server.cannonsY[i] + 1)) {
				return true;
			}
		}
		return false;
	...
	public void loadCannons() {
		for (int i = 0; i < Server.cannonsX.length; i++) {
			if (Server.cannonsX[i] != 0) {
				player.getActionSender().checkObjectSpawn(6, Server.cannonsX[i],
						Server.cannonsY[i], 0, 10);
			}
		}
	}
	...
	public void loadCannon(int x, int y) {
		...
		if (!myCannon(x, y)) {
			player.getActionSender()
					.sendMessage("You can't load somebody else's cannon!");
			return;
		}
		if (myBalls <= 29) {
			int amount = cballs - myBalls;
			...
			if (player.getItemAssistant().playerHasItem(ballsID)) {
				player.getActionSender()
						.sendMessage(amount > 1
								? "You load the cannon with " + amount
										+ " cannonballs."
								: "You load the cannon with 1 cannonball.");
			} else {
				player.getActionSender().sendMessage(
						"You have no cannonballs to load into the cannon.");
			}
		}
	}
	...
	public void clickCannon(int x, int y) {
		if (!myCannon(x, y)) {
			player.getActionSender()
					.sendMessage("You can't fire somebody else's cannon!");
			return;
		}
		if (myBalls == 0) {
			player.getActionSender()
					.sendMessage("Your cannon has run out of cannonballs.");
			return;
		}
		if (myBalls >= 1 && rotating == false) {
			...
			player.getActionSender()
					.sendMessage("Your cannon is already shooting.");
		}
	}
	...
	public void handleDisconnect() {
		...
		for (int i = 0; i < Server.cannonsX.length; i++) {
			if (Server.cannonsX[i] == player.cannonX
					&& Server.cannonsY[i] == player.cannonY) {
				Server.cannonsX[i] = 0;
				Server.cannonsY[i] = 0;
	...
	private void rotateCannon() {
		switch (rotation) {
		case 1: // north
			player.getPlayerAssistant().objectAnim(player.cannonX,
					player.cannonY, 516, 10, -1);
			break;
		case 2: // north-east
			player.getPlayerAssistant().objectAnim(player.cannonX,
					player.cannonY, 517, 10, -1);
			break;
		case 3: // east
			player.getPlayerAssistant().objectAnim(player.cannonX,
					player.cannonY, 518, 10, -1);
			break;
		case 4: // south-east
			player.getPlayerAssistant().objectAnim(player.cannonX,
					player.cannonY, 519, 10, -1);
			break;
		case 5: // south
			player.getPlayerAssistant().objectAnim(player.cannonX,
					player.cannonY, 520, 10, -1);
			break;
		case 6: // south-west
			player.getPlayerAssistant().objectAnim(player.cannonX,
					player.cannonY, 521, 10, -1);
			break;
		case 7: // west
			player.getPlayerAssistant().objectAnim(player.cannonX,
					player.cannonY, 514, 10, -1);
			break;
		case 8: // north-west
			player.getPlayerAssistant().objectAnim(player.cannonX,
					player.cannonY, 515, 10, -1);
			rotation = 0;
			break;
		}
	}
	...
	public void pickup(int x, int y) {
		if (!myCannon(x, y)) {
			player.getActionSender()
					.sendMessage("You can't pick up somebody else's cannon!");
			return;
		}
		if (rotating == true) {
			rotating = false;
		...
		if (player.getItemAssistant().freeSlots() > 3) {
			player.startAnimation(827);
			player.getActionSender()
					.sendMessage("You pick up the cannon. It's really heavy.");
			...
			player.getActionSender().sendMessage(
					"You don't have enough free inventory slots to do that.");
		}
		if (myBalls > 0) {
	...
	public boolean noSetUpArea() {
		return (player.absX >= 2024 && player.absX <= 2047
				&& player.absY >= 2047 && player.absY <= 4542)
				|| player.inBank() || player.inFightCaves()
				|| (player.absX >= 3161 && player.absX <= 3168
						&& player.absY >= 3486 && player.absY <= 3493);
	}
	...
	public void shootNpcs() {
		...
		if (target != null) {
			...
			if (player.inMulti() && target.inMulti()) {
				...
				myBalls -= 1;
				player.getPlayerAssistant().addSkillXP(damage * expRate,
						player.playerRanged);
			}
			if (!player.inMulti()) {
				if (target.underAttackBy > 0
						&& target.underAttackBy != player.playerId) {
					return;
				}
				if (player.underAttackBy2 > 0
						&& player.underAttackBy2 != target.npcId) {
					return;
				}
				cannonProjectile(target);
				target.hitDiff2 = damage;
	...
	private Npc targetNpc() {
		for (int i = 0; i < NpcHandler.maxNPCs; i++) {
			...
			int theirX = npc.absX;
			int theirY = npc.absY;
			if (!npc.isDead && !npc.isDead && npc.HP != 0 && npc.npcType != 1266
					&& npc.npcType != 1268 && inDistance(theirX, theirY)) {
				switch (rotation) {
				case 1:
	...
	public boolean inDistance(int npcX, int npcY) {
		return (npcX >= player.cannonX - maxDistance
				&& npcX <= player.cannonX + maxDistance
	...
	private void cannonProjectile(Npc n) {
		...
		player.getPlayerAssistant().createPlayersProjectile(oX, oY, offY, offX,
				50, 60, 53, 20, 20, -player.oldNpcIndex + 1, 30);
	}
