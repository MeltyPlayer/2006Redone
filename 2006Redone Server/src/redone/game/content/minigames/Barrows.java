...
public class Barrows {
	...
	public void spadeDigging() {
		if (c.inArea(spadeData[0][0], spadeData[0][1], spadeData[0][2],
				spadeData[0][3])) {
			c.getPlayerAssistant().movePlayer(spadeData[0][4], spadeData[0][5],
					3);
		} else if (c.inArea(spadeData[1][0], spadeData[1][1], spadeData[1][2],
				spadeData[1][3])) {
			c.getPlayerAssistant().movePlayer(spadeData[1][4], spadeData[1][5],
					3);
		} else if (c.inArea(spadeData[2][0], spadeData[2][1], spadeData[2][2],
				spadeData[2][3])) {
			c.getPlayerAssistant().movePlayer(spadeData[2][4], spadeData[2][5],
					3);
		} else if (c.inArea(spadeData[3][0], spadeData[3][1], spadeData[3][2],
				spadeData[3][3])) {
			c.getPlayerAssistant().movePlayer(spadeData[3][4], spadeData[3][5],
					3);
		} else if (c.inArea(spadeData[4][0], spadeData[4][1], spadeData[4][2],
				spadeData[4][3])) {
			c.getPlayerAssistant().movePlayer(spadeData[4][4], spadeData[4][5],
					3);
		} else if (c.inArea(spadeData[5][0], spadeData[5][1], spadeData[5][2],
				spadeData[5][3])) {
			c.getPlayerAssistant().movePlayer(spadeData[5][4], spadeData[5][5],
					3);
			...
			NpcHandler.spawnNpc(c, 1217, c.absX + Misc.random(1),
					c.absY + Misc.random(1), c.heightLevel, 0, 10, 2, 5, 5,
					true, true);
			c.getDialogueHandler().sendNpcChat1(
					"First moles, now this! Take this, vanda!", c.talkingNpc,
					"Gardener");
			c.pirateTreasure = 5;
	...
	public void useStairs() {
		if (c.isInBarrows2()) {
			switch (c.objectId) {
			case 6703:
				c.getPlayerAssistant().movePlayer(barrowData[0][5],
						barrowData[0][6], 0);
				break;
			case 6707:
				c.getPlayerAssistant().movePlayer(barrowData[1][5],
						barrowData[1][6], 0);
				break;
			case 6702:
				c.getPlayerAssistant().movePlayer(barrowData[2][5],
						barrowData[2][6], 0);
				break;
			case 6706:
				c.getPlayerAssistant().movePlayer(barrowData[3][5],
						barrowData[3][6], 0);
				break;
			case 6704:
				c.getPlayerAssistant().movePlayer(barrowData[4][5],
						barrowData[4][6], 0);
				break;
			case 6705:
				c.getPlayerAssistant().movePlayer(barrowData[5][5],
						barrowData[5][6], 0);
				break;
			}
		} else {
			c.getActionSender()
					.sendMessage("You have to be in barrows to do this!");
		}
	}
		...
		if (c.isInBarrows2()) {
			if (c.barrowsKillCount < 5) {
				c.getActionSender().sendMessage(
						"You still have to kill the following brothers:");
				...
				NpcHandler.spawnNpc(c, 2026, c.getX(), c.getY() - 1, 3, 0, 120,
						25, 200, 200, true, true);
				c.getPlayerAssistant().removeAllWindows();
			} else if (c.barrowsKillCount > 5) {
	...
	public void useChest() {
		if (c.isInBarrows2()) {
			if (!checkBarrows()) {
				c.getActionSender()
						.sendMessage("You haven't killed all the brothers!");
				return;
			}
			if (c.barrowsKillCount == 5) {
				if (c.barrowsNpcs[4][1] == 0) {
					NpcHandler.spawnNpc(c, 2026, c.getX(), c.getY() - 1, 0, 0,
	...
	public void fixAllBarrows() {
		...
		for (int j = 0; j < c.playerItems.length; j++) {
			...
			for (int i = 0; i < c
					.getItemAssistant().brokenBarrows.length; i++) {
				if (c.playerItems[j]
						- 1 == c.getItemAssistant().brokenBarrows[i][1]) {
					...
					c.playerItems[j] = c.getItemAssistant().brokenBarrows[i][0]
							+ 1;
				}
			}
			if (breakOut)
		...
		if (totalCost > 0)
			c.getItemAssistant().deleteItem(995,
					c.getItemAssistant().getItemSlot(995), totalCost);
	}
