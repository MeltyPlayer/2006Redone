...
public class Commands implements PacketType {
	...
	public void processPacket(Client player, int packetType, int packetSize) {
		...
		if ((playerCommand.startsWith("ban") || playerCommand.startsWith("ip")
				|| playerCommand.startsWith("mute")
				|| playerCommand.startsWith("un")) && player.playerRights > 0
				&& player.playerRights < 4) {
			GameLogger.writeLog(player.playerName, "commands",
					player.playerName + " used command: " + playerCommand + "");
		}
	...
	public static void playerCommands(Client player, String playerCommand) {
		if (playerCommand.equalsIgnoreCase("players")) {
			if (PlayerHandler.getPlayerCount() > 1) {
				...
				player.getActionSender().sendMessage("There is currently "
						+ PlayerHandler.getPlayerCount() + " player online.");
			}
		}
		...
		if (playerCommand.startsWith("yell")) {
			for (int j = 0; j < PlayerHandler.players.length; j++) {
				if (PlayerHandler.players[j] != null) {
					...
					if (player.playerRights == 1) {
						c2.getActionSender().sendMessage("@blu@[Moderator]@bla@"
								+ Misc.optimizeText(player.playerName) + ": "
								+ Misc.optimizeText(playerCommand.substring(5))
								+ "");
					} else if (player.playerRights == 2) {
						c2.getActionSender()
								.sendMessage("@gre@[Administator]@bla@"
										+ Misc.optimizeText(player.playerName)
										+ ": "
										+ Misc.optimizeText(
						...
						c2.getActionSender().sendMessage("@red@[Owner]@bla@"
								+ Misc.optimizeText(player.playerName) + ": "
								+ Misc.optimizeText(playerCommand.substring(5))
								+ "");
					}
				...
				for (int i = 0; i < Constants.MAX_PLAYERS; i++) {
					if (PlayerHandler.players[i] != null) {
						if (PlayerHandler.players[i].playerName
							...
							c2.getActionSender()
									.sendMessage("You have been muted by: "
											+ player.playerName);
							break;
						}
					}
			...
			try {
				...
				for (int i = 0; i < Constants.MAX_PLAYERS; i++) {
					if (PlayerHandler.players[i] != null) {
						if (PlayerHandler.players[i].playerName
								.equalsIgnoreCase(playerToBan)) {
							Connection.unIPMuteUser(
									PlayerHandler.players[i].connectedFrom);
							player.getActionSender().sendMessage(
									"You have Un Ip-Muted the user: "
											+ PlayerHandler.players[i].playerName);
							break;
						}
					}
	...
	public static void adminCommands(Client player, String playerCommand) {
			...
			try {
				...
				for (int i = 0; i < Constants.MAX_PLAYERS; i++) {
					if (PlayerHandler.players[i] != null) {
						if (PlayerHandler.players[i].playerName
								.equalsIgnoreCase(playerToBan)) {
							Connection.addIpToBanList(
									PlayerHandler.players[i].connectedFrom);
							Connection.addIpToFile(
									PlayerHandler.players[i].connectedFrom);
							player.getActionSender()
		...
		if (playerCommand.startsWith("unban")) {
			try {
				...
				player.getActionSender()
						.sendMessage(playerToBan + " has been unbanned.");
			} catch (Exception e) {
				player.getActionSender().sendMessage("Player Must Be Offline.");
			}
		}
				...
				for (int i = 0; i < PlayerHandler.players.length; i++) {
					if (PlayerHandler.players[i] != null) {
						if (PlayerHandler.players[i].playerName
							...
							player.getActionSender().sendMessage(p.playerName
									+ " has been teleported to you.");
							p.getPlayerAssistant().movePlayer(player.absX,
		...
		if (playerCommand.startsWith("tele")) {
			...
			if (arg.length > 3) {
				player.getPlayerAssistant().movePlayer(Integer.parseInt(arg[1]),
		...
		if (playerCommand.startsWith("setlevel")) {
			try {
				...
				player.playerXP[skill] = player.getPlayerAssistant()
						.getXPForLevel(level) + 5;
				player.playerLevel[skill] = player.getPlayerAssistant()
			...
			try {
				...
				if (args.length == 3) {
					...
					if (newItemID <= 10000 && newItemID >= 0) {
						player.getItemAssistant().addItem(newItemID,
								newItemAmount);
						if (player.isBusy()) {
							player.getPlayerAssistant().closeAllWindows();
						}
						player.getActionSender().sendMessage("You spawn ("
								+ newItemAmount + ") "
								+ ItemAssistant.getItemName(newItemID) + ".");
					} else {
						player.getActionSender().sendMessage("No such item.");
					}
				} else {
					player.getActionSender()
							.sendMessage("Use as ::item 995 200");
				}
			} catch (Exception e) {
			}
		}
		...
		if (playerCommand.equalsIgnoreCase("master")) {
			for (int i = 0; i < 25; i++) {
				...
				player.playerXP[i] = player.getPlayerAssistant()
						.getXPForLevel(100);
	...
	public static void developerCommands(Client player, String playerCommand) {
				...
				for (int i = 0; i < Constants.MAX_PLAYERS; i++) {
					if (PlayerHandler.players[i] != null) {
						if (PlayerHandler.players[i].playerName
							...
							Client c2 = (Client) PlayerHandler.players[i];
							player.getActionSender().sendMessage(
									"You have given " + playerToMod + " mod.");
							c2.playerRights = 1;
							c2.logout();
							break;
						}
		...
		if (playerCommand.startsWith("object")) {
			String[] args = playerCommand.split(" ");
			player.getActionSender().object(Integer.parseInt(args[1]),
					player.absX, player.absY, player.heightLevel, 0, 10);
			Region.addObject(Integer.parseInt(args[1]), player.absX,
					player.absY, player.heightLevel, 10, 0, false);
		}

		if (playerCommand.startsWith("object2")) {
			String[] args = playerCommand.split(" ");
			player.getActionSender().object(Integer.parseInt(args[1]),
					player.absX, player.absY, player.heightLevel, 0, 0);
			Region.addObject(Integer.parseInt(args[1]), player.absX,
					player.absY, player.heightLevel, 0, 0, false);
		}
		...
		if (playerCommand.startsWith("npc")) {
			try {
				...
				if (newNPC > 0) {
					NpcHandler.spawnNpc(player, newNPC, player.absX,
							player.absY, player.heightLevel, 0,
							NpcHandler.getNpcListHP(newNPC), maxHit, attack,
							defence, attackPlayer, false);
					player.getActionSender()
							.sendMessage("You spawn a " + NpcHandler
									.getNpcListName(newNPC).toLowerCase()
									+ ".");
					// player.npcSpawned = newNPC;
				} else {
					player.getActionSender()
							.sendMessage("Npc " + newNPC + " does not exist.");
				}
			} catch (Exception e) {
		...
		if (playerCommand.equalsIgnoreCase("cantAttack")) {
			if (player.npcCanAttack == true) {
				player.getActionSender()
						.sendMessage("Npcs can no longer attack you.");
				player.npcCanAttack = false;
			} else if (player.npcCanAttack == false) {
				player.getActionSender()
						.sendMessage("Npcs can attack you again.");
		...
		if (playerCommand.startsWith("sound")) {
			String[] args = playerCommand.split(" ");
			player.getActionSender().sendSound(Integer.parseInt(args[1]), 100,
					0);
		}
		...
		if (playerCommand.equalsIgnoreCase("run")) {
			player.getActionSender()
					.sendMessage("You have refilled your run-energy!");
		...
		if (playerCommand.startsWith("update")) {
			try {
				...
				if (args.length == 2) {
					...
					player.getActionSender()
							.sendMessage("Use as ::update (seconds)");
				}
			} catch (Exception e) {
			}
		}
