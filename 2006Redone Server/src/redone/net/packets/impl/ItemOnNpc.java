...
public class ItemOnNpc implements PacketType {
	...
	public void processPacket(final Client player, int packetType,
		...
		switch (npcId) {
		case 43:
			NpcHandler.npcs[i].requestTransformTime(player, 1735, 893, 43, 42,
					50);
			break;
		}
		if (player.getItemAssistant().playerHasItem(995, 1) && npcId == 736) {
			player.getItemAssistant().deleteItem2(995, 1);
			player.getDialogueHandler().sendNpcChat1("Thanks!", player.npcType,
					"Emily");
		}
		if (player.getItemAssistant().playerHasItem(1927, 1)
				&& player.gertCat == 2 && npcId == 2997) {
